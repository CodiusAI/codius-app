---
title: Voice
description: Codius voice architecture, local-first model execution, and provider configuration.
nav: Voice
order: 41
...
Codius has first-class voice support for dictation and voice mode conversations with your coding environment.

## Philosophy

Voice is local-first. You can run speech fully on-device, or choose OpenAI for speech features. For voice reasoning/orchestration, Codius reuses agent providers already installed and authenticated on your machine.
...
Missing models are downloaded at daemon startup into `$CODIUS_HOME/models/local-speech`. Downloads happen only for missing files.
...
```json
{
  ...
  "providers": {
    "local": {
      "modelsDir": "~/.codius/models/local-speech"
    }
  }
}
```
...
The `language` field applies only to the OpenAI STT provider: set `features.dictation.stt.language` for dictation and `features.voiceMode.stt.language` for voice mode. If voice language is omitted, Codius uses the dictation language before falling back to `en`. It has no effect on the local Parakeet models.
...
- `CODIUS_VOICE_LLM_PROVIDER`, voice agent provider override
- `CODIUS_DICTATION_STT_PROVIDER`, `CODIUS_VOICE_STT_PROVIDER`, `CODIUS_VOICE_TTS_PROVIDER`, speech provider selection (`local` or `openai`)
...
- `CODIUS_LOCAL_MODELS_DIR`, local model storage directory
- `CODIUS_DICTATION_LOCAL_STT_MODEL`, local dictation STT model ID
- `CODIUS_VOICE_LOCAL_STT_MODEL`, `CODIUS_VOICE_LOCAL_TTS_MODEL`, local voice STT/TTS model IDs
- `CODIUS_DICTATION_LANGUAGE`, dictation STT language (OpenAI STT only; ignored by local Parakeet)
- `CODIUS_VOICE_LANGUAGE`, voice mode STT language; falls back to `CODIUS_DICTATION_LANGUAGE` when unset (OpenAI STT only; ignored by local Parakeet)
