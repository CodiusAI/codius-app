...
description: "Codius orchestration skills: teach coding agents to spawn, coordinate, and manage other agents using slash commands."
nav: Skills
order: 32
...
Codius ships orchestration skills that teach coding agents how to use Codius tools and the CLI to spawn, coordinate, and manage other agents. Skills package common workflows as slash commands, so agents know how to orchestrate without you writing the briefing and safety rails each time.
...
## `/codius`, Codius Reference

The foundational skill. Codius reference for managing agents and workspaces. Load it when an agent needs to create agents, send them prompts, or manage workspace isolation.

Not typically invoked directly by users, it's a reference that other skills depend on.

```
/codius show me the Codius CLI surface for creating an agent in a worktree-isolated workspace
```

## `/codius-handoff`, Task Handoff
...
```
/codius-handoff hand off the auth fix to codex in a worktree-isolated workspace
/codius-handoff hand this to claude opus for review
```

## `/codius-loop`, Iterative Loops
...
```
/codius-loop keep trying until the changed test file passes, max 5 iterations
/codius-loop babysit PR 123 until checks are green, check every 2m, max-time 1h
```

## `/codius-committee`, Committee Planning
...
```
/codius-committee why are the websocket connections dropping under load?
/codius-committee plan the auth system migration
```

## `/codius-advisor`, Advisor
...
```
/codius-advisor did I miss anything in this migration plan?
/codius-advisor --provider claude/opus what is the UX risk in this flow?
```
