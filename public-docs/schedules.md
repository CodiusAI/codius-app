...
Codius also has **heartbeats**. A heartbeat sends a recurring prompt back into one existing agent so it can reassess and continue the same conversation.
...
- **[From the CLI](/docs/schedules-cli)** — `codius schedule create`, for headless boxes and scripts.
