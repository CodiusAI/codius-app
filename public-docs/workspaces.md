---
title: Workspaces
description: Understand how Codius groups working directories, agents, terminals, and browsers into workspaces.
nav: Workspaces
order: 10
category: Workspaces
---
...
Codius is organized around workspaces, not chats.
...
The sidebar starts with projects. A project can be a git repository, a GitHub project, or any directory on a machine running the Codius daemon.
...
In Codius, the workspace is the stable container. The sessions are what you run inside it.
...
The workspace is the product concept; a git worktree is one way to isolate its files. More than one workspace can refer to the same managed worktree, and Codius removes that worktree after its last workspace is archived.
...
codius workspace create --isolation worktree --path ~/dev/my-app --base main
```
...
Creating an agent and creating a workspace are separate actions. Pass a workspace ID when you want an agent in a specific existing workspace. A bare `codius run` from a human shell creates a new local workspace; when one agent runs it, Codius recognizes the caller and creates a subagent in the caller's workspace.

## Worktrees

Every workspace in Codius is backed by a working directory. When that directory is a git worktree, you get a separate branch and isolated environment for each task.
