...
Read the **codius** skill. Before choosing committee members, read `~/.codius/orchestration-preferences.json` unless the user explicitly named providers in this request. Do not create committee agents until you have read it.
...
Create both agents in parallel via Codius with `[Committee] <task>` titles and the same prompt. Wait for both — not just whichever finishes first.
