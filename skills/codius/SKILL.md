...
**`archive_workspace`** — `{ workspaceId }`. Archives the workspace, its agents, and its terminals. Local directories remain; Codius removes an owned worktree only after its final active workspace reference is archived.
...
All providers shown in this document are examples, you must resolve real providers either found in the preferences or by calling Codius provider tooling to find a valid provider.
...
codius workspace create --isolation worktree --mode checkout-branch --branch existing-work
codius workspace create --isolation worktree --mode checkout-pr --pr-number 42
codius run --provider codex/gpt-5.4 --mode full-access --workspace <workspace-id> "<prompt>"
codius run --provider codex/gpt-5.4 --mode full-access --new-workspace worktree --worktree-mode branch-off --new-branch fix-x --base main "<prompt>"
codius send <agent-id> "<follow-up>"
codius ls
codius schedule create --cron "*/15 * * * *" "ping main build"
codius heartbeat create --cron "*/15 * * * *" "check the build"
```
...
- Windows: `C:\Program Files\Codius\resources\bin\codius.cmd`

The desktop app's first-run hook (`installCli`) symlinks this to `~/.local/bin/codius` (macOS/Linux) or drops a `.cmd` trampoline (Windows) and adds `~/.local/bin` to PATH via shell rc files. If that didn't take, offer to symlink it — don't do it silently.
...
|                | Default                                                          |
| -------------- | ---------------------------------------------------------------- |
| Listen address | `127.0.0.1:6767` (override `CODIUS_LISTEN`)                      |
| Home           | `~/.codius` (override `CODIUS_HOME`)                             |
| Daemon log     | `$CODIUS_HOME/daemon.log`                                        |
| Agent state    | `$CODIUS_HOME/agents/<id>.json`                                  |
| Worktrees      | `$CODIUS_HOME/worktrees/` (or `worktrees.root` in `config.json`) |
| PID file       | `$CODIUS_HOME/codius.pid`                                        |
