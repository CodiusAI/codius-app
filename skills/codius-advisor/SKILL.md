...
Read the **codius** skill. Before choosing a provider, read `~/.codius/orchestration-preferences.json` unless the user explicitly named a provider in this request. Do not create the advisor until you have read it.
...
- `/codius-advisor /unslop` → advisor runs `/unslop` on the current diff.
- `/codius-advisor /unslop-risk` → advisor does an unslop-risk review.
- `/codius-advisor $diagnose this build failure` → advisor invokes `/diagnose`.
...
Create the advisor agent via Codius with a `[Advisor] <topic>` title and the briefing as the initial prompt. Wait for it to finish. Read its response. Synthesize for the user — the advisor's verdict + your recommendation.
