...
| ------------------------------------- | ------------------------ |
| `CODIUS_GIT_MAX_PROCESSES_PER_SECOND` | `maxProcessesPerSecond`  |
| `CODIUS_GIT_MAX_PROCESS_CONCURRENCY`  | `maxProcessConcurrency`  |
| `CODIUS_GIT_CONCURRENCY`              | Legacy concurrency alias |

`CODIUS_GIT_MAX_PROCESS_CONCURRENCY` wins when it and the legacy alias are both set. Restart the
daemon after changing the file or environment. Run `codius daemon restart` for a standalone daemon.
For a desktop-managed daemon, fully quit and reopen Codius Desktop.
