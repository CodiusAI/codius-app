...
Codius surfaces terminal activity as a tab indicator (the same "running" dot used by agents).
...
OpenCode uses a server plugin instead of command hooks. The plugin listens to OpenCode bus events and emits these Codius hook events:
...
- Codex hooks are written to `~/.codex/hooks.json` (or `CODEX_HOME/hooks.json` when that override is set). Codex supports a native `commandWindows`, so each Codius hook includes both POSIX and Windows commands. Non-managed Codex hooks are trust-gated by Codex; users may see Codex's hook review prompt before the hook runs.
- OpenCode gets a self-contained plugin at `$XDG_CONFIG_HOME/opencode/plugins/codius-terminal-activity.js` (or `~/.config/opencode/plugins/codius-terminal-activity.js` when XDG is unset; `OPENCODE_CONFIG_DIR` still wins when set).
