...
Codius passes those variables through to the Codex app-server process **and** maps them into Codex's thread config under `model_provider` / `model_providers`, because Codex reads provider routing from config rather than from `OPENAI_BASE_URL` alone.
...
### What Codius wires up

Under the hood, for each custom Codex provider Codius injects this into Codex's config:
...
- `base_url` — taken from `OPENAI_BASE_URL`. If it does not already end in `/v1`, Codius appends `/v1`. Trailing slashes are stripped.
- `wire_api` — always `"responses"` (OpenAI Responses API protocol).
- `env_key` — set to `"OPENAI_API_KEY"` when that env var is present and non-empty, so Codex reads the key from the same env var Codius passes through.
...
- Set `models` explicitly. Custom endpoints expose their own model IDs (`anthropic/claude-opus-4-7`, `qwen/qwen3-coder`, `local/llama`, etc.), and Codius does not discover them automatically for Codex.
...
Each profile appears as a separate provider in the Codius app. You can select which one to use when launching an agent.
...
The `command` array completely replaces the default command for that provider. The binary must exist on the system — Codius checks for its availability and will mark the provider as unavailable if not found.
...
Custom OMP profiles should extend `omp`. They inherit the OMP adapter's `rpc-ui` approvals, native Codius host tools, provider-managed subagents, and import behavior:
...
`params.sessionDir` is used only for importing sessions that were started outside Codius. If `command` or XDG env vars move OMP's state directory, set `params.sessionDir` to the resulting OMP JSONL session directory; launching and resuming still go through the configured command.
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) is an open standard for communication between editors and AI coding agents — think LSP but for AI agents. Any agent that supports ACP can be added to Codius as a custom provider.

ACP agents communicate over JSON-RPC 2.0 on stdio. Codius spawns the agent process and talks to it through stdin/stdout.

Codius also ships an in-app ACP provider catalog for common agents, including CodeWhale, Cursor, DeepAgents, DimCode, Gemini CLI, Hermes, Qwen Code, and Kimi Code. Catalog entries create the same `extends: "acp"` provider config shown below.
...
Codius tools such as subagent creation come from the shared internal tool catalog. ACP providers receive those tools through the MCP fallback by default because ACP exposes `mcpServers`, not Codius's native tool catalog. Some ACP adapters cannot create sessions when `mcpServers` is non-empty. Disable injected MCP for those providers with `params.supportsMcpServers: false`:
...
by default. To let a compliant agent delegate those operations to Codius instead,
...
Only enable capabilities Codius should execute. When the agent and Codius run in
different environments, configure equivalent absolute workspace paths before
delegating filesystem or terminal operations to Codius.

### Generic ACP diagnostics

Codius diagnostics for `extends: "acp"` providers report the configured command, resolved launcher binary, version output, ACP `initialize`, ACP `session/new`, model count, modes, and final status.
