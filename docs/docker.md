# Running Codius in Docker

Codius publishes a container image for running the daemon on a server, VM, NAS,
...
- stores daemon state and agent credentials under `/home/codius`
...
WebSocket requests still require `CODIUS_PASSWORD` when one is configured.
...
If you set `CODIUS_PASSWORD`, enter the same password when adding the direct
daemon connection in the web UI or another Codius client.
...
other agent CLIs. That keeps the default image small and avoids coupling Codius
...
```bash
docker build -f Dockerfile -t codius-with-agents .
```
...
Agent credentials and config persist in `/home/codius`, alongside daemon state.
...
or `compose.environment`; Codius passes them to launched agents.

## Volumes

| Mount          | Purpose                                                                    |
| -------------- | -------------------------------------------------------------------------- |
| `/home/codius` | Codius state under `.codius` plus agent config such as `.codex`, `.claude` |
...
write them. The built-in `codius` user has uid/gid `1000:1000`. For a different
...
When serving Codius behind a reverse proxy, forward normal HTTP requests and
...
If you reach the daemon by DNS name, set `CODIUS_HOSTNAMES` so host-header
...
- Set `CODIUS_PASSWORD` for any published port or network-reachable deployment.
- Prefer HTTPS at the reverse proxy for direct browser access.
- Use the Codius relay for untrusted networks or mobile access when you do not
...
```bash
docker build -f docker/base/Dockerfile -t codius:local .
```
...
```bash
docker build \
  --build-arg CODIUS_VERSION=0.1.102 \
  -t codius:0.1.102 \
...
gh workflow run docker.yml \
  --ref main \
  -f codius_version=0.1.102-beta.1 \
  -f publish=true
```

Manual Docker publishes require an explicit `codius_version`. The workflow builds
...
- **The web UI loads but cannot connect**: if `CODIUS_PASSWORD` is set, add a
...
- **Logs**: inspect `docker logs codius` or
  `/home/codius/.codius/daemon.log` inside the container.
