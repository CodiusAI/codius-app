...
avdmanager create avd -n codius -k "system-images;android-35;google_apis;arm64-v8a" -d pixel_7
emulator @codius     # start it; leave running
```
...
avdmanager create avd -n codius -k "system-images;android-35;google_apis;x86_64" -d pixel_7
emulator @codius     # start it; leave running
```
...
`npm run android` builds and installs the dev client, but two connections have to reach your Mac from inside the emulator — Metro (the JS bundle) and the Codius daemon — and **the emulator does not share the host's loopback**: `localhost` inside the emulator is the emulator itself. Reach the host at `10.0.2.2` (the standard AVD's host alias) for both:

```bash
REACT_NATIVE_PACKAGER_HOSTNAME=10.0.2.2 \
  EXPO_PUBLIC_LOCAL_DAEMON=10.0.2.2:$CODIUS_SERVICE_DAEMON_PORT \
  npm run android
```
...
- **`EXPO_PUBLIC_LOCAL_DAEMON=10.0.2.2:<port>`** — the client's daemon endpoint (`packages/app/src/runtime/host-runtime.ts`); when unset it defaults to `localhost:6767`, the production daemon. Use `$CODIUS_SERVICE_DAEMON_PORT` for a worktree daemon running as a Codius service, or `6768` for a standalone `npm run dev:server`. It is inlined into the JS bundle at Metro bundle time, so set it on the build command and clear the Metro cache (`npx expo start -c`) if a change doesn't take.

**Alternative — `adb reverse` + `localhost`** (if `10.0.2.2` misbehaves):

```bash
adb reverse tcp:8081 tcp:8081
adb reverse tcp:$CODIUS_SERVICE_DAEMON_PORT tcp:$CODIUS_SERVICE_DAEMON_PORT
REACT_NATIVE_PACKAGER_HOSTNAME=localhost \
  EXPO_PUBLIC_LOCAL_DAEMON=localhost:$CODIUS_SERVICE_DAEMON_PORT \
  npm run android
```
...
F-Droid builds should set `CODIUS_FDROID_BUILD=1` when running Expo prebuild:

```bash
cd packages/app
CODIUS_FDROID_BUILD=1 APP_VARIANT=production npx expo prebuild --platform android --clean --non-interactive
cd android
CODIUS_FDROID_BUILD=1 ./gradlew assembleRelease --no-daemon --max-workers=1 -Dorg.gradle.parallel=false
```
...
```bash
CODIUS_FDROID_BUILD=1 ./gradlew assembleRelease \
...
Keep the excluded npm packages installed. Normal builds use them, while the F-Droid profile removes only their Android native modules and config plugins. Codius always applies `expo-gradle-jvmargs` with `-Xmx4096m` and `-XX:MaxMetaspaceSize=1024m` so local Expo prebuilds have enough Gradle heap whether they use precompiled AARs or source-built Expo modules.
