...
export async function setProjectCustomIcon(input: {
  ...
  return serialize(input.projectId, async () => {
    ...
    if (input.source.type === "automatic") {
      ...
      validateIcon(bytes);
      await writeFileAtomic(cachePath(input.codiusHome, input.projectId), bytes);
...
export async function readProjectIcon(input: {
  ...
  try {
    return validateIcon(await readFile(cachePath(input.codiusHome, input.project.projectId)));
  } catch {
    return null;
  }
}
...
export async function removeProjectCustomIcon(input: {
  ...
  await rm(cachePath(input.codiusHome, input.projectId), { force: true });
}
...
function cachePath(codiusHome: string, projectId: string): string {
  const key = createHash("sha256").update(projectId).digest("hex");
  return join(codiusHome, "projects", "icons", `${key}.bin`);
}
