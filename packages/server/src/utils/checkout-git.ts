...
import { parseGitHubRemoteIdentity, parseGitRemoteLocation } from "@codius.ai/protocol/git-remote";
import { maxBase64EncryptedPlaintextByteLength } from "@codius.ai/relay";
...
import {
  getCodiusWorktreeChangeRequestHintForBranch,
  type CodiusWorktreeMetadata,
  readCodiusWorktreeMetadata,
  rebindCodiusWorktreeChangeRequestHint,
...
export async function renameCurrentBranch(
  ...
  if (currentBranch) {
    rebindCodiusWorktreeChangeRequestHint(worktreeRoot, previousBranch, currentBranch);
  }
...
function buildPullRequestLookupTargetFromMetadata(
  ...
  const target = getCodiusWorktreeChangeRequestHintForBranch(metadata, currentBranch);
  if (!target) {
    return null;
  }
  return {
...
// Temporary until diffs load lazily per file. The Codius relay's 32 MiB frame limit is
