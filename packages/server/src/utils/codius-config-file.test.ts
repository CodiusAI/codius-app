...
import {
  readCodiusConfigForEdit,
  statCodiusConfigPath,
  writeCodiusConfigForEdit,
} from "./codius-config-file.js";

describe("codius config file substrate", () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = realpathSync(mkdtempSync(join(tmpdir(), "codius-config-file-test-")));
  });
  ...
  it("returns null config and revision when codius.json is missing", () => {
    const result = readCodiusConfigForEdit(tempDir);

    expect(result).toEqual({ ok: true, config: null, revision: null });
  });

  it("returns invalid_project_config for invalid JSON", () => {
    writeFileSync(join(tempDir, "codius.json"), "{ invalid json\n");
  ...
  it("preserves raw lifecycle string and array forms with a revision token", () => {
    ...
    expect(result).toEqual({
      ...
      revision: statCodiusConfigPath(tempDir),
    });
  });
  ...
  it("writes pretty JSON with a trailing newline when revision matches", () => {
    writeFileSync(join(tempDir, "codius.json"), JSON.stringify({ worktree: { setup: "old" } }));
    const expectedRevision = statCodiusConfigPath(tempDir);

    const result = writeCodiusConfigForEdit({
      repoRoot: tempDir,
    ...
    expect(readFileSync(join(tempDir, "codius.json"), "utf8")).toBe(
      '{\n  "worktree": {\n    "setup": "npm install"\n  }\n}\n',
    );
  });
  ...
  it.skipIf(isPlatform("win32"))(
    ...
    () => {
      writeFileSync(join(tempDir, "codius.json"), JSON.stringify({ worktree: { setup: "old" } }));
      const expectedRevision = statCodiusConfigPath(tempDir);
      writeFileSync(join(tempDir, "codius.json"), JSON.stringify({ worktree: { setup: "new" } }));
      const currentRevision = statCodiusConfigPath(tempDir);
  ...
  it("round-trips unknown top-level, worktree, and script-entry fields", () => {
    ...
    expect(readCodiusConfigForEdit(tempDir)).toEqual({
      ok: true,
      config,
      revision: statCodiusConfigPath(tempDir),
    });
  ...
  it("creates codius.json when the file is still missing and expected revision is null", () => {
    ...
    expect(result).toEqual({
      ok: true,
      config: { scripts: { dev: { command: "npm run dev" } } },
      revision: statCodiusConfigPath(join(tempDir, "nested")),
    });
  });
});
