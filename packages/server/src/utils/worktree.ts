...
export interface RollbackCreatedCodiusWorktreeOptions extends DeleteCodiusWorktreeOptions {
...
export async function rollbackCreatedCodiusWorktree(
  options: RollbackCreatedCodiusWorktreeOptions,
  cause: unknown,
): Promise<never> {
  let cleanupError: unknown;
  try {
...
export const createWorktree = async ({
  ...
  const { worktreeIncludePlan, worktreePath } = await (async () => {
    try {
      const codiusWorktreesBaseRoot = resolveCodiusWorktreesBaseRoot({
        codiusHome,
        worktreesRoot,
      });
      const codiusWorktreesRoot = await getCodiusWorktreesRoot(cwd, codiusHome, worktreesRoot);
      const includePlan = await readWorktreeIncludePlan({
        sourceRoot: cwd,
        excludedSourceRoots: [codiusWorktreesBaseRoot],
      });
      const requestedWorktreePath = join(codiusWorktreesRoot, worktreeSlug);
  ...
  try {
    ...
    await rollbackCreatedCodiusWorktree(
      {
        cwd,
        worktreePath,
        teardownCwds: [],
...
async function fetchNewLocalBranchAtomically(options: {
  ...
  const temporaryRef = `refs/codius/worktree-fetch/${randomUUID()}`;
  try {
    await runGitCommand(
