...
const expected = JSON.parse(process.env.CODIUS_EXPECTED_ARGV_JSON);
const sliceFrom = process.env.CODIUS_ARGV_SLICE_FROM ? Number(process.env.CODIUS_ARGV_SLICE_FROM) : 2;
...
function makeFixture(): {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "codius spawn regression "));
  tempDirs.push(root);

  const fakeDaemonNode = path.join(root, "Fake Codius.exe");
...
async function runFixture(params: {
  ...
  const child = spawnProcess(params.command, params.args, {
    env: {
      ...process.env,
      CODIUS_EXPECTED_ARGV_JSON: JSON.stringify(["--config", JSON_ARG]),
    },
...
function makeLaunchFixture(ext: "exe" | "cmd" | "bat"): LaunchFixture {
  const root = mkdtempSync(path.join(tmpdir(), `codius-launch-${ext}-`));
  tempDirs.push(root);

  // Unique base name so a globally installed binary cannot satisfy findExecutable.
  const command = `codius-launch-fake-${path.basename(root)}`;
...
async function findAndLaunch(fixture: LaunchFixture): Promise<{
  ...
  return withPathPrepended(fixture.root, async () => {
    ...
    const child = spawnProcess(found, fixture.args, {
      env: {
        ...process.env,
        CODIUS_EXPECTED_ARGV_JSON: fixture.expectedArgvJson,
        CODIUS_ARGV_SLICE_FROM: String(fixture.sliceFrom),
      },
      stdio: ["ignore", "pipe", "pipe"],
    });
