...
import type { ProjectIconSource } from "@codius.ai/protocol/messages";
import {
...
async function project() {
  const rootPath = await tempDir("codius-project-root-");
  const codiusHome = await tempDir("codius-home-");
  ...
  return {
    ...
    set: (source: ProjectIconSource) =>
      setProjectCustomIcon({ codiusHome, projectId: "project-a", source, projects }),
    read: () => readProjectIcon({ codiusHome, project: record }),
    revision: () => record.customIconRevision,
    remove: () => removeProjectCustomIcon({ codiusHome, projectId: "project-a" }),
  };
}
...
describe("project custom icon", () => {
  ...
  it("drops the stored image when the project is removed", async () => {
    ...
    await expect(
      readProjectIcon({ codiusHome: target.codiusHome, project: stored }),
    ).resolves.toBeNull();
  });
