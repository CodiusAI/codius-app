...
`;
  ...
  it("reconciles a PR worktree lookup from current branch tracking", async () => {
    ...
    const workspaceDir = join(codiusHome, "worktrees", "repo", "pr-worktree");
    mkdirSync(join(codiusHome, "worktrees", "repo"), { recursive: true });
    ...
    writeCodiusWorktreeMetadata(workspaceDir, {
      baseRefName: "main",
    ...
    expect(await readPullRequestLookupTargetFromFacts(workspaceDir, codiusHome)).toMatchObject({
      headRef: "old-change",
      headRepositoryOwner: "contributor",
    });
    ...
    startGitCommandMetrics();
    const switchedTarget = await readPullRequestLookupTargetFromFacts(workspaceDir, codiusHome);
  ...
  it("does not apply ambiguous legacy PR metadata to a suffixed branch", async () => {
    ...
    const workspaceDir = join(codiusHome, "worktrees", "repo", "legacy-pr-worktree");
    mkdirSync(join(codiusHome, "worktrees", "repo"), { recursive: true });
    ...
    const lookupTarget = await readPullRequestLookupTargetFromFacts(workspaceDir, codiusHome);
  ...
  it("does not apply a legacy fork hint to an ownerless branch with the same head", async () => {
    ...
    const workspaceDir = join(codiusHome, "worktrees", "repo", "legacy-fork-worktree");
    mkdirSync(join(codiusHome, "worktrees", "repo"), { recursive: true });
    execFileSync("git", ["worktree", "add", workspaceDir, "contributor/old-change"], {
      cwd: repoDir,
    });
    ...
    const forkFacts = await getCheckoutSnapshotFacts(workspaceDir, { codiusHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "legacy-fork-branch" },
      { codiusHome, facts: forkFacts },
    );
    ...
    const ownerlessFacts = await getCheckoutSnapshotFacts(workspaceDir, { codiusHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "ownerless-same-head" },
      { codiusHome, facts: ownerlessFacts },
    );
  ...
  it("recognizes a normalized GitHub owner branch from legacy Enterprise metadata", async () => {
    ...
    const workspaceDir = join(codiusHome, "worktrees", "repo", "legacy-enterprise-worktree");
    mkdirSync(join(codiusHome, "worktrees", "repo"), { recursive: true });
    ...
    const facts = await getCheckoutSnapshotFacts(workspaceDir, { codiusHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
    ...
    const repointedFacts = await getCheckoutSnapshotFacts(workspaceDir, { codiusHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "repointed-enterprise-owner" },
      { codiusHome, facts: repointedFacts },
    );
  ...
  it("keeps a ref-only change request bound across rename but not branch switch", async () => {
    ...
    const workspaceDir = join(codiusHome, "worktrees", "repo", "gitlab-mr-worktree");
    mkdirSync(join(codiusHome, "worktrees", "repo"), { recursive: true });
    ...
    expect(readCodiusWorktreeMetadata(workspaceDir)?.changeRequestLookupTarget).toMatchObject({
      localBranchName: "feature/renamed",
    });
    const renamedFacts = await getCheckoutSnapshotFacts(workspaceDir, { codiusHome });
    const renamedStatus = await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "renamed-change-request" },
      { codiusHome, facts: renamedFacts },
    );
    ...
    const switchedFacts = await getCheckoutSnapshotFacts(workspaceDir, { codiusHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "switched-after-rename" },
      { codiusHome, facts: switchedFacts },
    );
  ...
  it("keeps fork identity when the local and tracked branch names match", async () => {
    execFileSync("git", ["remote", "add", "origin", "https://github.com/getcodius/codius.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["remote", "add", "contributor", "git@github.com:contributor/codius.git"], {
      cwd: repoDir,
    });
  ...
  it("keeps the local branch lookup when a fork tracks the upstream base branch", async () => {
    execFileSync("git", ["checkout", "-b", "local-feature"], { cwd: repoDir });
    execFileSync("git", ["remote", "add", "origin", "git@github.com:contributor/codius.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["remote", "add", "upstream", "https://github.com/getcodius/codius.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "branch.local-feature.remote", "upstream"], {
      cwd: repoDir,
