...
it("delivers rapid small writes complete and in order through worker coalescing", async () => {
  ...
  const session = trackTerminal(
    await manager.createTerminal({
      workspaceId: "ws-test",
      cwd,
      env: { CODIUS_TERMINAL_BURST_GATE: burstGatePath },
      ...nodeTerminalCommand(`
      const fs = require("node:fs");
      const gatePath = process.env.CODIUS_TERMINAL_BURST_GATE;
...
it("keeps registered cwd env inheritance behind the worker manager interface", async () => {
  ...
  manager.registerCwdEnv({
    cwd,
    env: { CODIUS_WORKER_TERMINAL_TEST: "worker-env" },
  });
  trackTerminal(
    await manager.createTerminal({
      workspaceId: "ws-test",
      ...
      require("node:fs").writeFileSync(
        ...
        process.env.CODIUS_WORKER_TERMINAL_TEST ?? "",
      );
      setInterval(() => {}, 1000);
    `),
    }),
...
it("injects parent-minted terminal activity env through the worker", async () => {
    ...
    await manager.createTerminal({
      ...
      ...nodeTerminalCommand(`
        require("node:fs").writeFileSync(
          ...
          JSON.stringify({
            terminalId: process.env.CODIUS_TERMINAL_ID,
            token: process.env.CODIUS_ACTIVITY_TOKEN,
            url: process.env.CODIUS_TERMINAL_ACTIVITY_URL,
            hookCli: process.env.CODIUS_HOOK_CLI,
  ...
  const codiusCliPath = resolveCodiusCliExecutablePath();
  expect(codiusCliBinDir).not.toBeNull();
  expect(codiusCliPath).not.toBeNull();
  ...
  expect(env.hookCli).toBe(codiusCliPath);
  ...
  expect(env.path?.split(delimiter)[0]).toBe(codiusCliBinDir);
});
