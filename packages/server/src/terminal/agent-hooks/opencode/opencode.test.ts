...
describe("OpenCode terminal agent hooks", () => {
  it("installs a self-contained OpenCode plugin idempotently", () => {
    const configDir = createTempDir("codius-opencode-config-");
    ...
    expect(firstInstall.configPath).toBe(join(configDir, "plugins", "codius-terminal-activity.js"));
  ...
  it("writes the plugin that maps OpenCode bus events to codius hook events", () => {
    ...
    expect(source).toContain('Bun.spawn(["codius", "hooks", "opencode", event]');
    expect(source).toContain("CODIUS_TERMINAL_ID");
  });

  it("uninstalls the OpenCode plugin file", () => {
    const configDir = createTempDir("codius-opencode-config-uninstall-");
  ...
  it("prefers OPENCODE_CONFIG_DIR over the XDG config home", () => {
    const homeDir = createTempDir("codius-home-");
    const configDir = createTempDir("codius-opencode-override-");
    const xdgConfigHome = createTempDir("codius-xdg-config-");
    ...
    expect(configPath).toBe(join(configDir, "plugins", "codius-terminal-activity.js"));
  });
  ...
  it("uses the XDG config home for the default OpenCode config dir", () => {
    ...
    expect(configPath).toBe(
      join(xdgConfigHome, "opencode", "plugins", "codius-terminal-activity.js"),
    );
  });
  ...
  it("falls back to the home .config OpenCode dir without an XDG config home", () => {
    ...
    expect(configPath).toBe(
      join(homeDir, ".config", "opencode", "plugins", "codius-terminal-activity.js"),
    );
  });
