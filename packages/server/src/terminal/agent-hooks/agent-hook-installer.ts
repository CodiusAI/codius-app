...
export function buildAgentHookShellCommand<TConfig>(
  ...
  const hookCommand = `"\${CODIUS_HOOK_CLI:-codius}" hooks ${shellToken(provider.id)} ${shellToken(event.event)}`;
  return `if [ -n "$CODIUS_TERMINAL_ID" ]; then ${hookCommand}; fi`;
}
...
export function buildAgentHookWindowsCommand<TConfig>(
  ...
  return `if defined CODIUS_TERMINAL_ID (if defined CODIUS_HOOK_CLI ("%CODIUS_HOOK_CLI%" ${hookArgs}) else (codius ${hookArgs})) else (exit /b 0)`;
}
