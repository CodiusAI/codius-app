...
export const codexHooksFormat: AgentHookConfigFormat<CodexHooksFile> = {
  ...
  install(config, provider) {
    ...
    for (const event of provider.events) {
      const userEntries = removeCodiusHooks(hooks[event.event], install.hookMarker);
      hooks[event.event] = [
        ...userEntries,
        {
          matcher: "",
  ...
  uninstall(config, provider) {
    ...
    const hooks = normalizeHooks(config.hooks);
    for (const event of provider.events) {
      const entries = removeCodiusHooks(hooks[event.event], install.hookMarker);
      if (entries.length > 0) {
    ...
    return provider.events.every((event) =>
      normalizeMatchers(hooks[event.event]).some((entry) =>
        normalizeCommandHooks(entry.hooks).some((hook) =>
          hasCodiusCommands(hook, install.hookMarker),
        ),
      ),
    );
  },
...
function removeCodiusHooks(value: unknown, marker: string): CodexMatcherGroup[] {
...
function hasCodiusCommands(hook: CodexCommandHook, marker: string): boolean {
  return (
