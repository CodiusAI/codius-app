...
describe("Codex terminal agent hooks", () => {
  it("installs POSIX and Windows hook commands idempotently", () => {
    const configDir = createTempDir("codius-codex-config-");
    ...
    for (const event of codexAgentHookProvider.events) {
      expect(commandHooks(config, event.event)).toEqual([
        {
          command: `if [ -n "$CODIUS_TERMINAL_ID" ]; then "\${CODIUS_HOOK_CLI:-codius}" hooks codex ${event.event}; fi`,
          commandWindows: `if defined CODIUS_TERMINAL_ID (if defined CODIUS_HOOK_CLI ("%CODIUS_HOOK_CLI%" hooks codex ${event.event}) else (codius hooks codex ${event.event})) else (exit /b 0)`,
        },
      ]);
    }
  ...
  it("preserves unrelated user hooks", () => {
    const configDir = createTempDir("codius-codex-config-preserve-");
    writeFileSync(
    ...
    expect(stopCommands).toEqual([
      "say codex done",
      'if [ -n "$CODIUS_TERMINAL_ID" ]; then "${CODIUS_HOOK_CLI:-codius}" hooks codex Stop; fi',
    ]);
  });

  it("uninstalls only marker-matched hooks", () => {
    const configDir = createTempDir("codius-codex-config-uninstall-");
    installAgentHooks(codexAgentHookProvider, { configDir });
    const config = readHooksFile(configDir);
    config.hooks = {
      ...config.hooks,
