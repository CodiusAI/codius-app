...
import { resolveCodiusCliBinDir } from "../../terminal.js";
...
describe.skipIf(!claudeAvailability.available)(
  ...
  () => {
    it("posts running then idle through the installed Claude config hooks", async () => {
      ...
      const configDir = createTempDir("codius-real-claude-config-");
      ...
      try {
        const claude = pty.spawn(
          ...
          ["--settings", join(configDir, "settings.json"), "-p", "reply with: done"],
          {
            ...
            env: {
              ...process.env,
              CODIUS_TERMINAL_ID: terminalId,
              CODIUS_ACTIVITY_TOKEN: token,
              CODIUS_TERMINAL_ACTIVITY_URL: recorder.url,
              PATH: [codiusCliBinDir, process.env.PATH].filter(isString).join(delimiter),
            },
          },
        );
