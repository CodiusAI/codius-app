...
function createFakeCliBinDir(): string {
  const dir = createTempDir("codius-cli-bin-");
  writeFileSync(join(dir, "codius"), "");
  return dir;
}
...
describe("Claude terminal agent hooks", () => {
  it("installs registered provider hooks idempotently", () => {
    const configDir = createTempDir("codius-claude-config-");
    ...
    for (const event of provider.events) {
      const codiusCommands = hookCommands(settings, event.event).filter((command) =>
        command.includes(install.hookMarker),
      );
      expect(codiusCommands).toHaveLength(1);
      expect(codiusCommands[0]).toBe(
        `if [ -n "$CODIUS_TERMINAL_ID" ]; then "\${CODIUS_HOOK_CLI:-codius}" hooks ${provider.id} ${event.event}; fi`,
      );
    }
  ...
  it("preserves unrelated user hooks", () => {
    const configDir = createTempDir("codius-claude-config-preserve-");
    writeFileSync(
  ...
  it("uninstalls only marker-matched hooks", () => {
    const configDir = createTempDir("codius-claude-config-uninstall-");
    installRegisteredAgentHooks({ configDir });
    const settings = readSettings(configDir);
    settings.hooks = {
      ...settings.hooks,
  ...
  it("builds a minimal gated hook command", () => {
    ...
    expect(command).toBe(
      'if [ -n "$CODIUS_TERMINAL_ID" ]; then "${CODIUS_HOOK_CLI:-codius}" hooks claude UserPromptSubmit; fi',
    );
  });

  it.skipIf(isPlatform("win32")).each(AGENT_HOOK_PROVIDERS.claude.events)(
    "$event hook command exits 0 when CODIUS_TERMINAL_ID is unset",
    (event) => {
      const provider = AGENT_HOOK_PROVIDERS.claude;
      const command = buildAgentHookShellCommand(provider, event);

      const result = spawnSync("/bin/sh", ["-c", command], {
        env: { PATH: process.env.PATH ?? "", CODIUS_HOOK_CLI: "codius" },
        stdio: "ignore",
      });
  ...
  it("prepends the codius CLI directory and injects the hook CLI path", () => {
    ...
    const env = buildTerminalEnvironment({
      ...
      codiusCliBinDir: cliBinDir,
      codiusHookCliPath: hookCliPath,
    });
