...
function hookPaths(root: string) {
  return {
    ...
    opencode: join(root, "opencode", "plugins", "codius-terminal-activity.js"),
  };
}

function createStore(codiusHome: string, enableTerminalAgentHooks: boolean): DaemonConfigStore {
...
describe("applyTerminalAgentHookSetting", () => {
  it("leaves agent configs untouched when the setting is disabled", () => {
    const root = createTempDir("codius-hook-setting-");
    const store = createStore(createTempDir("codius-hook-setting-home-"), false);
  ...
  it("installs agent hooks when the setting is enabled", () => {
    const root = createTempDir("codius-hook-setting-");
    const store = createStore(createTempDir("codius-hook-setting-home-"), true);
