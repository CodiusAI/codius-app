...
async function expectWorktreePresentInList(repoDir: string, worktreePath: string): Promise<void> {
  await expect
    .poll(
      async () => {
        const listed = await ctx.client.getCodiusWorktreeList({ cwd: repoDir });
...
test("create_agent_request creates a worktree and auto-archives both after the first turn", async () => {
  ...
  const listedWithWorktree = await ctx.client.getCodiusWorktreeList({ cwd: repoDir });
...
test("create_agent_request auto-archives a nested workspace from an existing Codius worktree", async () => {
  ...
  await ctx.client.archiveCodiusWorktree({ worktreePath: source.worktreePath });
}, 30000);
...
test("failed nested worktree creation cleans up the created workspace and backing directory", async () => {
  ...
  await expect
    .poll(
      async () => {
        const listed = await ctx.client.getCodiusWorktreeList({ cwd: source.repoDir });
        return (
...
test("create_agent_request with autoArchive archives only the agent when no worktree was created", async () => {
  ...
  const worktrees = await ctx.client.getCodiusWorktreeList({ cwd: repoDir });
...
test("create_agent_request with worktree but no autoArchive leaves agent and worktree active", async () => {
  ...
  await ctx.client.archiveCodiusWorktree({ worktreePath: created.worktreePath });
});
