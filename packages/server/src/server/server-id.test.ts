...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "codius-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.CODIUS_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per CODIUS_HOME", () => {
  ...
  it("respects and persists CODIUS_SERVER_ID override", () => {
    process.env.CODIUS_SERVER_ID = "test-daemon-id";
