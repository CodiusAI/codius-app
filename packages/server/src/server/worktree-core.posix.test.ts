...
function createGitRepo(): { tempDir: string; repoDir: string; codiusHome: string } {
  ...
  return { tempDir, repoDir, codiusHome };
}

function createGitRepoWithDevBranch(): { tempDir: string; repoDir: string; codiusHome: string } {
  const { tempDir, repoDir, codiusHome } = createGitRepo();
...
function createGitRepoWithOriginMain(): { tempDir: string; repoDir: string; codiusHome: string } {
  const { tempDir, repoDir, codiusHome } = createGitRepo();
...
function createGitHubPrRemoteRepo(): { tempDir: string; repoDir: string; codiusHome: string } {
  const { tempDir, repoDir, codiusHome } = createGitRepo();
...
function createSameRepoGitHubPrRemoteRepo(): {
  ...
  return { tempDir, repoDir, remoteDir, codiusHome };
}
...
function createForkGitHubPrRemoteRepo(): {
  ...
  return { tempDir, repoDir, headRemoteDir, codiusHome };
}
...
describe.skipIf(isPlatform("win32"))("worktree-core POSIX-only", () => {
  describe("createWorktreeCore", () => {
    ...
    test("creates branch-off worktrees from origin main without tracking origin main", async () => {
      const { tempDir, repoDir, codiusHome } = createGitRepoWithOriginMain();
      cleanupPaths.push(tempDir);
    ...
    test("checks out an explicit GitHub PR branch with legacy RPC fields", async () => {
      const { tempDir, repoDir, codiusHome } = createGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
    ...
    test("branches off an explicit refName base", async () => {
      const { tempDir, repoDir, codiusHome } = createGitRepoWithDevBranch();
      cleanupPaths.push(tempDir);
    ...
    test("tracks a same-repo PR branch from a single-branch clone", async () => {
      const { tempDir, repoDir, remoteDir, codiusHome } = createSameRepoGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
    ...
    test("checks out a GitLab MR source branch through the resolved forge service", async () => {
      const { tempDir, repoDir, codiusHome } = createGitRepoWithOriginFeatureBranch();
      cleanupPaths.push(tempDir);
    ...
    test("checks out a GitLab MR ref when the source branch is gone", async () => {
      const { tempDir, repoDir, codiusHome } = createGitLabMrRefOnlyRemoteRepo();
      cleanupPaths.push(tempDir);
      const gitlab: ForgeService = {
        ...createGitHubServiceStub(),
        getPullRequestCheckoutTarget: async ({ number }) => ({
    ...
    test("checks out the authoritative GitLab MR ref before a same-named origin branch", async () => {
      const { tempDir, repoDir, codiusHome } = createGitLabMrWithConflictingOriginBranchRepo();
      cleanupPaths.push(tempDir);
    ...
    test("does not add a fallback push remote for a cross-repo MR without a push URL", async () => {
      ...
      const pushRefspec = getGitConfigValue(
        second.worktree.worktreePath,
        "remote.codius-pr-14.push",
      );
      const pushRemoteUrl = getGitConfigValue(
        second.worktree.worktreePath,
        "remote.codius-pr-14.url",
      );
      const metadata = readCodiusWorktreeMetadata(second.worktree.worktreePath);
      const facts = await getCheckoutSnapshotFacts(second.worktree.worktreePath, { codiusHome });
    ...
    test("checks out a same-repo GitHub PR with valid origin tracking", async () => {
      ...
      const status = await getCheckoutStatus(result.worktree.worktreePath, { codiusHome });
    ...
    test("pushes a deduplicated same-repo GitHub PR branch to the PR head", async () => {
      ...
      const pushRemote = execFileSync(
        ...
        .toString()
        .trim();
      const pushRefspec = execFileSync("git", ["config", "remote.codius-pr-1790.push"], {
        cwd: second.worktree.worktreePath,
        stdio: "pipe",
      })
        .toString()
        .trim();
      const pushRemoteUrl = execFileSync("git", ["config", "remote.codius-pr-1790.url"], {
        cwd: second.worktree.worktreePath,
        stdio: "pipe",
      })
        .toString()
      ...
      expect(pushRemote).toBe("codius-pr-1790");
    ...
    test("derives a deduped PR lookup target from git config when metadata has no target", async () => {
      ...
      writeFileSync(
        getCodiusWorktreeMetadataPath(second.worktree.worktreePath),
    ...
    test("checks out a fork PR whose head branch collides with local main", async () => {
      const { tempDir, repoDir, headRemoteDir, codiusHome } = createForkGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
      const github = {
        ...createGitHubServiceStub(),
        getPullRequestCheckoutTarget: async () => ({
      ...
      const worktreeBranch = execFileSync("git", ["branch", "--show-current"], {
        ...
        .trim();
      const readme = readFileSync(path.join(result.worktree.worktreePath, "README.md"), "utf8");
      const cleanStatus = await getCheckoutStatus(result.worktree.worktreePath, { codiusHome });
      const pushRefspec = execFileSync("git", ["config", "remote.codius-pr-526.push"], {
        cwd: result.worktree.worktreePath,
        stdio: "pipe",
      })
        .toString()
      ...
      expect(getBranchUpstream(result.worktree.worktreePath)).toBe("codius-pr-526/main");
    ...
    test("pushes a fork PR when the contributor branch cannot be fetched at checkout", async () => {
      ...
      expect(getBranchUpstream(result.worktree.worktreePath)).toBe("codius-pr-526/main");
      expect(pushRemote).toBe("codius-pr-526");
      expect(pushRefspec).toBe("HEAD:refs/heads/main");
      expect(remotePrHead).toBe(localHead);
    ...
    test("uses a unique local branch when the same fork PR branch already exists", async () => {
      ...
      expect(getBranchUpstream(first.worktree.worktreePath)).toBe("codius-pr-526/main");
      expect(getBranchUpstream(second.worktree.worktreePath)).toBe("codius-pr-526/main");
    });
    ...
    test("reuses an existing branch-off worktree for the same slug", async () => {
      ...
      const first = await createCoreWorktree(
        { cwd: repoDir, worktreeSlug: "reused-worktree", codiusHome, runSetup: false },
        deps,
      );
      const second = await createCoreWorktree(
        { cwd: repoDir, worktreeSlug: "reused-worktree", codiusHome, runSetup: false },
