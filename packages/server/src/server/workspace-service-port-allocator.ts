...
export interface AllocateWorkspaceServicePortOptions {
  allocation: CodiusServicePortAllocation | undefined;
  cwd: string;
  scriptName: string;
  workspaceId: string;
  ...
  try {
    result = await execCommand(
      ...
      [options.scriptName, options.workspaceId, options.branchName ?? "", options.cwd],
      {
        cwd: options.cwd,
        envOverlay: {
          CODIUS_SCRIPTNAME: options.scriptName,
          CODIUS_WORKSPACE_ID: options.workspaceId,
          CODIUS_BRANCH_NAME: options.branchName ?? "",
          CODIUS_WORKTREE_PATH: options.cwd,
        },
