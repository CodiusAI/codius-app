...
export function buildWorkspaceServiceEnv(
  ...
  const env: Record<string, string> = {
    ...
    CODIUS_PORT: String(selfPeer.port),
  };
  ...
  if (selfProxyUrl) {
    env.CODIUS_URL = selfProxyUrl;
  }

  for (const peer of options.peers) {
    const envName = normalizeServiceEnvName(peer.scriptName);
    env[`CODIUS_SERVICE_${envName}_PORT`] = String(peer.port);
    ...
    if (peerProxyUrl) {
      env[`CODIUS_SERVICE_${envName}_URL`] = peerProxyUrl;
    }
  }
