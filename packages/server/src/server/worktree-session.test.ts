...
import {
  ...
  createCodiusWorktreeWorkflow,
  handleCodiusWorktreeArchiveRequest,
  handleCodiusWorktreeListRequest,
  resolveGitCreateBaseBranch,
  runWorktreeSetupInBackground,
  handleCreateCodiusWorktreeRequest,
...
import {
  createCodiusWorktree as createCodiusWorktreeService,
  type CreateCodiusWorktreeFn,
...
function createWorkflowForRequestTest(options: {
  codiusHome: string;
  createCodiusWorktree?: CreateCodiusWorktreeFn;
  ...
  return async (input: Parameters<CreateCodiusWorktreeFn>[0]) => {
    const createCodiusWorktree =
      options.createCodiusWorktree ??
      createCodiusWorktreeForTest({ codiusHome: options.codiusHome });
...
function createCodiusWorktreeForTest(options: {
  codiusHome: string;
  events?: string[];
}): CreateCodiusWorktreeFn {
  ...
  return (input, serviceOptions) => {
    return createCodiusWorktreeService(input, {
...
describe("handleCodiusWorktreeListRequest", () => {
  test("lists worktrees through the workspace git service", async () => {
    ...
    await handleCodiusWorktreeListRequest(
      {
      ...
      {
        type: "codius_worktree_list_request",
        cwd: "/tmp/repo",
    ...
    expect(emitted).toContainEqual({
      type: "codius_worktree_list_response",
      payload: {
        worktrees: [
          {
            worktreePath: "/tmp/codius-home/worktrees/repo/feature",
            createdAt: "2026-04-12T00:00:00.000Z",
            branchName: "feature",
            head: "abc123",
          },
...
describe("create-agent worktree setup boundary", () => {
  test("agent setup continuation starts setup for the created agent timeline", async () => {
    ...
    try {
      const result = await createCodiusWorktreeWorkflow(
        {
          codiusHome,
          createCodiusWorktree: createCodiusWorktreeForTest({ codiusHome }),
...
function createGitRepo(options?: { codiusConfig?: Record<string, unknown> }) {
...
describe("runWorktreeSetupInBackground", () => {
  ...
  test("runs setup from an exact workspace subdirectory", async () => {
    ...
    writeFileSync(
      path.join(sourceWorkspaceCwd, "codius.json"),
      JSON.stringify({
        worktree: {
  ...
  test("archives the pending workspace and emits a failed snapshot when setup cannot start", async () => {
    ...
    writeFileSync(path.join(repoDir, "codius.json"), "{ invalid json\n");
    execFileSync("git", ["add", "codius.json"], { cwd: repoDir, stdio: "pipe" });
    ...
    expect(progressMessages[1]?.payload.error).toMatch(
      /Failed to parse codius\.json at .*codius\.json/,
    );
    expect(progressMessages[1]?.payload.detail.commands).toEqual([]);
    expect(snapshots.get(workspaceId)).toMatchObject({
      status: "failed",
      error: expect.stringMatching(/Failed to parse codius\.json at .*codius\.json/),
    });
    ...
    const snapshots = new Map([
      [
        "ws-feature-a",
        {
          ...
          detail: {
            type: "worktree_setup",
            worktreePath: "/repo/.codius/worktrees/feature-a",
...
describe("handleCreateCodiusWorktreeRequest", () => {
  const cleanupPaths: string[] = [];
  ...
  test("checks out the GitHub PR branch when githubPrNumber is supplied", async () => {
    ...
    await handleCreateCodiusWorktreeRequest(
      {
        codiusHome,
        ...
        emit: (message) => emitted.push(message),
        sessionLogger: logger,
        createCodiusWorktreeWorkflow: createWorkflowForRequestTest({ codiusHome }),
      },
      {
    ...
    const response = emitted.find(
      (
        message,
      ): message is Extract<SessionOutboundMessage, { type: "create_codius_worktree_response" }> =>
        message.type === "create_codius_worktree_response",
    );
  ...
  test("buildAgentSessionConfig passes prompt and attachment context into worktree creation", async () => {
    const createCodiusWorktree = vi.fn(async () => ({
      worktree: {
    ...
    expect(createCodiusWorktree).toHaveBeenCalledWith(
  ...
  test("createCodiusWorktreeForTest forwards the default branch resolver for branch-off intents", async () => {
    const { tempDir, repoDir } = createGitRepo();
    cleanupPaths.push(tempDir);
    const codiusHome = path.join(tempDir, ".codius");
    const resolveDefaultBranch = vi.fn(async () => "main");

    const result = await createCodiusWorktreeForTest({ codiusHome })(
      {
        cwd: repoDir,
    ...
    try {
      await handleCreateCodiusWorktreeRequest(
        {
          ...
          sessionLogger: createLogger(),
          emit: (message) => emitted.push(message),
          createCodiusWorktreeWorkflow: createWorkflowForRequestTest({
            codiusHome,
            createCodiusWorktree: createCodiusWorktreeForTest({ codiusHome, events }),
          }),
      ...
      const response = emitted.find(
        (
          ...
          { type: "create_codius_worktree_response" }
        > => message.type === "create_codius_worktree_response",
      );
      expect(response?.payload.error).toBeNull();
    } finally {
  ...
  test("creates the worktree before emitting the response", async () => {
    ...
    try {
      await handleCreateCodiusWorktreeRequest(
        {
          ...
          createCodiusWorktreeWorkflow: createWorkflowForRequestTest({
            codiusHome,
            createCodiusWorktree: async (input) => {
              const result = await createCodiusWorktreeForTest({ codiusHome })(input);
      ...
      expect(emitted.map((message) => message.type).slice(0, 2)).toEqual([
        "create_codius_worktree_response",
        "workspace_update",
      ]);
...
describe("handleCodiusWorktreeArchiveRequest worktree scope", () => {
  const cleanupPaths: string[] = [];
  ...
  test("archives every active workspace on the directory and removes it", async () => {
    ...
    await handleCodiusWorktreeArchiveRequest(
      {
        codiusHome,
        github: createGitHubServiceStub(),
        workspaceGitService: {
    ...
    expect(
      emitted.find((message) => message.type === "codius_worktree_archive_response"),
    ).toMatchObject({
      payload: {
        success: true,
        error: null,
  ...
  test("ignores deleteWorktreeFromDisk:true and derives directory removal from remaining references", async () => {
    ...
    await handleCodiusWorktreeArchiveRequest(deps, {
      type: "codius_worktree_archive_request",
    ...
    expect(existsSync(sharedCwd)).toBe(false);
    expect(
      emitted.filter((message) => message.type === "codius_worktree_archive_response"),
    ).toHaveLength(2);
  });
});
