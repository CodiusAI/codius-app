...
describe("runAsyncWorktreeBootstrap", () => {
  ...
  function assertServiceTerminalCallSelfEnv(params: {
    ...
    expect(createTerminalCalls[0]?.env?.CODIUS_PORT).toEqual(expect.any(String));
    expect(createTerminalCalls[0]?.env?.HOST).toBe("127.0.0.1");
    expect(createTerminalCalls[0]?.env?.CODIUS_URL).toBe(
      "http://api--feature-socket-service--repo.localhost:6767",
    );
    expect(createTerminalCalls[0]?.env?.CODIUS_SERVICE_API_PORT).toBe(
      createTerminalCalls[0]?.env?.CODIUS_PORT,
    );
    expect(createTerminalCalls[0]?.env?.CODIUS_SERVICE_API_URL).toBe(
      "http://api--feature-socket-service--repo.localhost:6767",
    );
  }
  ...
  async function assertServiceTerminalCallPeerEnv(params: {
    ...
    expect(createTerminalCalls[0]?.env?.CODIUS_SERVICE_APP_SERVER_PORT).toBe(
      String(plannedAppServerPort),
    );
    expect(createTerminalCalls[0]?.env?.CODIUS_SERVICE_APP_SERVER_URL).toBe(
  ...
  function commitCodiusScripts(
    scripts: Record<string, { command: string; type?: "script" | "service" }>,
    message = "add script config",
  ): void {
    writeFileSync(join(repoDir, "codius.json"), JSON.stringify({ scripts }));
    execFileSync("git", ["add", "codius.json"], { cwd: repoDir, stdio: "pipe" });
  ...
  it("refreshes a stopped service port on respawn and updates the route", async () => {
    ...
    expect(createTerminalCalls[2]?.env?.CODIUS_SERVICE_WORKER_PORT).toBe(String(workerPort));
  ...
  it("fails normalized service env name collisions before terminal creation", async () => {
    ...
    expect(createTerminalCalls).toHaveLength(1);
    expect(createTerminalCalls[0]?.env).toHaveProperty("CODIUS_SERVICE_APP_SERVER_PORT");
    expect(createTerminalCalls[0]?.env).toHaveProperty("CODIUS_SERVICE_WORKER_PORT");
  });
