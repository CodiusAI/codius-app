...
describe("isWebSocketSameOrigin", () => {
  test("accepts exact same-origin websocket upgrades", () => {
    ...
    expect(isWebSocketSameOrigin("https://codius.example.com", "codius.example.com")).toBe(true);
  });
  ...
  test("rejects non-loopback cross-origin upgrades", () => {
    ...
    expect(isWebSocketSameOrigin("http://127.0.0.1:32775", "codius.example.com:32775")).toBe(false);
  });
});
