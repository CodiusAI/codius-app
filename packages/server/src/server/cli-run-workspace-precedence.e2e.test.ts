...
test("daemon resolves human and managed CLI workspace ownership", async () => {
  ...
  const cwd = mkdtempSync(path.join(tmpdir(), "codius-cli-run-cwd-"));
  const otherCwd = mkdtempSync(path.join(tmpdir(), "codius-cli-run-other-cwd-"));
  ...
  try {
    ...
    // $CODIUS_WORKSPACE_ID land a run) creates no new workspace record: the
