  ...
  describe("runAsyncWorktreeBootstrap", () => {
    ...
    it("keeps only the final carriage-return-updated content in command logs", async () => {
      ...
      const persistedSetupItem = persisted.find(
        (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
          item.type === "tool_call" && item.name === "codius_worktree_setup",
      );
    ...
    it("shares the same worktree runtime port across setup and bootstrap terminals", async () => {
      writeFileSync(
        ...
        JSON.stringify({
          worktree: {
            setup: ['echo "$CODIUS_WORKTREE_PORT" > setup-port.txt'],
            terminals: [
              {
                name: "Port Terminal",
                command: "true",
      ...
      expect(registeredEnvs[0]?.env.CODIUS_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalEnvs.length).toBeGreaterThan(0);
      expect(createTerminalEnvs[0]?.CODIUS_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalWorkspaceIds).toEqual(["ws-shared-runtime-port"]);

      const terminalToolCall = persisted.find(
        (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
          item.type === "tool_call" &&
          item.name === "codius_worktree_terminals" &&
    ...
    it("injects real peer service env into terminal-backed services", async () => {
      ...
      expect(apiEnv.CODIUS_SERVICE_API_URL).toBe(
        "http://api--feature-peer-env--repo.localhost:6767",
      );
      expect(apiEnv.CODIUS_SERVICE_WEB_URL).toBe(
        "http://web--feature-peer-env--repo.localhost:6767",
      );
      expect(apiEnv.CODIUS_SERVICE_API_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.CODIUS_SERVICE_WEB_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.CODIUS_URL).toBe(apiEnv.CODIUS_SERVICE_API_URL);
      expect(apiEnv.CODIUS_PORT).toBe(apiEnv.CODIUS_SERVICE_API_PORT);
      expect(apiEnv).not.toHaveProperty("PORT");

      expect(webEnv.CODIUS_SERVICE_API_URL).toBe(
        "http://api--feature-peer-env--repo.localhost:6767",
      );
      expect(webEnv.CODIUS_SERVICE_WEB_URL).toBe(
        "http://web--feature-peer-env--repo.localhost:6767",
      );
      expect(webEnv.CODIUS_SERVICE_API_PORT).toBe(apiEnv.CODIUS_SERVICE_API_PORT);
      expect(webEnv.CODIUS_SERVICE_WEB_PORT).toBe(apiEnv.CODIUS_SERVICE_WEB_PORT);
      expect(webEnv.CODIUS_URL).toBe(webEnv.CODIUS_SERVICE_WEB_URL);
      expect(webEnv.CODIUS_PORT).toBe(webEnv.CODIUS_SERVICE_WEB_PORT);
      expect(webEnv).not.toHaveProperty("PORT");

      const apiPort = Number(apiEnv.CODIUS_SERVICE_API_PORT);
      const webPort = Number(apiEnv.CODIUS_SERVICE_WEB_PORT);
