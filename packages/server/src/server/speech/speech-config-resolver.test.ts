...
describe("resolveSpeechConfig", () => {
  test("resolves local-first defaults without env overrides", () => {
    const codiusHome = "/tmp/codius-home";
    ...
    expect(result.speech.local).toEqual({
      modelsDir: path.join(codiusHome, "models", "local-speech"),
      models: {
  ...
  test("resolves feature-scoped local speech settings", () => {
    ...
    const env = {
      CODIUS_DICTATION_LOCAL_STT_MODEL: "parakeet-tdt-0.6b-v2-int8",
      CODIUS_VOICE_LOCAL_STT_MODEL: "parakeet-tdt-0.6b-v2-int8",
      CODIUS_VOICE_LOCAL_TTS_MODEL: "kokoro-en-v0_19",
      CODIUS_VOICE_LOCAL_TTS_SPEAKER_ID: "5",
      CODIUS_VOICE_LOCAL_TTS_SPEED: "1.35",
      CODIUS_DICTATION_LANGUAGE: "es",
      CODIUS_VOICE_LANGUAGE: "pt",
      CODIUS_LOCAL_MODELS_DIR: "/tmp/models",
      OPENAI_API_KEY: "env-key",
      CODIUS_VOICE_STT_PROVIDER: "openai",
      CODIUS_DICTATION_STT_PROVIDER: "local",
      CODIUS_VOICE_TTS_PROVIDER: "local",
