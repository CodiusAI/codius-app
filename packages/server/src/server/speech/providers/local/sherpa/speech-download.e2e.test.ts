...
const RUN = process.env.CODIUS_SPEECH_E2E_DOWNLOAD === "1";
...
downloadTest(
  ...
  async () => {
    ...
    const codiusHomeRoot = mkdtempSync(path.join(tmpdir(), "codius-speech-download-"));
    const modelsDir = path.join(codiusHomeRoot, ".codius", "models", "local-speech");
