...
export async function generateLocalPairingOffer(args: {
  ...
  const relayEndpoint = args.relayEndpoint ?? "relay.codius.ai:443";
  const relayPublicEndpoint = args.relayPublicEndpoint ?? relayEndpoint;
  const relayUseTls = args.relayUseTls ?? relayEndpoint === "relay.codius.ai:443";
  const relayPublicUseTls = args.relayPublicUseTls ?? relayUseTls;
  const appBaseUrl = args.appBaseUrl ?? "https://app.codius.ai";
  const serverId = getOrCreateServerId(args.codiusHome, { logger: args.logger });
  const daemonKeyPair = await loadOrCreateDaemonKeyPair(args.codiusHome, args.logger);
