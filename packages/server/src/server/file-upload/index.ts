...
export class FileUploadStore {
  ...
  beginUpload(request: FileUploadRequest): void {
    ...
    const uploadDir = join(this.codiusHome, "uploads", id);
  ...
  private async startWriting(upload: PendingUpload): Promise<void> {
    await mkdir(join(this.codiusHome, "uploads", upload.id), { recursive: true });
  ...
  private async removeUploadDirectory(upload: PendingUpload): Promise<void> {
    await rm(join(this.codiusHome, "uploads", upload.id), { recursive: true, force: true }).catch(
      () => undefined,
    );
  }
}
