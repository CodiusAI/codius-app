...
async function createHome(config: object = {}): Promise<string> {
  const home = await mkdtemp(path.join(os.tmpdir(), "codius-config-git-"));
  roots.push(home);
...
describe("daemon Git process config", () => {
  ...
  test("new environment variables override config.json", async () => {
    ...
    expect(
      loadConfig(home, {
        env: {
          CODIUS_GIT_MAX_PROCESSES_PER_SECOND: "12",
          CODIUS_GIT_MAX_PROCESS_CONCURRENCY: "6",
        },
      }).git,
    ).toEqual({
  ...
  test("accepts legacy CODIUS_GIT_CONCURRENCY below the renamed variable", async () => {
    const home = await createHome();

    expect(
      loadConfig(home, {
        env: {
          CODIUS_GIT_CONCURRENCY: "3",
        },
    ...
    expect(
      loadConfig(home, {
        env: {
          CODIUS_GIT_CONCURRENCY: "3",
          CODIUS_GIT_MAX_PROCESS_CONCURRENCY: "7",
        },
      }).git?.maxProcessConcurrency,
    ).toBe(7);
  });
