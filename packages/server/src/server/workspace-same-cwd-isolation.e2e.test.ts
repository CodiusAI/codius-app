...
function seedSameCwdWorkspaces(): { codiusHomeRoot: string; cwd: string } {
  const codiusHomeRoot = mkdtempSync(path.join(tmpdir(), "codius-same-cwd-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "codius-same-cwd-dir-"));
  const projectsDir = path.join(codiusHomeRoot, ".codius", "projects");
  ...
  return { codiusHomeRoot, cwd };
}

async function seedWorkspaceWithLegacyAgent(): Promise<{ codiusHomeRoot: string; cwd: string }> {
  const codiusHomeRoot = mkdtempSync(path.join(tmpdir(), "codius-legacy-agent-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "codius-legacy-agent-dir-"));
  ...
  const agentStorage = new AgentStorage(path.join(codiusHome, "agents"), createTestLogger());
...
test("daemon bootstrap migrates cwd-only legacy agents before same-cwd workspaces are added", async () => {
  const { codiusHomeRoot, cwd } = await seedWorkspaceWithLegacyAgent();
  const daemon = await createTestCodiusDaemon({ codiusHomeRoot });
...
test("workspace.create directory source with firstAgentContext generates a daemon-visible workspace title", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "codius-named-local-dir-"));
...
test("local workspace auto-title does not broadcast provider snapshot warm-up to clients", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "codius-title-snapshot-storm-"));
...
test("create_agent_request with workspaceId does not retitle an existing workspace", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "codius-agent-submit-title-"));
  const daemon = await createTestCodiusDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("creating another same-cwd local workspace keeps running status on the owning workspace only", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "codius-running-same-cwd-create-"));
  const daemon = await createTestCodiusDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("two workspaces sharing one cwd compute agent status per workspaceId", async () => {
  const { codiusHomeRoot, cwd } = seedSameCwdWorkspaces();
  const daemon = await createTestCodiusDaemon({ codiusHomeRoot });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
    appVersion: "0.1.82",
