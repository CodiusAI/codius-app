...
async function createCodiusHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "codius-config-auth-"));
  roots.push(root);
...
describe("daemon auth config", () => {
  ...
  test("loads optional auth password hash from config.json", async () => {
    const codiusHome = await createCodiusHome({
      version: 1,
      daemon: {
  ...
  test("lets CODIUS_PASSWORD override config.json auth password hash", async () => {
    const codiusHome = await createCodiusHome({
      version: 1,
      daemon: {
        auth: { password: CONFIG_PASSWORD_HASH },
