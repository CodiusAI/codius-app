...
import { isCodiusDictationDebugEnabled } from "../agent/recordings-debug.js";
...
const DICTATION_SILENCE_PEAK_THRESHOLD = Number.parseInt(
  process.env.CODIUS_DICTATION_SILENCE_PEAK_THRESHOLD ?? "300",
  10,
);
...
export class DictationStreamManager {
  ...
  constructor(params: {
    ...
    this.autoCommitSeconds =
      ...
      parseNonNegativeNumber(process.env.CODIUS_DICTATION_AUTO_COMMIT_SECONDS) ??
  ...
  public async handleStart(dictationId: string, format: string): Promise<void> {
    ...
    const transcriptionPrompt =
      process.env.CODIUS_DICTATION_TRANSCRIPTION_PROMPT ??
  ...
  private async maybePersistDictationStreamAudio(dictationId: string): Promise<string | null> {
    if (!isCodiusDictationDebugEnabled()) {
      return null;
    }
