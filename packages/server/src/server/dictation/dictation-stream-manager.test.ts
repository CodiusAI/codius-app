...
describe("DictationStreamManager (finish buffer-too-small tolerance)", () => {
  const env = {
    dictationDebug: process.env.CODIUS_DICTATION_DEBUG,
  };

  beforeEach(() => {
    vi.useFakeTimers();
    process.env.CODIUS_DICTATION_DEBUG = "false";
  });

  afterEach(() => {
    vi.useRealTimers();
    process.env.CODIUS_DICTATION_DEBUG = env.dictationDebug;
  });
...
describe("DictationStreamManager (provider-agnostic provider)", () => {
  ...
  it("uses CODIUS_DICTATION_LANGUAGE when set", async () => {
    const sttProvider = await startWithResolvedDictationLanguage({
      env: {
        CODIUS_DICTATION_LANGUAGE: "pt",
  ...
  it("treats empty CODIUS_DICTATION_LANGUAGE as unset", async () => {
    const sttProvider = await startWithResolvedDictationLanguage({
      env: {
        CODIUS_DICTATION_LANGUAGE: "  ",
      } as NodeJS.ProcessEnv,
    });
  ...
  it("auto-commits while streaming and assembles final transcript in segment order", async () => {
    const originalDebug = process.env.CODIUS_DICTATION_DEBUG;
    process.env.CODIUS_DICTATION_DEBUG = "false";
    ...
    try {
      ...
      if (originalDebug === undefined) {
        delete process.env.CODIUS_DICTATION_DEBUG;
      } else {
        process.env.CODIUS_DICTATION_DEBUG = originalDebug;
      }
    }
  });
  ...
  it("drops dangling uncommitted non-final transcripts when finishing after silence tail clear", async () => {
    vi.useFakeTimers();
    const previousDebug = process.env.CODIUS_DICTATION_DEBUG;
    process.env.CODIUS_DICTATION_DEBUG = "false";
    try {
      const session = new FakeRealtimeSession();
      const emitted: Array<{ type: string; payload: unknown }> = [];
      ...
      process.env.CODIUS_DICTATION_DEBUG = previousDebug;
      vi.useRealTimers();
    }
  });
});
