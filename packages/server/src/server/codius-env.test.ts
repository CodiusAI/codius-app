...
import {
  ...
  createCodiusInternalEnv,
  resolveCodiusNodeEnv,
} from "./codius-env.js";

describe("codius env contract", () => {
  const ELECTRON_RUN_AS_NODE = "ELECTRON_RUN_AS_NODE";
  const CODIUS_NODE_ENV = "CODIUS_NODE_ENV";
  const baseEnv = {
    [ELECTRON_RUN_AS_NODE]: "1",
    ELECTRON_NO_ATTACH_CONSOLE: "1",
    NODE_ENV: "development",
    PATH: "/usr/bin",
    CODIUS_AGENT_ID: "agent-123",
  ...
  test("builds external process env by scrubbing runtime control vars after overlays", () => {
    ...
    expect(env.CODIUS_AGENT_ID).toBe("agent-123");
  ...
  test("builds external command env without process.execPath special-casing", () => {
    ...
    expect(env.CODIUS_DESKTOP_MANAGED).toBeUndefined();
    expect(env[CODIUS_NODE_ENV]).toBeUndefined();
    expect(env.CODIUS_SUPERVISED).toBeUndefined();
  });
  ...
  test("builds self node command with Electron node mode", () => {
    ...
    expect(command.env.CODIUS_DESKTOP_MANAGED).toBeUndefined();
    expect(command.env[CODIUS_NODE_ENV]).toBeUndefined();
    expect(command.env.CODIUS_SUPERVISED).toBeUndefined();
  });
  ...
  test("does not use user NODE_ENV as Codius runtime mode", () => {
    expect(resolveCodiusNodeEnv({ NODE_ENV: "development" })).toBeUndefined();
    expect(resolveCodiusNodeEnv({ NODE_ENV: "development", CODIUS_NODE_ENV: "production" })).toBe(
      "production",
    );
    expect(resolveCodiusNodeEnv({ NODE_ENV: "test", CODIUS_NODE_ENV: "local" })).toBeUndefined();
  });
});
