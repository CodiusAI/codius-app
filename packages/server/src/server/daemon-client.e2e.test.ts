...
const localModelsDir =
  process.env.CODIUS_LOCAL_MODELS_DIR ?? path.join(homedir(), ".codius", "models", "local-speech");
  ...
  try {
    ...
    expect(result).toEqual({
      ...
      file: {
        ...
        mimeType: "text/plain",
        size: 11,
        path: path.join(daemon.codiusHome, "uploads", "upload_req-upload-e2e", "notes.txt"),
      },
      error: null,
    });
...
test("returns home-scoped directory suggestions", async () => {
  const insideHomeDir = mkdtempSync(path.join(homedir(), "codius-dir-suggestion-"));
  const rootBrowseDir = mkdtempSync(path.join(homedir(), "000-codius-root-browse-"));
  const outsideHomeDir = mkdtempSync(path.join(tmpdir(), "codius-dir-suggestion-outside-"));
...
test("returns typed relative suggestions within a requested directory", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "codius-workspace-suggestion-"));
...
test("finds workspace files inside the OpenCode directory", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "codius-opencode-suggestion-"));
...
test("a Desktop-managed daemon does not advertise npm self-update", async () => {
  const daemon = await createTestCodiusDaemon({ desktopManaged: true });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
