...
export type PersistedWorkspacePlacement = Pick<
  ...
  | "worktreeRoot"
  | "baseBranch"
  | "isCodiusOwnedWorktree"
  | "mainRepoRoot"
>;

export type MutableWorkspacePlacement = Pick<
  PersistedWorkspaceRecord,
  "kind" | "branch" | "worktreeRoot" | "isCodiusOwnedWorktree" | "mainRepoRoot"
>;
...
export function initialWorkspacePlacement(
  ...
  return {
    ...
    baseBranch: null,
    isCodiusOwnedWorktree: input.checkout.isGit && input.checkout.isCodiusOwnedWorktree,
...
export function reconcileWorkspacePlacement(input: {
  ...
  if (input.workspace.isCodiusOwnedWorktree !== observed.isCodiusOwnedWorktree)
    fields.isCodiusOwnedWorktree = observed.isCodiusOwnedWorktree;
...
export function checkoutFromPersistedWorkspacePlacement(input: {
  ...
  if (workspace.isCodiusOwnedWorktree && workspace.mainRepoRoot) {
    return {
      ...checkout,
      isGit: true,
      isCodiusOwnedWorktree: true,
...
export function checkoutLiteFromGitSnapshot(
  ...
  if (git.isCodiusOwnedWorktree && git.mainRepoRoot) {
    return {
      cwd,
      isGit: true,
