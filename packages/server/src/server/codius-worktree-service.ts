...
import {
  ...
  rollbackCreatedCodiusWorktree,
  seedCodiusConfigFile,
  validateBranchSlug,
  type WorktreeConfig,
} from "../utils/worktree.js";
import { getCurrentBranch, localBranchExists, renameCurrentBranch } from "../utils/checkout-git.js";
import {
  markCodiusWorktreeFirstAgentBranchAutoNameAttempted,
...
export interface CreateCodiusWorktreeInput extends CreateWorktreeCoreInput {
  projectId?: string;
  title?: string;
}

export interface CreateCodiusWorktreeResult {
...
export type CreateCodiusWorktreeFn = (
...
export interface CreateCodiusWorktreeDeps extends CreateWorktreeCoreDeps {
  workspaceGitService: WorkspaceGitService;
  workspaceProvisioning: Pick<WorkspaceProvisioningService, "createWorkspaceForWorktree">;
}

export async function createCodiusWorktree(
  input: CreateCodiusWorktreeInput,
  deps: CreateCodiusWorktreeDeps,
): Promise<CreateCodiusWorktreeResult> {
  ...
  try {
    ...
    return rollbackCreatedCodiusWorktree(
      {
...
export async function attemptFirstAgentBranchAutoName(options: {
  ...
  let metadata: ReturnType<typeof readCodiusWorktreeMetadata>;
  try {
    metadata = readCodiusWorktreeMetadata(options.cwd);
  } catch {
    return { attempted: false, renamed: false, branchName: null };
  }
  if (
  ...
  if ((await getCurrentBranchImpl(options.cwd)) !== placeholderBranchName) {
    markCodiusWorktreeFirstAgentBranchAutoNameAttempted(options.cwd);
...
function maybeMarkFirstAgentBranchAutoNameEligible(options: {
  ...
  writeCodiusWorktreeFirstAgentBranchAutoNameMetadata(createdWorktree.worktree.worktreePath, {
