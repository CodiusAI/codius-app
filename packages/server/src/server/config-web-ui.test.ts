...
async function createCodiusHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "codius-config-web-ui-"));
  roots.push(root);
...
describe("daemon web UI config", () => {
  ...
  test("CODIUS_WEB_UI_ENABLED overrides persisted setting", async () => {
    const home = await createCodiusHome({
      version: 1,
      features: { webUi: { enabled: true } },
    });

    const config = loadConfig(home, { env: { CODIUS_WEB_UI_ENABLED: "false" } });

    expect(config.webUi.enabled).toBe(false);
  });

  test("CODIUS_WEB_UI_ENABLED=true enables web UI", async () => {
    const home = await createCodiusHome({ version: 1 });

    const config = loadConfig(home, { env: { CODIUS_WEB_UI_ENABLED: "true" } });
  ...
  test("CLI web UI enable override wins over env and persisted config", async () => {
    ...
    const config = loadConfig(home, {
      env: { CODIUS_WEB_UI_ENABLED: "false" },
  ...
  test("CLI web UI disable override wins over env and persisted config", async () => {
    ...
    const config = loadConfig(home, {
      env: { CODIUS_WEB_UI_ENABLED: "true" },
  ...
  test("resolves CODIUS_WEB_UI_DIST_DIR as absolute path", async () => {
    const home = await createCodiusHome({ version: 1 });
    const distDir = path.join(os.tmpdir(), "codius-web-ui-dist");

    const config = loadConfig(home, { env: { CODIUS_WEB_UI_DIST_DIR: distDir } });

    expect(config.webUi.distDir).toBe(path.resolve(distDir));
  });

  test("resolves relative persisted distDir against CODIUS_HOME", async () => {
    const home = await createCodiusHome({
      version: 1,
  ...
  test("CODIUS_WEB_UI_DIST_DIR overrides persisted distDir", async () => {
    const home = await createCodiusHome({
      version: 1,
    ...
    const config = loadConfig(home, { env: { CODIUS_WEB_UI_DIST_DIR: envDir } });
