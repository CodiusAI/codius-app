...
import {
  ...
  type CreateCodiusWorktreeDeps,
} from "./codius-worktree-service.js";
import { readCodiusWorktreeMetadata } from "../utils/worktree-metadata.js";
import { createWorktree, getCodiusWorktreesRoot } from "../utils/worktree.js";
...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createCodiusWorktree(
    {
      cwd: repoDir,
...
test("seeds an uncommitted exact-project config into the mapped worktree directory", async () => {
  ...
  writeFileSync(path.join(sourceDir, "codius.json"), config);
  ...
  expect(readFileSync(path.join(result.workspace.cwd, "codius.json"), "utf8")).toBe(config);
  expect(existsSync(path.join(result.worktree.worktreePath, "codius.json"))).toBe(false);
});
...
test("removes a new worktree when its ref does not contain the selected project directory", async () => {
  ...
  const worktreePath = path.join(
    await getCodiusWorktreesRoot(repoDir, codiusHome),
    "missing-subproject",
  );
...
test("maps a nested cwd from an existing Codius worktree into the next worktree", async () => {
  const { repoDir, tempDir } = createGitRepo();
  cleanupPaths.push(tempDir);
  const codiusHome = path.join(tempDir, ".codius");
  ...
  commitAll(repoDir, "add subproject");
  const deps = createDeps();
  const source = await createCodiusWorktree(
    {
      cwd: repoDir,
  ...
  const created = await createCodiusWorktree(
    {
      cwd: sourceCwd,
...
test("rejects source checkout planning before creating a worktree", async () => {
  ...
  expect(existsSync(path.join(codiusHome, "worktrees"))).toBe(false);
...
test.skipIf(isPlatform("win32"))(
  ...
  async () => {
    ...
    const first = await createCodiusWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createCodiusWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
...
test("renames an eligible unnamed branch-off worktree once on first agent context", async () => {
  ...
  expect(readCodiusWorktreeMetadata(created.worktree.worktreePath)).toMatchObject({
    version: 2,
...
test("does not mutate registries or broadcast when core worktree creation fails", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "codius-worktree-service-"));
  cleanupPaths.push(tempDir);
  const deps = createDeps();
...
// Worktree restore (Unit 3): recreate a deleted Codius-owned worktree from its
...
// repoRoot=null, so deleteCodiusWorktree's `git worktree remove`/`prune` is
...
interface TestDeps extends CreateCodiusWorktreeDeps {
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isCodiusOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
