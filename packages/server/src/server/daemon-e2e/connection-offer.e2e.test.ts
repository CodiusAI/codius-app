...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    process.env.CODIUS_PRIMARY_LAN_IP = "192.168.1.12";
    ...
    try {
      ...
      expect(offerUrl.startsWith("https://app.codius.ai/#offer=")).toBe(true);
      ...
      expect(offer.relay.endpoint).toBe("relay.codius.ai:443");
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(path.join(os.tmpdir(), "codius-offer-home-"));

    const { logger: logger1 } = createCapturingLogger();
    const daemon1 = await createTestCodiusDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
      codiusHomeRoot: tempHomeRoot,
      cleanup: false,
    });
    ...
    try {
      ...
      const daemon2 = await createTestCodiusDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        codiusHomeRoot: tempHomeRoot,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(path.join(os.tmpdir(), "codius-offer-e2e-"));
    ...
    const env = {
      ...process.env,
      CODIUS_HOME: tempHome,
      CODIUS_LISTEN: `0.0.0.0:${port}`,
      OPENAI_API_KEY: "",
