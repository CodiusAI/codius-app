...
test("project.add creates a project without creating a workspace", async () => {
  ...
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "codius-add-project-repo-")));
    const codiusHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "codius-add-project-home-")),
    );
    ...
    restoreEnv("CODIUS_SUPERVISED", previousSupervised);
  }
}, 30_000);
...
test("archiving the last workspace leaves the project parent with no workspaces", async () => {
  ...
  try {
    const repoRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "codius-empty-project-repo-")),
    );
    const codiusHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "codius-empty-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(codiusHomeRoot);
