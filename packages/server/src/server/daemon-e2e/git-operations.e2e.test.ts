...
test("runs codius.json setup asynchronously and reports status via timeline tool_call", async () => {
  ...
  execSync("git -c commit.gpgsign=false commit -m 'add codius.json'", {
    cwd: repoRoot,
    stdio: "pipe",
  });
  ...
  expect(agent.cwd).toContain(path.join(".codius", "worktrees"));
  ...
  const completed = await waitForTimelineToolCall(
    collector.messages,
    agent.id,
    (item) => item.name === "codius_worktree_setup" && item.status === "completed",
    20000,
  );
...
test("bootstraps configured worktree terminals after setup succeeds", async () => {
  await withShell("/bin/sh", async () => {
    ...
    const setupCommand =
      'while [ ! -f "$CODIUS_WORKTREE_PATH/allow-setup" ]; do sleep 0.05; done; echo "done" > "$CODIUS_WORKTREE_PATH/setup-done.txt"; echo "$CODIUS_WORKTREE_PORT" > "$CODIUS_WORKTREE_PATH/setup-port.txt"';
    writeFileSync(
      path.join(repoRoot, "codius.json"),
      JSON.stringify({
        worktree: {
    ...
    const terminalsBootstrapToolCall = await waitForTimelineToolCall(
      collector.messages,
      agent.id,
      (item) => item.name === "codius_worktree_terminals" && item.status === "completed",
      30000,
    );
    ...
    ctx.client.sendTerminalInput(manualTerminalId, {
      type: "input",
      data: 'echo "$CODIUS_WORKTREE_PORT" > "$CODIUS_WORKTREE_PATH/manual-terminal-port.txt"\r',
    });
    await waitForPathExists({
...
test("reports failures via timeline tool_call without deleting the created worktree", async () => {
  ...
  const setupCommand =
    'echo "started" > "$CODIUS_WORKTREE_PATH/setup-start.txt"; sleep 0.1; echo "boom" 1>&2; exit 7';
  writeFileSync(
    path.join(repoRoot, "codius.json"),
    JSON.stringify({
      worktree: {
  ...
  const started = await waitForTimelineToolCall(
    collector.messages,
    agent.id,
    (item) => item.name === "codius_worktree_setup" && item.status === "running",
    10000,
  );
...
test("creates agent in ~/.codius/worktrees/{hash} when worktree is requested", async () => {
  const cwd = tmpCwd();
  ...
  expect(realpathSync(agent.cwd)).toBe(
    realpathSync(path.join(ctx.daemon.codiusHome, "worktrees", projectHash, "worktree-test")),
  );
  expect(existsSync(agent.cwd)).toBe(true);
...
test("archiving a worktree shuts down its terminals but leaves the worktree on disk", async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      worktree: {
        ...
        teardown: [`echo "$CODIUS_WORKTREE_PATH" > "${teardownMarkerPath}"`],
      },
    }),
  );
  execSync("git add codius.json", { cwd: repoRoot, stdio: "pipe" });
  ...
  const archive = await ctx.client.archiveCodiusWorktree({
