...
function initGitRepo(repoDir: string): void {
  ...
  execSync("git config user.email 'codius-test@example.com'", {
    cwd: repoDir,
    stdio: "pipe",
  });
  execSync("git config user.name 'Codius Test'", {
    cwd: repoDir,
    stdio: "pipe",
  });
    ...
    async () => {
      ...
      try {
        ...
        const worktree = await createLegacyWorktreeForTest({
          ...
          codiusHome: ctx.daemon.codiusHome,
        });
        ...
        const worktreeList = await ctx.client.getCodiusWorktreeList({
          cwd: repoDir,
        });
        ...
        const archiveResult = await ctx.client.archiveCodiusWorktree({
        ...
        const worktreeListAfter = await ctx.client.getCodiusWorktreeList({
          cwd: repoDir,
        });
        expect(
