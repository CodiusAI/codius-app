...
const cleanupDaemons = new Set<TestCodiusDaemon>();
...
test("openProject preserves a worktree's exact-root project without rehoming it", async () => {
  const previousSupervised = process.env.CODIUS_SUPERVISED;
  process.env.CODIUS_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "codius-open-project-repo-")));
    const worktreeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "codius-open-project-worktree-")),
    );
    const codiusHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "codius-open-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(worktreeRoot);
    cleanupPaths.add(codiusHomeRoot);

    execSync("git init -b main", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.email 'test@codius-ai.dev'", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.name 'Codius Test'", { cwd: repoRoot, stdio: "pipe" });
    ...
    const projectsPath = path.join(codiusHome, "projects", "projects.json");
    const workspacesPath = path.join(codiusHome, "projects", "workspaces.json");
    ...
    const daemon = await createTestCodiusDaemon({ codiusHomeRoot, cleanup: false });
    ...
    process.env.CODIUS_SUPERVISED = previousSupervised;
  }
}, 30_000);
