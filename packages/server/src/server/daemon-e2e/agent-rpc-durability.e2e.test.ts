...
test("agent fetch RPCs tolerate an agent whose workspace project record is gone", async () => {
  ...
  try {
    daemon = await createTestCodiusDaemon({
      codiusHomeRoot: fixture.codiusHomeRoot,
      cleanup: false,
    });
...
function seedStaleAgentFixture(): StaleAgentFixture {
  const healthyCwd = mkdtempSync(path.join(os.tmpdir(), "codius-healthy-agent-"));
  const orphanCwd = mkdtempSync(path.join(os.tmpdir(), "codius-orphan-agent-"));
  const codiusHomeRoot = mkdtempSync(path.join(os.tmpdir(), "codius-orphan-agent-home-"));
  const codiusHome = path.join(codiusHomeRoot, ".codius");
  const projectsDir = path.join(codiusHome, "projects");
  const agentsDir = path.join(codiusHome, "agents");
  const healthyProjectId = "proj-healthy-agent-rpc";
  const healthyWorkspaceId = "ws-healthy-agent-rpc";
  const orphanWorkspaceId = "c:\\Users\\codius\\stale-project";
  ...
  return {
    ...
    orphanAgentId,
    codiusHomeRoot,
    cleanupPaths: [healthyCwd, orphanCwd, codiusHomeRoot],
  };
}
