...
export async function createOpenCodeOmoRealRuntime(): Promise<OpenCodeOmoRealRuntime> {
  ...
  const openCodeVersion =
    process.env.CODIUS_REAL_OPENCODE_VERSION?.trim() || PINNED_OPENCODE_VERSION;
  const omoVersion = process.env.CODIUS_REAL_OMO_VERSION?.trim() || PINNED_OMO_VERSION;
  ...
  if (model === NO_AUTH_MODEL) {
    await runCommand({
      command: openCodeCommand,
      args: ["run", "--model", model, "Reply with exactly: CODIUS_BIG_PICKLE_PROBE_OK"],
      cwd: paths.workspace,
      env: runtimeEnv,
      artifactName: "big-pickle-probe.log",
      artifacts: paths.artifacts,
      secrets,
      requiredOutput: "CODIUS_BIG_PICKLE_PROBE_OK",
    });
  }

  const previousCodiusHome = process.env.CODIUS_HOME;
  ...
  let daemon: TestCodiusDaemon | null = null;
  let client: DaemonClient | null = null;
  try {
    process.env.CODIUS_HOME = path.join(paths.codiusHomeRoot, ".codius");
    ...
    daemon = await createTestCodiusDaemon({
      agentClients: { opencode: openCodeClient },
      logger,
      codiusHomeRoot: paths.codiusHomeRoot,
    ...
    return {
      ...
      close: async (passed) => {
        try {
          ...
          restoreEnvironment("CODIUS_HOME", previousCodiusHome);
        }
      },
    };
  } catch (error) {
...
function createRuntimePaths(): RuntimePaths {
  const root = mkdtempSync(path.join(tmpdir(), "codius-real-opencode-omo-"));
  const paths: RuntimePaths = {
    root,
    home: path.join(root, "home"),
    codiusHomeRoot: path.join(root, "codius-home"),
...
function resolveModel(openRouterApiKey: string | null): string {
  const explicitModel = process.env.CODIUS_REAL_OPENCODE_MODEL?.trim();
  if (explicitModel) {
...
function buildRuntimeEnv(paths: RuntimePaths, openRouterApiKey: string | null): NodeJS.ProcessEnv {
  return {
    ...
    CODIUS_HOME: path.join(paths.codiusHomeRoot, ".codius"),
