...
function getServerIdPath(codiusHome: string): string {
  return path.join(codiusHome, SERVER_ID_FILENAME);
}
...
/**
 * Stable daemon identifier scoped to a given $CODIUS_HOME.
 *
 * - Persisted to `$CODIUS_HOME/server-id`
 * - Can be overridden via `CODIUS_SERVER_ID` (useful for tests)
 */
export function getOrCreateServerId(
  ...
  const serverIdPath = getServerIdPath(codiusHome);

  const envOverride =
    typeof env.CODIUS_SERVER_ID === "string" && env.CODIUS_SERVER_ID.trim().length > 0
      ? env.CODIUS_SERVER_ID.trim()
      : null;
  ...
  if (envOverride) {
    ...
    if (!existsSync(serverIdPath)) {
      try {
        ...
        log?.info({ serverId: envOverride }, "Persisted CODIUS_SERVER_ID override");
      } catch (error) {
        log?.warn({ error }, "Failed to persist CODIUS_SERVER_ID override");
      }
    } else {
