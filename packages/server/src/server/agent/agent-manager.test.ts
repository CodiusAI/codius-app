  ...
  override async createSession(
    ...
    const script = `
      process.stdout.write(JSON.stringify({
        ...
        agentId: process.env.CODIUS_AGENT_ID ?? null
      }));
    `;
...
test("createAgent injects codius MCP server only into provider launch config", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
...
test("createAgent passes native Codius tools through launch context without internal MCP", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);

  const codiusTools: CodiusToolCatalog = {
    tools: new Map(),
    getTool: () => undefined,
    executeTool: async () => {
  ...
  const manager = new AgentManager({
    ...
    logger,
    mcpBaseUrl: "http://127.0.0.1:6767/mcp/agents",
    codiusToolCatalogFactory: () => codiusTools,
  ...
  expect(client.lastLaunchContext?.codiusTools).toBe(codiusTools);
  expect(client.lastConfig?.mcpServers).toEqual({
    custom: {
      type: "stdio",
      command: "custom-mcp",
...
test("createAgent injects the MCP auth token as a bearer header into the launch config", async () => {
  ...
  expect(client.lastConfig?.mcpServers?.codius).toEqual({
    type: "http",
    url: `http://127.0.0.1:6767/mcp/agents?callerAgentId=${snapshot.id}`,
...
test("resumeAgentFromPersistence replaces stored internal codius MCP with current runtime URL", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
  const client = new TestAgentClient();
...
test("resumeAgentFromPersistence drops stored internal codius MCP when runtime injection is disabled", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
  const client = new TestAgentClient();
...
test("createAgent preserves a user-provided codius MCP config", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
  ...
  const snapshot = await manager.createAgent(
    {
      ...
      mcpServers: {
        codius: {
          type: "http",
          url: "https://example.com/custom-codius",
        },
      },
    },
    undefined,
  ...
  const resumed = await manager.resumeAgentFromPersistence(handle, {
    ...
    mcpServers: {
      codius: {
        type: "stdio",
        command: "node",
        args: ["/tmp/mcp-bridge.mjs", "--socket", "/tmp/codius.sock"],
      },
    },
  });
...
test("user_message events wrapping a codius-system envelope are not added to the timeline", async () => {
...
test("user_message events wrapping a codius-system envelope are not restored during history replay", async () => {
