...
function createCatalog(tools: CodiusToolDefinition[]): CodiusToolCatalog {
...
class OmpHostToolHarness {
  ...
  private resolveControlledResult: ((result: CodiusToolResult) => void) | null = null;
  ...
  static async withTools(tools: CodiusToolDefinition[]): Promise<OmpHostToolHarness> {
  ...
  static async cancellable(): Promise<OmpHostToolHarness> {
    ...
    const tool: CodiusToolDefinition = {
      name: "wait_for_agent",
      description: "Wait for a Codius agent.",
      handler: async (_input, context) => {
        harness.controlledSignal = context.signal ?? null;
        harness.resolveControlledStart?.();
        return await new Promise<CodiusToolResult>((resolve) => {
  ...
  completeControlledCall(result: CodiusToolResult): void {
  ...
  private routerInput() {
    return { runtimeSession: this.runtimeSession, codiusTools: this.catalog, logger: this.logger };
  }
}

describe("OMP host tools", () => {
  test("serializes the caller-scoped Codius catalog for set_host_tools", () => {
