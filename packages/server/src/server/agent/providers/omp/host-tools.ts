...
import {
  ...
  serializeCodiusToolInputParameters,
} from "../../tools/codius-tool-serialization.js";
import type { CodiusToolCatalog, CodiusToolResult } from "../../tools/types.js";
...
interface OmpHostToolRouterInput {
  ...
  catalog: CodiusToolCatalog;
  logger: Logger;
}

const routersByRuntimeSession = new WeakMap<OmpRuntimeSession, OmpHostToolRouter>();

export function serializeOmpHostTools(catalog: CodiusToolCatalog): OmpRpcHostToolDefinition[] {
...
function sendMissingCatalogResult(
  ...
  runtimeSession.sendHostToolResult(
    toOmpHostToolErrorResult(
      request.id,
      `Host tool "${request.toolName}" was called before Codius tools were registered`,
    ),
  );
}
...
class OmpHostToolRouter {
  ...
  private readonly catalog: CodiusToolCatalog;
  ...
  private sendUpdate(callId: string, result: CodiusToolResult): void {
...
function toOmpHostToolResult(id: string, result: CodiusToolResult): OmpRpcHostToolResult {
...
function toOmpAgentToolResult(result: CodiusToolResult): OmpAgentToolResult {
