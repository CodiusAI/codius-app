...
export function normalizeClaudeAskUserQuestionRequestInput(
  ...
  // model-supplied option. Codius's shared question UI uses allowOther for that
...
export function normalizeClaudeAskUserQuestionUpdatedInput(
  ...
  // Codius's shared question UI serializes answers by question header, but Claude's
...
class ClaudeAgentSession implements AgentSession {
   ...
   * — and the level Codius requests is not necessarily the level that runs, because a model that
  ...
  private mapBlocksToTimeline(
    ...
    const items: AgentTimelineItem[] = [];
    // User SDK entries can arrive as multiple text blocks, but Codius treats them as one message.
