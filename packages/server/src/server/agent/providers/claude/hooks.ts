...
/**
 * Combine Codius's observation hooks with any the user configured, per event.
 *
 * Neither side wins: Claude Code runs every matcher registered for an event, so appending keeps
 * a user's hooks working while Codius keeps observing. Assigning either one would silently drop
