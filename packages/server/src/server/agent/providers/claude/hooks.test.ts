...
describe("mergeClaudeHooks", () => {
  ...
  it("keeps Codius's hooks when the user configured none", () => {
  ...
  it("appends user hooks for the same event instead of replacing them", () => {
    ...
    // Both must survive: dropping Codius's would silently stop effort being observed, and
    // dropping the user's would break their configuration.
    expect(merged.PreToolUse).toHaveLength(2);
  });

  it("carries through events Codius does not register", () => {
