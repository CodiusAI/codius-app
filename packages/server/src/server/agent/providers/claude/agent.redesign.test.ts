...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "CODIUS_ENV_SENTINEL_SECRET";
  const runtimeSecret = "CODIUS_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "CODIUS_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.CODIUS_TEST_SENTINEL_SECRET;
  process.env.CODIUS_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        CODIUS_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.CODIUS_TEST_SENTINEL_SECRET;
    } else {
      process.env.CODIUS_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
