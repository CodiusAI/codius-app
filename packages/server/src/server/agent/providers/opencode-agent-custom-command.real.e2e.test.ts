...
describe("opencode custom command Big Pickle E2E (real)", () => {
  ...
  test("executes a global custom command through Codius using Big Pickle", async () => {
    ...
    const projectDir = await mkdtemp(path.join(tmpdir(), "codius-opencode-big-pickle-"));
    ...
    try {
      ...
      await writeFile(
        ...
        [
          "---",
          "description: Codius issue 903 Big Pickle custom command",
          "agent: build",
          "---",
          "",
