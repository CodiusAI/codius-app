...
describe("ProviderSnapshotManager public surface", () => {
  ...
  test("CODIUS_PROVIDER_REFRESH_TIMEOUT_MS env var is honored when no option is given", async () => {
    vi.stubEnv("CODIUS_PROVIDER_REFRESH_TIMEOUT_MS", "1");
    const isAvailable = vi.fn(() => new Promise<boolean>(() => {}));
    const manager = new ProviderSnapshotManager({
      logger: createTestLogger(),
      providerOverrides: {
  ...
  test("CODIUS_PROVIDER_REFRESH_TIMEOUT_MS env var is ignored when option is provided", async () => {
    vi.stubEnv("CODIUS_PROVIDER_REFRESH_TIMEOUT_MS", "1");
    const isAvailable = vi.fn(() => new Promise<boolean>(() => {}));
    const manager = new ProviderSnapshotManager({
      logger: createTestLogger(),
  ...
  test("getProviderDiagnostic reports a stuck catalog refresh inside the diagnostic", async () => {
    await withEnv("CODIUS_ENABLE_MOCK_SLOW", "true", async () => {
      vi.useFakeTimers();
      const manager = new ProviderSnapshotManager({
        logger: createTestLogger(),
        isDev: true,
