...
let tempRoot: string;
let daemonHandle: TestCodiusDaemon;
...
beforeAll(async () => {
  ...
  daemonHandle = await createTestCodiusDaemon({ agentClients: createRecordingAgentClients() });
...
describe("Suite A: Core Fixes", () => {
  ...
  test("agentManager.createAgent injects codius MCP using the daemon listen target", async () => {
    let agentId: string | null = null;
    try {
      ...
      expect(snapshot.config.mcpServers?.codius).toBeUndefined();

      const liveAgent = daemonHandle.daemon.agentManager.getAgent(agentId);
      expect(liveAgent?.config.mcpServers?.codius).toBeUndefined();
    } finally {
      await archiveAgentIfPresent(agentId);
    }
  });
