...
import {
  ...
  type CreateCodiusWorktreeCommandInput,
  createCodiusWorktreeCommand,
...
import type {
  CodiusToolCatalog,
  CodiusToolConfig,
  CodiusToolDefinition,
  CodiusToolExecutionContext,
  CodiusToolResult,
} from "./types.js";

export interface CodiusToolHostDependencies {
  ...
  createCodiusWorktree?: CreateCodiusWorktreeWorkflowFn;
...
type WorkspaceWorktreeTarget = Pick<
  CreateCodiusWorktreeCommandInput,
...
export function createCodiusToolCatalog(options: CodiusToolHostDependencies): CodiusToolCatalog {
  const {
    agentManager,
    agentStorage,
    terminalManager,
  ...
  const childLogger = logger.child({ module: "agent", component: "codius-tool-catalog" });
  const callerContext = callerAgentId ? (resolveCallerContext?.(callerAgentId) ?? null) : null;

  const parseToolInput = async (tool: CodiusToolDefinition, input: unknown): Promise<unknown> => {
  ...
  const tools = new Map<string, CodiusToolDefinition>();
  const registerTool = (
    name: string,
    ...
    handler: (input: any, context: CodiusToolExecutionContext) => Promise<CodiusToolResult>,
  ) => {
    tools.set(name, {
      name,
      title: config.title,
      ...
      handler: handler as CodiusToolDefinition["handler"],
    });
  };
  const toCatalog = (): CodiusToolCatalog => ({
    tools,
    ...
    async executeTool(
      ...
      if (!tool) {
        throw new Error(`Codius tool not found: ${name}`);
      }
    ...
    z
      .object({
        ...
        worktreeSlug: z
          ...
          .min(1)
          .optional()
          .describe("Optional worktree slug/path label. Omit to let Codius generate one."),
        branchName: z
          .string()
          .min(1)
          .optional()
      ...
      .strict()
      .describe("Create a new branch in a new Codius worktree."),
    z
      .object({
      ...
      .strict()
      .describe("Check out an existing branch in a new Codius worktree."),
    z
      .object({
      ...
      .strict()
      .describe("Check out a GitHub pull request in a new Codius worktree."),
  ]);
  ...
  registerTool(
    "create_workspace",
    {
      ...
      description:
        "Create a workspace using an existing local checkout or a new Codius-managed worktree.",
      inputSchema: {
        isolation: z.enum(["local", "worktree"]),
        path: z
          .string()
    ...
    async ({
      ...
      if (isolation === "local") {
        ...
        const result = await createCodiusWorktreeCommand(
          {
            codiusHome: options.codiusHome,
            worktreesRoot: options.worktreesRoot,
            createCodiusWorktreeWorkflow: options.createCodiusWorktree,
          },
          {
            cwd,
            ...(projectId ? { projectId } : {}),
    ...
    async (args: unknown) => {
      ...
      const {
      ...
      } = await createAgentCommand(
        {
          ...
          terminalManager,
          providerSnapshotManager,
          createCodiusWorktree: options.createCodiusWorktree,
  ...
  registerTool(
    "start_workspace_script",
    {
      ...
      description:
        "Start one configured workspace script through Codius's managed workspace-script launcher.",
      inputSchema: {
        workspaceId: z.string().describe("Workspace ID containing the configured script."),
        scriptName: z.string().min(1).describe("Configured codius.json script name to start."),
      },
      outputSchema: {
  ...
  registerTool(
    "stop_workspace_script",
    {
      ...
      inputSchema: {
        ...
        scriptName: z.string().min(1).describe("Configured codius.json script name to stop."),
      },
      outputSchema: {
        script: WorkspaceScriptPayloadSchema,
      },
...
function archiveWorktreeDependencies(
  options: CodiusToolHostDependencies,
  ...
  return {
    codiusHome: options.codiusHome,
    codiusWorktreesBaseRoot: options.worktreesRoot,
