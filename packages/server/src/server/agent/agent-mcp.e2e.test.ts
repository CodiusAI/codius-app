...
class RecordingAgentClient implements AgentClient {
  ...
  constructor(
    ...
    this.capabilities = {
      ...
      supportsNativeCodiusTools: false,
    };
  }
...
describe("agent MCP end-to-end (offline)", () => {
  test("create_agent runs initial prompt and affects filesystem", async () => {
    const codiusHome = await mkdtemp(path.join(os.tmpdir(), "codius-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "codius-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "codius-agent-cwd-"));
    const port = await getAvailablePort();

    const daemonConfig: CodiusDaemonConfig = {
    ...
    const daemon = await createCodiusDaemon(daemonConfig, pino({ level: "silent" }));
    await daemon.start();
  ...
  test("create_agent auto-injects codius MCP by default and can be disabled", async () => {
    const codiusHome = await mkdtemp(path.join(os.tmpdir(), "codius-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "codius-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "codius-agent-cwd-"));
    const port = await getAvailablePort();
    ...
    const disabledCodiusHome = await mkdtemp(path.join(os.tmpdir(), "codius-home-disabled-"));
    const disabledStaticDir = await mkdtemp(path.join(os.tmpdir(), "codius-static-disabled-"));
    const disabledAgentCwd = await mkdtemp(path.join(os.tmpdir(), "codius-agent-cwd-disabled-"));
    const disabledPort = await getAvailablePort();
    const disabledRecorder: LaunchRecorder = { recordedLaunches: [] };
    const disabledDaemonConfig: CodiusDaemonConfig = {
      listen: `127.0.0.1:${disabledPort}`,
      codiusHome: disabledCodiusHome,
      corsAllowedOrigins: [],
      hostnames: true,
      mcpEnabled: true,
      ...
      agentStoragePath: path.join(disabledCodiusHome, "agents"),
    };
    const disabledDaemon = await createCodiusDaemon(
    ...
    try {
      ...
      expect(injectedAgent?.config.mcpServers?.codius).toBeUndefined();
      ...
      expect(disabledRecorder.recordedLaunches.at(-1)?.mcpServers?.codius).toBeUndefined();
      const disabledAgent = disabledDaemon.agentManager.getAgent(disabledAgentId!);
      expect(disabledAgent?.config.mcpServers?.codius).toBeUndefined();
    } finally {
      if (agentId) {
        await client.callTool({ name: "kill_agent", args: { agentId } });
      }
      ...
      await rm(disabledCodiusHome, { recursive: true, force: true });
  ...
  test("create_agent with worktree is async and boots terminals only after setup success", async () => {
    const codiusHome = await mkdtemp(path.join(os.tmpdir(), "codius-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "codius-static-"));
    const repoRoot = await mkdtemp(path.join(os.tmpdir(), "codius-worktree-repo-"));
    const port = await getAvailablePort();
    ...
    try {
      ...
      const setupCommand =
        'while [ ! -f "$CODIUS_WORKTREE_PATH/allow-setup" ]; do sleep 0.05; done; echo "done" > "$CODIUS_WORKTREE_PATH/setup-done.txt"';
      await writeFile(
        path.join(repoRoot, "codius.json"),
        JSON.stringify({
          worktree: {
            setup: [setupCommand],
            terminals: [
      ...
      execSync("git add codius.json", { cwd: repoRoot, stdio: "pipe" });
