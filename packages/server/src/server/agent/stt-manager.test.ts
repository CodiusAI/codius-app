...
describe("STTManager", () => {
  ...
  it("uses CODIUS_VOICE_LANGUAGE over CODIUS_DICTATION_LANGUAGE", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
        CODIUS_VOICE_LANGUAGE: "pt",
        CODIUS_DICTATION_LANGUAGE: "es",
  ...
  it("uses CODIUS_DICTATION_LANGUAGE when CODIUS_VOICE_LANGUAGE is unset", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses streaming segmentation for batch transcription and concatenates segment finals", async () => {
    const original = process.env.CODIUS_STT_BATCH_COMMIT_EVERY_SECONDS;
    process.env.CODIUS_STT_BATCH_COMMIT_EVERY_SECONDS = "1";
    ...
    try {
      ...
      if (original === undefined) {
        delete process.env.CODIUS_STT_BATCH_COMMIT_EVERY_SECONDS;
      } else {
        process.env.CODIUS_STT_BATCH_COMMIT_EVERY_SECONDS = original;
      }
    }
  });
});
