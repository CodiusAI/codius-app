...
import { addModelVisibleStructuredContent } from "./tools/codius-tool-serialization.js";
import { createCodiusToolCatalog, type CodiusToolHostDependencies } from "./tools/codius-tools.js";
import type { CodiusToolResult } from "./tools/types.js";

export type AgentMcpServerOptions = CodiusToolHostDependencies;

type McpToolContext = RequestHandlerExtra<ServerRequest, ServerNotification>;

function toMcpToolResult(result: CodiusToolResult): CallToolResult {
...
export async function createAgentMcpServer(options: AgentMcpServerOptions): Promise<McpServer> {
  const catalog = await createCodiusToolCatalog(options);
