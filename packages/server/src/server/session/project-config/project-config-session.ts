...
} from "../../../utils/codius-config-file.js";
...
/**
 * A client's read/write surface for a project's on-disk codius.json. Resolves the
...
export class ProjectConfigSession {
  ...
  async handleReadProjectConfigRequest(
    ...
    const result = readCodiusConfigForEdit(repoRoot);
    if (!result.ok) {
      this.logger.warn(
