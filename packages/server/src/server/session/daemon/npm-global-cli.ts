...
export interface NpmGlobalCodiusInstall {
  version: string;
  packagePath: string;
...
export interface NpmGlobalCodiusCli {
  inspect(): Promise<NpmGlobalCodiusInstall>;
...
function parseNpmGlobalCodiusInstall(stdout: string): NpmGlobalCodiusInstall | null {
  ...
  const rawCliPackage = list.data.dependencies?.[CODIUS_CLI_PACKAGE];
...
export class DefaultNpmGlobalCodiusCli implements NpmGlobalCodiusCli {
  constructor(private readonly runCommand: CommandRunner = runExternalCommand) {}

  async inspect(): Promise<NpmGlobalCodiusInstall> {
    const result = await this.runCommand(
      "npm",
      ["-g", "ls", CODIUS_CLI_PACKAGE, "--json", "--depth=0", "--long"],
      {
    ...
    const install = parseNpmGlobalCodiusInstall(result.stdout);
    if (!install) {
      throw new Error(`${CODIUS_CLI_PACKAGE} is not installed with npm -g on this host`);
    }
    return install;
  }

  installLatest(): Promise<CommandResult> {
    return this.runCommand("npm", ["install", "-g", `${CODIUS_CLI_PACKAGE}@latest`], {
...
export const npmGlobalCodiusCli = new DefaultNpmGlobalCodiusCli();
