...
import type { CommandResult, NpmGlobalCodiusInstall } from "./npm-global-cli.js";
...
type Inspection = NpmGlobalCodiusInstall | Error;
...
describe("DaemonSelfUpdater", () => {
  ...
  test("updates a daemon that is running from the npm global cli install", async () => {
    ...
    const runtime = createRuntime({
      calls,
      inspections: [npmGlobalCodiusInstall("0.1.15"), npmGlobalCodiusInstall("0.1.96")],
    });
  ...
  test("does not update a daemon whose version does not match the npm global cli", async () => {
    ...
    const runtime = createRuntime({
      calls,
      inspections: [npmGlobalCodiusInstall("0.1.15")],
    });
  ...
  test("does not update linked global installs", async () => {
    const runtime = createRuntime({
      inspections: [npmGlobalCodiusInstall("0.1.15", { linked: true })],
    });
  ...
  test("rejects concurrent update requests", async () => {
    ...
    const runtime: DaemonSelfUpdateRuntime = {
      npm: {
        async inspect() {
          calls.push("inspect");
          return npmGlobalCodiusInstall("0.1.15");
        },
        async installLatest() {
          calls.push("installLatest");
