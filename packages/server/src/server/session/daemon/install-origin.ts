...
import { CODIUS_CLI_PACKAGE, type NpmGlobalCodiusInstall } from "./npm-global-cli.js";
...
export function validateDaemonInstallOrigin(
  install: NpmGlobalCodiusInstall,
  daemonVersion: string | null,
  runtime: DaemonInstallOriginRuntime = daemonInstallOriginRuntime,
): string | null {
  if (install.isLinked) {
    return `The global ${CODIUS_CLI_PACKAGE} install is linked; self-update only supports normal npm global installs.`;
  }

  if (daemonVersion && install.version !== daemonVersion) {
    return `This daemon is not running from the npm global ${CODIUS_CLI_PACKAGE} install (global npm has ${install.version}, daemon is ${daemonVersion}).`;
  }
  ...
  if (!isCurrentServerUnderNpmInstall(currentServerPackageRoot, install)) {
    return `This daemon is not running from the npm global ${CODIUS_CLI_PACKAGE} install.`;
  }
