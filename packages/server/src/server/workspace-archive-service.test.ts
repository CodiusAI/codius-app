...
interface ArchiveDepsInput {
  ...
  codiusWorktreesBaseRoot?: string;
...
function createArchiveDeps(input: ArchiveDepsInput): ArchiveTestDependencies {
  ...
  return {
    ...
    codiusWorktreesBaseRoot: input.codiusWorktreesBaseRoot,
...
describe("archiveByScope", () => {
  test("workspace scope archives the record and removes the directory on last reference", async () => {
    ...
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "last-ref-workspace");
  ...
  test("workspace scope runs teardown while keeping a directory referenced by a sibling", async () => {
    const { tempDir, repoDir } = createGitRepo();
    writeFileSync(
      path.join(repoDir, "codius.json"),
      JSON.stringify({
        worktree: {
          teardown: [
            "node -e \"require('fs').writeFileSync(process.env.CODIUS_SOURCE_CHECKOUT_PATH + '/shared-teardown.log', 'ok')\"",
          ],
        },
      }),
    );
    ...
    const codiusHome = path.join(tempDir, ".codius");
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "sibling-workspace");
  ...
  test("workspace scope keeps a worktree for an active workspace in a subdirectory", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const codiusHome = path.join(tempDir, ".codius");
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "subdirectory-sibling");
  ...
  test("archiving a subdirectory workspace keeps its active worktree root", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const codiusHome = path.join(tempDir, ".codius");
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "subdirectory-target");
  ...
  test("workspace scope runs teardown from the exact nested workspace before deleting its worktree", async () => {
    ...
    writeFileSync(
      path.join(sourceNested, "codius.json"),
      JSON.stringify({
        worktree: {
          teardown: [
            "node -e \"require('fs').writeFileSync(process.env.CODIUS_SOURCE_CHECKOUT_PATH + '/nested-teardown.log', process.cwd())\"",
          ],
        },
      }),
    );
    ...
    const codiusHome = path.join(tempDir, ".codius");
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "nested-teardown");
  ...
  test("worktree scope archives root and subdirectory workspaces before removing the backing worktree", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        worktree: {
          teardown: [
            "node -e \"const fs=require('fs');const out=process.env.CODIUS_SOURCE_CHECKOUT_PATH+'/root-scope-teardown.log';if(fs.existsSync(out))process.exit(2);fs.writeFileSync(out,'ok')\"",
          ],
        },
      }),
    );
    writeFileSync(
      ...
      JSON.stringify({
        worktree: {
          teardown: [
            "node -e \"require('fs').writeFileSync(process.env.CODIUS_SOURCE_CHECKOUT_PATH+'/nested-scope-teardown.log','ok')\"",
          ],
        },
      }),
    );
    ...
    const codiusHome = path.join(tempDir, ".codius");
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "worktree-scope");
  ...
  test("workspace scope never removes a non-Codius-owned directory", async () => {
  ...
  test("worktree scope keeps the directory when one record teardown fails", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const codiusHome = path.join(tempDir, ".codius");
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "partial-failure");
  ...
  test("worktree scope removes an owned directory with zero matching records", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const codiusHome = path.join(tempDir, ".codius");
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "zero-records");
  ...
  test("marks archiving, emits an upsert carrying the archiving state, then clears it and emits a remove", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const codiusHome = path.join(tempDir, ".codius");
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "lifecycle");
  ...
  test("archives stored snapshots only for the target workspace", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const codiusHome = path.join(tempDir, ".codius");
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "snapshot-scope");
  ...
  test("worktree scope archives three workspaces on the directory and removes it", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const codiusHome = path.join(tempDir, ".codius");
    const worktree = await createCodiusOwnedWorktree(repoDir, codiusHome, "worktree-scope-n3");
