...
function resolveFilePath(codiusHome: string, configuredPath: string | undefined): string {
  const fallback = path.join(codiusHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (!configuredPath) {
    return fallback;
  }
  ...
  return path.resolve(codiusHome, configuredPath);
}
...
function resolveConfiguredCodiusHome(options: ResolveLogConfigOptions | undefined): string {
  if (options?.codiusHome) {
    return options.codiusHome;
  }
  return resolveCodiusHome();
}
...
export function resolveLogConfig(
  ...
  const codiusHome = resolveConfiguredCodiusHome(options);
  ...
  const file =
    options?.file !== false && persistedLog?.file
      ? {
          ...
          path: resolveFilePath(codiusHome, persistedLog.file.path),
        }
      : undefined;
