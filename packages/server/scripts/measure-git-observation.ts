...
async function main(): Promise<void> {
  const tempDir = mkdtempSync(path.join(tmpdir(), "codius-git-observation-measurement-"));
  ...
  runGit(repoDir, ["config", "user.email", "measurement@codius.local"]);
  ...
  try {
    healthy = createMeasuredService({ repoDir, codiusHome });
    ...
    degraded = createMeasuredService({ repoDir, codiusHome, failWorktreeWatch: true });
    startGitCommandMetrics();
    ...
    const outputPath = process.env.CODIUS_GIT_OBSERVATION_REPORT?.trim();
    if (outputPath) {
