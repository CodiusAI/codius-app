...
async function verifyInjectedMcpForProvider(
  ...
  try {
    const prompt = [
      ...
      "If you have a tool called list_agents or create_agent from a codius MCP server, call list_agents once.",
      "After checking, reply with exactly CODIUS_MCP_FOUND.",
      "If you do not have those tools, reply with exactly CODIUS_MCP_NOT_FOUND.",
    ...
    if (!assistantText.includes("CODIUS_MCP_FOUND")) {
      throw new Error(
        `Expected assistant to confirm Codius MCP availability. Assistant text:\n${assistantText}`,
      );
    }
...
async function main(): Promise<void> {
  ...
  const rootCwd = await mkdtemp(path.join(os.tmpdir(), "codius-mcp-inject-real-"));
