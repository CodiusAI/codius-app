...
import { createTestCodiusDaemon } from "../src/server/test-utils/codius-daemon.js";
...
function parseArgs(): { sourceHome: string; frozenHomeRoot: string | null; scenario: Scenario } {
  let sourceHome = process.env.CODIUS_BENCHMARK_SOURCE_HOME ?? path.join(os.homedir(), ".codius");
  let frozenHomeRoot = process.env.CODIUS_BENCHMARK_FROZEN_HOME_ROOT ?? null;
  let scenario = (process.env.CODIUS_BENCHMARK_SCENARIO ?? "snapshotOnly") as Scenario;
...
async function freezeHome(sourceHome: string, requestedRoot: string | null): Promise<string> {
  const frozenHomeRoot = requestedRoot ?? mkdtempSync(path.join(os.tmpdir(), "codius-real-home-"));
  if (process.env.CODIUS_BENCHMARK_REUSE_FROZEN_HOME === "1") {
    return frozenHomeRoot;
  }
  const frozenHome = path.join(frozenHomeRoot, ".codius");
...
async function main(): Promise<void> {
  ...
  const daemon = await createTestCodiusDaemon({ codiusHomeRoot: frozenHomeRoot, cleanup: false });
