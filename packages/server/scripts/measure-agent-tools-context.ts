...
const MEASUREMENT_CWD = "/tmp/codius-agent-tools-measurement";
...
Measures the MCP tools/list payload for Codius agent tools.
...
async function measureCatalog(params: {
  ...
  const client = new Client({ name: "codius-agent-tools-measurement", version: "0.0.0" });
...
function formatMarkdown(measurements: CatalogMeasurement[], top: number): string {
  ...
  const delta =
    withoutBrowser && withBrowser
      ...
      : null;
  const lines: string[] = [];
  lines.push("# Codius Agent Tool Catalog Context");
  lines.push("");
