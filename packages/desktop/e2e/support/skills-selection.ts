...
export async function createSkillsSandbox(
  ...
  const bundledSkills = options.bundledSkills ?? ["codius", "codius-advisor", "codius-loop"];
  const root = await mkdtemp(path.join(os.tmpdir(), "codius-e2e-skills-"));
...
export async function serveRealSkillsCommands(page: Page, sandbox: SkillsSandbox): Promise<void> {
  ...
  await page.addInitScript((commands: readonly string[]) => {
    const bridge = (window as unknown as { codiusDesktop?: { invoke: unknown } }).codiusDesktop;
    ...
    bridge.invoke = (command: string, args?: Record<string, unknown>) =>
      skillCommands.has(command)
        ? window.__codiusSkillsInvoke!(command, args ?? null)
        : inner(command, args);
  }, SKILL_COMMANDS);
}
