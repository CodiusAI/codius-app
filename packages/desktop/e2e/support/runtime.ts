 ...
 * status endpoint, PID from the codius.pid lock file, log path from the
 * E2E_CODIUS_HOME directory. Call this in Node test code (not in the browser).
 */
export async function loadRealDaemonState(): Promise<RealDaemonState> {
  const port = getE2EDaemonPort();
  const codiusHome = process.env.E2E_CODIUS_HOME;
  if (!codiusHome) throw new Error("E2E_CODIUS_HOME not set — the worker fixture must run first");
  ...
  try {
    const raw = readFileSync(`${codiusHome}/codius.pid`, "utf8");
    ...
    console.warn("[desktop-updates] codius.pid not found:", err);
  }

  return { version: data.version, pid, logPath: `${codiusHome}/daemon.log` };
}
...
/**
 * Injects window.codiusDesktop before app load so all Electron-gated code
...
export async function installDesktopRuntime(
  ...
  await page.addInitScript((cfg) => {
    ...
    (window as unknown as { codiusDesktop: unknown }).codiusDesktop = desktopBridge;
  }, config);
}
