...
test.describe("Choosing installed skills", () => {
  test("installs only the skills selected in Settings", async ({ page, startSkills }) => {
    ...
    await chooseCustomSkills(page, ["codius", "codius-loop"]);
    await saveSkillSelection(page);

    await expectSkillsInstalled(skills, ["codius", "codius-loop"]);
    await expectSelectedSkills(page, ["codius", "codius-loop"]);
  });
  ...
  test("all skills includes every available skill", async ({ page, startSkills }) => {
    ...
    await openSkillsIntegrations(page);
    await openSkillSelection(page);
    await chooseCustomSkills(page, ["codius"]);
    await saveSkillSelection(page);
  ...
  test("cancelling leaves the installed skills untouched", async ({ page, startSkills }) => {
    ...
    await openSkillSelection(page);
    await toggleSkill(page, "codius-loop");
    await cancelSkillSelection(page);

    await expectSkillsInstalled(skills, ["codius"]);
    await expectSelectedSkills(page, ["codius"]);
  });
...
test.describe("Removing an installed skill", () => {
  test("delete-only drift stays in Choose skills instead of Update", async ({
    ...
    await startSkills({
      installed: { mode: "custom", skills: ["codius"] },
      placeOnDisk: {
        skill: "codius-loop",
        target: "claude",
        files: { "SKILL.md": "# codius-loop\n", "notes/mine.md": "my notes" },
      },
    });
    await gotoAppShell(page);
    await openSkillsIntegrations(page);
  ...
  test("warns before deleting the skill folders", async ({ page, startSkills }) => {
    ...
    await openSkillSelection(page);
    await chooseCustomSkills(page, ["codius", "codius-advisor"]);
    await saveSkillSelection(page);

    await expectRemovalWarning(page, ["codius-loop"]);
    await expectSkillSelectionOpen(page);
    await expectSkillsInstalled(skills, skills.bundledSkills);
  ...
  test("confirming the warning removes the skill", async ({ page, startSkills }) => {
    ...
    await expectRemovalWarning(page, ["codius-loop"]);
    await expectSkillsInstalled(skills, ["codius", "codius-advisor"]);
    await expectSelectedSkills(page, ["codius", "codius-advisor"]);
  });

  test("warns for a skill installed in only one agent directory", async ({ page, startSkills }) => {
    const skills = await startSkills({
      installed: { mode: "all" },
      keepOnlyIn: { skill: "codius-loop", target: "claude" },
    ...
    await expectRemovalWarning(page, ["codius-loop"]);
    await expectSkillSelectionOpen(page);
    await expectSkillPresentIn(skills, "claude", "codius-loop");
    await expectSavedSkillSelection(skills, { mode: "all" });
  });
  ...
  test("warns for a skill on disk that the saved selection never mentioned", async ({
    ...
    // The committed preference excludes codius-loop, but the directory is back —
    ...
    await openSkillSelection(page);
    await toggleSkill(page, "codius-advisor");
    await saveSkillSelection(page);

    await expectRemovalWarning(page, ["codius-loop"]);
    await expectSkillSelectionOpen(page);
    await expectSkillFilePresent(skills, "claude", "codius-loop", "notes/mine.md");
    await expectSavedSkillSelection(skills, { mode: "custom", skills: ["codius"] });
  });
  ...
  test("warns about a directory that appears after the sheet was opened", async ({
    ...
    await skills.placeSkillOnDisk("codius-loop", "claude");
    await toggleSkill(page, "codius-advisor");
    await saveSkillSelection(page);
  ...
  test("keeps draft choices when fresher status arrives while the sheet is open", async ({
    ...
    await skills.commitSelectionExternally({ mode: "custom", skills: ["codius", "codius-loop"] }, [
      "codius-advisor",
    ]);
    await saveSkillSelection(page);
    await expectRemovalWarning(page, ["codius-loop"]);

    await expectSkillChoices(page, ["codius", "codius-advisor"]);
  });
