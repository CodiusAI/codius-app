...
export function resolveDevRuntime(env = process.env) {
  const remoteDebuggingPort = env.CODIUS_ELECTRON_REMOTE_DEBUGGING_PORT
    ? parsePort(env.CODIUS_ELECTRON_REMOTE_DEBUGGING_PORT, "CODIUS_ELECTRON_REMOTE_DEBUGGING_PORT")
    : 0;
  return {
    electronFlags: buildElectronFlags(env.CODIUS_ELECTRON_FLAGS, remoteDebuggingPort),
    userDataDir: resolveDevUserDataDir({
      devRoot: env.CODIUS_DEV_ROOT,
      inheritedUserDataDir: env.CODIUS_ELECTRON_USER_DATA_DIR,
      fallbackRoot: env.CODIUS_DEV_RUNTIME_FALLBACK_ROOT,
    }),
  };
}
