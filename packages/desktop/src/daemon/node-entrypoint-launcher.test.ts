...
describe("node-entrypoint-launcher", () => {
  describe("createNodeEntrypointInvocation", () => {
    it("uses the packaged runner when the desktop app is packaged", () => {
      expect(
        createNodeEntrypointInvocation({
          execPath: "/Applications/Codius.app/Contents/MacOS/Codius",
          isPackaged: true,
          packagedRunnerPath:
            "/Applications/Codius.app/Contents/Resources/app.asar/dist/daemon/node-entrypoint-runner.js",
        ...
        command: "/Applications/Codius.app/Contents/MacOS/Codius",
        args: [
    ...
    it("uses the entrypoint directly in development", () => {
      expect(
        createNodeEntrypointInvocation({
          ...
          argvMode: "node-script",
          args: ["ls"],
          baseEnv: { PATH: "/usr/bin", CODIUS_NODE_ENV: "production" },
        }),
      ).toEqual({
        command: "/opt/homebrew/bin/electron",
        args: ["--import", "tsx", "/tmp/codius-cli.js", "ls"],
        env: {
          PATH: "/usr/bin",
          ELECTRON_RUN_AS_NODE: "1",
