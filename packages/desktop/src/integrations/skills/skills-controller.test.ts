...
const BUNDLED_SKILLS = ["codius", "codius-advisor", "codius-loop"];

async function makeHarness(selectionStore?: SkillSelectionStore): Promise<Harness> {
  const root = await mkdtemp(path.join(os.tmpdir(), "codius-skills-controller-"));
    ...
    [targets.agentsDir, targets.claudeDir, targets.codexDir].map(async (dir) => {
      ...
      return [
        ...parentEntries.filter(
          (entry) => entry !== path.basename(dir) && !entry.startsWith(".codius-skills-recovered-"),
        ),
        ...rootEntries.filter((entry) => entry.startsWith(".codius-skills-transaction-")),
      ].sort();
    }),
  );
}
...
async function waitForTransactionDirectory(parent: string): Promise<void> {
  ...
  try {
    for await (const event of events) {
      if (event.filename?.startsWith(".codius-skills-transaction-")) return;
    }
  } finally {
    await events.return?.();
  }
...
describe("skills controller", () => {
  ...
  it("reports one snapshot with catalog, selection, status, and pending work", async () => {
    expect(await harness.controller.status()).toEqual({
      state: "not-installed",
      ops: [
        { kind: "add", name: "codius" },
        { kind: "add", name: "codius-advisor" },
        { kind: "add", name: "codius-loop" },
      ],
  ...
  it("installs every bundled skill while the selection is all", async () => {
    ...
    expect(await isInstalled(harness.targets, "codius-advisor")).toBe(true);
  });

  it("does not remove deselected directories during install", async () => {
    await harness.controller.save({ mode: "custom", skills: ["codius"] });
    await writeUserFile(harness.targets, "codius-loop", "notes/mine.md", "keep this");

    await harness.controller.install();

    expect(await readUserFile(harness.targets, "codius-loop", "notes/mine.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it("saves a custom selection, converges disk, and returns the refreshed snapshot", async () => {
    ...
    expect(snapshot).toEqual({
      ...
      ops: [],
      available: BUNDLED_SKILLS,
      installed: ["codius", "codius-loop"],
      selection: { mode: "custom", skills: ["codius", "codius-loop"] },
      confirmationRequired: null,
    });
    expect(await isInstalled(harness.targets, "codius")).toBe(true);
    expect(await isInstalled(harness.targets, "codius-loop")).toBe(true);
    expect(await isInstalled(harness.targets, "codius-advisor")).toBe(false);
  });
  ...
  it("removes a skill from disk when it is dropped from the selection", async () => {
    ...
    await harness.controller.save({
      mode: "custom",
      skills: ["codius"],
      confirmedRemovals: ["codius-advisor", "codius-loop"],
    });

    expect(await isInstalled(harness.targets, "codius")).toBe(true);
    expect(await isInstalled(harness.targets, "codius-advisor")).toBe(false);
    expect(await isInstalled(harness.targets, "codius-loop")).toBe(false);
  });
  ...
  it("keeps the saved selection after uninstall so a later install restores it", async () => {
    ...
    expect(afterUninstall).toEqual({
      ...
      available: BUNDLED_SKILLS,
      installed: [],
      selection: { mode: "custom", skills: ["codius"] },
    });
  ...
  it("treats an empty custom selection as uninstall while keeping the preference", async () => {
    ...
    expect(await isInstalled(harness.targets, "codius")).toBe(false);
  });
  ...
  it("saves no selection at all when the very first save fails", async () => {
    ...
    await expect(harness.controller.save({ mode: "custom", skills: ["codius"] })).rejects.toThrow();
    await rm(harness.targets.agentsDir, { force: true });
  ...
  it("restores deleted directories byte for byte when the selection cannot be committed", async () => {
    const store = createUnwritableSelectionStore({
      mode: "custom",
      skills: ["codius", "codius-loop"],
    });
    const readOnly = await makeHarness(store);
    await readOnly.controller.install();
    await writeUserFile(readOnly.targets, "codius-loop", "notes/mine.md", "hand written");

    // Deselects codius-loop and adds codius-advisor, then fails to commit.
    await expect(
      readOnly.controller.save({
        mode: "custom",
        skills: ["codius", "codius-advisor"],
        confirmedRemovals: ["codius-loop"],
      }),
    ...
    expect(await readUserFile(readOnly.targets, "codius-loop", "notes/mine.md")).toEqual([
      "hand written",
      "hand written",
      "hand written",
    ]);
  ...
  it("preserves files added by another writer before rollback", async () => {
    const selection: SkillSelection = { mode: "custom", skills: ["codius"] };
    const gated = createGatedUnwritableSelectionStore(selection);
    const readOnly = await makeHarness(gated.store);
    await readOnly.controller.install();
    await writeFile(path.join(readOnly.targets.sourceDir, "codius", "SKILL.md"), "codius-v2");

    const save = readOnly.controller.save(selection);
    await gated.persistenceStarted;
    await writeUserFile(readOnly.targets, "codius", "notes/concurrent.md", "keep this");
    gated.failPersistence();
    await expect(save).rejects.toThrow("selection store is read-only");

    expect(await readUserFile(readOnly.targets, "codius", "notes/concurrent.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it("does not automatically delete files preserved from a rolled-back add", async () => {
    const previous: SkillSelection = { mode: "custom", skills: ["codius"] };
    ...
    await gated.persistenceStarted;
    await writeUserFile(readOnly.targets, "codius-loop", "notes/concurrent.md", "keep this");
    gated.failPersistence();
    await expect(save).rejects.toThrow("selection store is read-only");

    await readOnly.controller.autoUpdate();
    expect(await readUserFile(readOnly.targets, "codius-loop", "notes/concurrent.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it("merges a deleted directory backup into files another writer recreated", async () => {
    ...
    const readOnly = await makeHarness(gated.store);
    await readOnly.controller.install();
    await writeUserFile(readOnly.targets, "codius-loop", "notes/before.md", "restore this");
    ...
    expect(await readUserFile(readOnly.targets, "codius-loop", "notes/before.md")).toEqual([
      "restore this",
      "restore this",
      "restore this",
    ]);
  ...
  it("atomically stages a deletion before another writer can recreate its path", async () => {
    ...
    const next: SkillSelection = { mode: "custom", skills: ["codius"] };
    await harness.controller.save(previous);

    const transaction = await beginSkillsTransaction(harness.targets, previous, next, [
      { kind: "delete", name: "codius-loop" },
    ]);

    expect(await isInstalled(harness.targets, "codius-loop")).toBe(false);
    await writeUserFile(harness.targets, "codius-loop", "notes/concurrent.md", "keep this");
    await transaction.rollback();
    expect(await readUserFile(harness.targets, "codius-loop", "notes/concurrent.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      ].map((root) => path.join(root, "codius-loop"));
  ...
  it("finishes rollback when an external deletion leaves no live or staged path", async () => {
    ...
    const codexStage = (await readdir(harness.targets.codexDir)).find((entry) =>
      entry.startsWith(".codius-skills-transaction-"),
    );
    expect(codexStage).toBeDefined();
    await rm(path.join(harness.targets.codexDir, codexStage!, "codius-loop"), {
      recursive: true,
      force: true,
    });
    ...
    expect(await readUserFile(harness.targets, "codius-loop", "SKILL.md")).toEqual([
      "codius-loop-v1",
      "codius-loop-v1",
      null,
    ]);
  ...
  it("quarantines a staged directory when an external file takes its live path", async () => {
    ...
    const live = path.join(harness.targets.codexDir, "codius-loop");
    ...
    const recovered = (await readdir(harness.targets.codexDir)).find((entry) =>
      entry.startsWith(".codius-skills-recovered-"),
    );
  ...
  it("quarantines staged files that collide with a recreated directory", async () => {
    ...
    const next: SkillSelection = { mode: "custom", skills: ["codius"] };
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "codius-loop", "notes/mine.md", "staged notes");
    ...
    const recovered = (await readdir(harness.targets.codexDir)).find((entry) =>
      entry.startsWith(".codius-skills-recovered-codius-loop-"),
    );
    expect(recovered).toBeDefined();
    expect(
  ...
  it("preserves incompatible live paths while rolling back adds and updates", async () => {
    ...
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "codius", "notes/mine.md", "keep this");

    const transaction = await beginSkillsTransaction(harness.targets, previous, next, [
      { kind: "update", name: "codius" },
      { kind: "add", name: "codius-advisor" },
    ]);
    const replacedUpdate = path.join(harness.targets.agentsDir, "codius");
    await rm(replacedUpdate, { recursive: true, force: true });
    await writeFile(replacedUpdate, "external update replacement");
    const replacedAdd = path.join(harness.targets.codexDir, "codius-advisor");
    ...
    const recovered = (await readdir(recoveryParent)).find((entry) =>
      entry.startsWith(".codius-skills-recovered-codius-"),
    );
    expect(recovered).toBeDefined();
    expect(await readFile(path.join(recoveryParent, recovered!, "notes", "mine.md"), "utf8")).toBe(
      "keep this",
    );
    expect(await readUserFile(harness.targets, "codius", "notes/mine.md")).toEqual([
      null,
      "keep this",
      "keep this",
    ]);
  ...
  it("recovers after an update backup was quarantined before transaction cleanup", async () => {
    ...
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "codius", "notes/mine.md", "captured notes");
    ...
    const entry = manifest.entries.find(
      (candidate) => candidate.livePath === path.join(harness.targets.agentsDir, "codius"),
    );
    expect(entry?.backupPath).toBeTruthy();
    const backup = path.join(transactionDir, entry!.backupPath!);
    const recovered = path.join(
      transactionParent,
      `.codius-skills-recovered-codius-${transactionName!.replace(".codius-skills-transaction-", "")}`,
    );
  ...
  it("preserves a directory that replaces a captured file before recovery", async () => {
    ...
    await harness.controller.save(previous);
    const live = path.join(harness.targets.agentsDir, "codius");
  ...
  it.skipIf(process.platform !== "linux")(
    ...
    async () => {
      const crossFilesystemRoot = await mkdtemp("/dev/shm/codius-skills-controller-");
      try {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const next: SkillSelection = { mode: "custom", skills: ["codius"] };
      await harness.controller.save(previous);
      const shared = path.join(harness.root, "home", "shared", "codius-loop");
      await mkdir(shared, { recursive: true });
      await writeFile(path.join(shared, "SKILL.md"), "shared target");
      const live = path.join(harness.targets.claudeDir, "codius-loop");
      await rm(live, { recursive: true, force: true });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const readOnly = await makeHarness(gated.store);
      await readOnly.controller.install();
      const shared = path.join(readOnly.root, "home", "shared", "codius");
      await mkdir(shared, { recursive: true });
      await writeFile(path.join(shared, "SKILL.md"), "codius-v1");
      const live = path.join(readOnly.targets.claudeDir, "codius");
      await rm(live, { recursive: true, force: true });
      ...
      expect((await lstat(live)).isSymbolicLink()).toBe(true);
      expect(await readFile(path.join(shared, "SKILL.md"), "utf8")).toBe("codius-v1");
      await rm(readOnly.root, { recursive: true, force: true });
    },
  );
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      for (const root of [
        ...
        const notes = path.join(root, "codius-loop", "notes");
      ...
      for (const root of [
        ...
        const restored = path.join(root, "codius-loop", "notes", "latest.md");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const readOnly = await makeHarness(gated.store);
      await readOnly.controller.install();
      await writeUserFile(readOnly.targets, "codius-loop", "hooks/run.sh", "#!/bin/sh\n");
      for (const root of [
        readOnly.targets.agentsDir,
        readOnly.targets.claudeDir,
        readOnly.targets.codexDir,
      ]) {
        await chmod(path.join(root, "codius-loop", "hooks", "run.sh"), 0o751);
      }
      ...
      for (const root of [
        ...
        const restored = await lstat(path.join(root, "codius-loop", "hooks", "run.sh"));
  ...
  it("does not delete an unrelated file that resembles transaction staging", async () => {
    const unrelated = path.join(
      ...
      ".codius-skills-transaction-my-notes",
    );
  ...
  it("recovers an interrupted save before the next controller operation", async () => {
    ...
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "codius-loop", "notes/mine.md", "hand written");
    await beginSkillsTransaction(harness.targets, previous, next, [
      { kind: "delete", name: "codius-loop" },
    ]);
    for (const root of [
      ...
      await rm(path.join(root, "codius-loop"), { recursive: true, force: true });
    }
  ...
  it("asks for confirmation naming the directories a save would delete", async () => {
    await harness.controller.save({ mode: "custom", skills: ["codius", "codius-loop"] });
    // Something puts a managed directory back after the UI took its snapshot.
    await writeUserFile(harness.targets, "codius-advisor", "SKILL.md", "external");
    ...
    expect(result.confirmationRequired).toEqual({ removals: ["codius-advisor"] });
    expect(await installedEverywhere(harness.targets)).toEqual([
      ["codius", "codius-advisor", "codius-loop"],
      ["codius", "codius-advisor", "codius-loop"],
      ["codius", "codius-advisor", "codius-loop"],
    ]);
    expect(result.selection).toEqual({ mode: "custom", skills: ["codius", "codius-loop"] });
  });
  ...
  it("applies the save once the removals are confirmed", async () => {
    ...
    expect(result.confirmationRequired).toBeNull();
    expect(result.selection).toEqual({ mode: "custom", skills: ["codius"] });
    expect(await installedEverywhere(harness.targets)).toEqual([
      ["codius"],
      ["codius"],
      ["codius"],
  ...
  it("asks again when another directory appears before the retry", async () => {
    await harness.controller.install();
    await writeUserFile(harness.targets, "codius-chat", "SKILL.md", "retired but present");
    ...
    expect(result.confirmationRequired).toEqual({
      removals: ["codius-advisor", "codius-chat", "codius-loop"],
    });
    expect(await installedEverywhere(harness.targets)).toEqual([
      ["codius", "codius-advisor", "codius-chat", "codius-loop"],
      ["codius", "codius-advisor", "codius-chat", "codius-loop"],
      ["codius", "codius-advisor", "codius-chat", "codius-loop"],
    ]);
  });
  ...
  it("does not commit when a new removal appears while the frozen plan is applying", async () => {
    ...
    await writeFile(path.join(harness.targets.sourceDir, "codius", "SKILL.md"), "codius-v2");
    ...
    await writeUserFile(harness.targets, "codius-chat", "notes/mine.md", "hand written");

    const result = await save;

    expect(result.confirmationRequired).toEqual({ removals: ["codius-chat"] });
    expect(result.selection).toEqual(selection);
    expect(await readUserFile(harness.targets, "codius-chat", "notes/mine.md")).toEqual([
      "hand written",
      "hand written",
      "hand written",
    ]);
  ...
  it("saves without asking when nothing would be deleted", async () => {
    const result = await harness.controller.save({ mode: "custom", skills: ["codius"] });
  ...
  it("preserves a regular file at a skill path when save convergence fails", async () => {
    ...
    const collision = path.join(harness.targets.agentsDir, "codius");
  ...
  it("serializes startup convergence with an interactive save", async () => {
    ...
    await rm(path.join(harness.targets.claudeDir, "codius-loop"), {
      recursive: true,
      force: true,
    });
    ...
    expect(saved.selection).toEqual({ mode: "custom", skills: ["codius"] });
    expect(await installedEverywhere(harness.targets)).toEqual([
      ["codius"],
      ["codius"],
      ["codius"],
  ...
  it("updates a drifted install without touching the saved selection", async () => {
    await harness.controller.save({ mode: "custom", skills: ["codius"] });
    await writeFile(path.join(harness.targets.agentsDir, "codius", "SKILL.md"), "stale");
