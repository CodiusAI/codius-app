...
async function makeSandbox(): Promise<Sandbox> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "codius-skill-sync-"));
...
describe("syncSkills", () => {
  ...
  it("overwrites on-disk skill content when the bundle differs", async () => {
    await writeBundleSkill(sandbox.sourceDir, "codius", {
      "SKILL.md": "new codius content",
    });
    const onDiskSkill = path.join(sandbox.agentsDir, "codius");
    await fs.mkdir(onDiskSkill, { recursive: true });
    await fs.writeFile(path.join(onDiskSkill, "SKILL.md"), "old codius content");
    ...
    const agentsContent = await fs.readFile(
      path.join(sandbox.agentsDir, "codius", "SKILL.md"),
      "utf-8",
    );
    expect(agentsContent).toBe("new codius content");
    const claudeContent = await fs.readFile(
      path.join(sandbox.claudeDir, "codius", "SKILL.md"),
      "utf-8",
    );
    expect(claudeContent).toBe("new codius content");
    const codexContent = await fs.readFile(
      path.join(sandbox.codexDir, "codius", "SKILL.md"),
      "utf-8",
    );
    expect(codexContent).toBe("new codius content");
  });

  it("installs new bundled skills, including references/, when not present on disk", async () => {
    await writeBundleSkill(sandbox.sourceDir, "codius-committee", {
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["codius-committee"],
    });

    expect(
      await fs.readFile(path.join(sandbox.agentsDir, "codius-committee", "SKILL.md"), "utf-8"),
    ).toBe("committee content");
    expect(
      await fs.readFile(
        path.join(sandbox.agentsDir, "codius-committee", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");
    expect(
      await fs.readFile(
        path.join(sandbox.codexDir, "codius-committee", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");

    const claudeSkillDir = path.join(sandbox.claudeDir, "codius-committee");
  ...
  it("leaves on-disk skills not in the bundle untouched", async () => {
    await writeBundleSkill(sandbox.sourceDir, "codius", { "SKILL.md": "new" });
  ...
  it("removes stale files previously written to managed skill dirs", async () => {
    ...
    await fs.rm(path.join(sandbox.sourceDir, "codius"), { recursive: true, force: true });
    await writeBundleSkill(sandbox.sourceDir, "codius", { "SKILL.md": "new" });
  ...
  it("skips skills listed in skillNames that are missing from the bundle without raising", async () => {
    await writeBundleSkill(sandbox.sourceDir, "codius", { "SKILL.md": "content" });
    // "codius-removed" is in skillNames but not in the bundle on disk.
    ...
    const result = await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["codius", "codius-removed"],
      onSkillError: (name, error) => errors.push({ name, error }),
    });

    expect(errors).toEqual([]);
    expect(result.processedSkills).toBe(1);
    expect(await fs.readFile(path.join(sandbox.agentsDir, "codius", "SKILL.md"), "utf-8")).toBe(
      "content",
    );
    await expect(fs.access(path.join(sandbox.agentsDir, "codius-removed"))).rejects.toThrow();
  });

  it("leaves on-disk skill content alone when the skill has been removed from the bundle", async () => {
    await writeBundleSkill(sandbox.sourceDir, "codius", { "SKILL.md": "current" });
    const deprecatedDir = path.join(sandbox.agentsDir, "codius-deprecated");
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["codius", "codius-deprecated"],
    });
  ...
  it("reports per-skill errors via onSkillError without throwing", async () => {
    ...
    await fs.writeFile(path.join(sandbox.agentsDir, "codius"), "blocking file");
    ...
    expect(errors).toContain("codius");
...
describe("removeSkill", () => {
  ...
  it("removes the skill from all three targets when present", async () => {
    ...
    await removeSkill("codius", {
      agentsDir: sandbox.agentsDir,
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
    });

    await expect(fs.access(path.join(sandbox.agentsDir, "codius"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.claudeDir, "codius"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.codexDir, "codius"))).rejects.toThrow();
  });
