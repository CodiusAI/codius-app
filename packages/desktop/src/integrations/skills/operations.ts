...
/** Every name Codius owns on disk: what it ships now plus what it used to ship. */
...
function hasInstalledCodiusSkill(disks: readonly TargetSkills[]): boolean {
...
export async function getSkillsStatus(
  ...
  if (!hasInstalledCodiusSkill(disks)) return { state: "not-installed", ops, available, installed };
