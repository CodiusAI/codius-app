...
export function registerNotificationHandlers(): void {
  ipcMain.handle("codius:notification:isSupported", () => {
    return Notification.isSupported();
  });

  ipcMain.handle("codius:notification:send", async (event, rawInput?: NotificationInput) => {
    ...
    notification.on("click", () => {
      ...
      if (win && data && Object.keys(data).length > 0) {
        ...
        win.webContents.send("codius:event:notification-click", payload);
      }
