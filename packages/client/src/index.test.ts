...
import { createCodiusClient } from "./index.js";
import type { CodiusAgent, CodiusClient, CodiusProviderConfig, CodiusWorkspace } from "./index.js";
...
async function connectClient(): Promise<{ client: CodiusClient; ws: FakeWebSocket }> {
  vi.stubGlobal("WebSocket", FakeWebSocket);
  const client = createCodiusClient({
    url: "ws://daemon.test",
  ...
  expect(hello.clientId).toEqual(expect.stringMatching(/^codius-sdk-/));
  ws.message(
    sessionMessage({
      type: "status",
      payload: {
...
function createWorkspace(input: Partial<CodiusWorkspace> = {}): CodiusWorkspace {
  return {
    id: "workspace_sdk",
...
function createAgent(input: Partial<CodiusAgent> = {}): CodiusAgent {
  return {
    id: "agent_sdk",
    provider: "codex",
    cwd: "/repo/sdk",
...
test("createCodiusClient exposes workspace list through the daemon client", async () => {
...
test("provider config builders shape existing create-agent config fields", async () => {
  ...
  const expectedProviderConfig = {
    ...
    thinkingOptionId: "high",
    featureValues: { webSearch: true },
  } satisfies CodiusProviderConfig;
