...
test("does not infer browser automation capabilities from Electron runtime", async () => {
  vi.stubGlobal("navigator", {
    userAgent:
      "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) Codius/0.1.89 Chrome/146 Electron/41.2.0 Safari/537.36",
  });
...
test("passes password as HTTP bearer header and WebSocket subprotocol", async () => {
  ...
  expect(transportFactory).toHaveBeenCalledWith({
    ...
    protocols: ["codius.bearer.shared-secret"],
  });
});
    ...
    wrapSessionMessage({
      ...
      payload: {
        ...
        file: {
          ...
          mimeType: "text/plain",
          size: 11,
          path: "/tmp/codius-uploads/upload_req-upload/notes.txt",
        },
        error: null,
      },
    }),
  ...
  mock.triggerMessage(
    wrapSessionMessage({
      ...
      payload: {
        ...
        detail: {
          type: "worktree_setup",
          worktreePath: "/tmp/project/.codius/worktrees/feature-a",
          ...
          commands: [
            {
              index: 1,
              command: "npm install",
              cwd: "/tmp/project/.codius/worktrees/feature-a",
              log: "phase-one\n",
              status: "running",
              exitCode: null,
            },
...
test("sends structured first-agent context attachments with create_codius_worktree_request", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const createPromise = client.createCodiusWorktree({
    cwd: "/tmp/project",
    worktreeSlug: "review-pr-123",
    firstAgentContext: {
      attachments: [
...
test("sends worktree base-ref fields in create_codius_worktree_request", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
...
test("omitting create_codius_worktree_request worktree base-ref fields preserves legacy wire shape", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  mock.triggerMessage(
    JSON.stringify({
      ...
      message: {
        ...
        payload: {
          directories: ["/Users/test/projects/codius"],
