...
describe("Hub device authorization", () => {
  it("opens activation URLs on Windows without a command shell", async () => {
    ...
    await browser.open("https://cloud.codius.test/activate?code=ABCD-EFGH-JKLMN");
    ...
    assert.deepEqual(launches, [
      {
        ...
        args: [
          ...
          "https://cloud.codius.test/activate?code=ABCD-EFGH-JKLMN",
        ],
      },
    ]);
  });
  ...
  it("opens the browser, follows Cloud cadence, and returns the approved enrollment token", async () => {
    ...
    const token = await authorization.approve("https://cloud.codius.test", "Studio Mac");

    assert.equal(token, "approved-enrollment-token-1234567890");
    assert.deepEqual(authorization.observed(), {
      starts: [{ hubUrl: "https://cloud.codius.test", displayName: "Studio Mac" }],
      polls: [
        {
          hubUrl: "https://cloud.codius.test",
      ...
      opened: ["https://cloud.codius.test/activate?code=ABCD-EFGH-JKLMN"],
      instructions: ["https://cloud.codius.test/activate ABCD-EFGH-JKLMN"],
    });
  });
  ...
  it("stops without an enrollment token when the browser denies the request", async () => {
    ...
    await assert.rejects(authorization.approve("https://cloud.codius.test", "Studio Mac"), {
  ...
  it("connects the real hub command with a browser-approved token", async () => {
    ...
    await createHubCommand({
      ...
      authorize: async (url, displayName) => {
        assert.equal(url, "https://cloud.codius.test");
        assert.equal(displayName, "Studio Mac");
        return "approved-enrollment-token-1234567890";
      },
      displayName: () => "Studio Mac",
    }).parseAsync(["node", "codius hub", "connect", "https://cloud.codius.test", "--json"], {
      from: "node",
    });

    assert.deepEqual(daemon.connections, [
      {
        url: "https://cloud.codius.test",
...
class FakeCloud implements CloudDeviceAuthorization {
  ...
  async start(hubUrl: string, displayName: string) {
    ...
    return {
      ...
      verificationUri: "https://cloud.codius.test/activate",
      verificationUriComplete: "https://cloud.codius.test/activate?code=ABCD-EFGH-JKLMN",
...
function hubStatus(state: string) {
  return {
    ...
    hubOrigin: state === "connected" ? "https://cloud.codius.test" : null,
