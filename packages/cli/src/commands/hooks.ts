...
interface HookEnvironment {
  ...
  CODIUS_ACTIVITY_TOKEN?: string;
  CODIUS_TERMINAL_ACTIVITY_URL?: string;
}
...
function resolveTarget(env: HookEnvironment) {
  const terminalId = env.CODIUS_TERMINAL_ID;
  const token = env.CODIUS_ACTIVITY_TOKEN;
  const url = env.CODIUS_TERMINAL_ACTIVITY_URL;
