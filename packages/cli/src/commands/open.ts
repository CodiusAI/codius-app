...
function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/Codius.app",
      path.join(homedir(), "Applications", "Codius.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Codius",
      "/opt/Codius/Codius",
      path.join(homedir(), "Applications", "Codius.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Codius", "Codius.exe");
