...
function describeDaemonAuthProbeFailure(host: string, failure: DaemonAuthProbeFailure): string {
  if (failure === "auth_required") {
    return `Daemon is reachable at ${host} but requires a password. Set CODIUS_PASSWORD and retry.`;
  }
  return `Daemon is reachable at ${host} but the supplied password was rejected. Check CODIUS_PASSWORD and retry.`;
}
