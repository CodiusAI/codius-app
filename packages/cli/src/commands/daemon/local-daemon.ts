...
const DAEMON_PID_FILENAME = "codius.pid";
...
function envWithHome(home?: string): NodeJS.ProcessEnv {
  ...
  return { ...process.env, CODIUS_HOME: home };
}
...
function buildChildEnv(options: DaemonStartOptions): NodeJS.ProcessEnv {
  ...
  if (options.home) {
    childEnv.CODIUS_HOME = options.home;
  }
  if (options.listen) {
    childEnv.CODIUS_LISTEN = options.listen;
  } else if (options.port) {
    childEnv.CODIUS_LISTEN = `127.0.0.1:${options.port}`;
  }
  if (options.hostnames) {
    childEnv.CODIUS_HOSTNAMES = options.hostnames;
  }
  if (options.relayUseTls === true) {
    childEnv.CODIUS_RELAY_USE_TLS = "true";
  }
  if (options.webUi === true) {
    childEnv.CODIUS_WEB_UI_ENABLED = "true";
  }
  if (options.webUi === false) {
    childEnv.CODIUS_WEB_UI_ENABLED = "false";
  }
  return childEnv;
}
...
function pidFilePath(codiusHome: string): string {
  return path.join(codiusHome, DAEMON_PID_FILENAME);
}
...
export function resolveLocalCodiusHome(home?: string): string {
  return resolveCodiusHome(envWithHome(home));
}
...
export function resolveLocalDaemonState(options: { home?: string } = {}): LocalDaemonState {
  const env: NodeJS.ProcessEnv = {
    ...
    CODIUS_LISTEN: undefined,
    CODIUS_HOSTNAMES: undefined,
    CODIUS_ALLOWED_HOSTS: undefined,
    CODIUS_RELAY_ENABLED: undefined,
    CODIUS_RELAY_ENDPOINT: undefined,
    CODIUS_RELAY_PUBLIC_ENDPOINT: undefined,
    CODIUS_RELAY_USE_TLS: undefined,
    CODIUS_RELAY_PUBLIC_USE_TLS: undefined,
  };
  const home = resolveCodiusHome(env);
  ...
  return {
    ...
    relayEndpoint: config.relayPublicEndpoint ?? config.relayEndpoint ?? "relay.codius.ai:443",
...
export function tailDaemonLog(home?: string, lines = 30): string | null {
  const logPath = path.join(resolveLocalCodiusHome(home), DAEMON_LOG_FILENAME);
...
export async function startLocalDaemonDetached(
  ...
  const codiusHome = runtime.resolveHome(childEnv);
  const logPath = path.join(codiusHome, DAEMON_LOG_FILENAME);
