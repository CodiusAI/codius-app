...
class FakeDaemonRuntime implements DaemonLaunchRuntime {
  ...
  resolveHome(env: NodeJS.ProcessEnv): string {
    return env.CODIUS_HOME ?? "/tmp/codius";
  }
...
async function createCodiusHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "codius-local-daemon-"));
  tempRoots.push(root);
...
describe("local daemon launch supervision", () => {
  ...
  test("foreground start spawns supervisor-entrypoint instead of server/index", async () => {
    ...
    const status = startLocalDaemonForeground({ home: "/tmp/codius-test", relay: false }, runtime);
  ...
  test("detached start spawns supervisor-entrypoint instead of server/index", async () => {
    ...
    const resultPromise = startLocalDaemonDetached(
      { home: "/tmp/codius-test", mcp: false },
      runtime,
    );
    await vi.advanceTimersByTimeAsync(1200);
    const result = await resultPromise;

    expect(result).toEqual({ pid: 4242, logPath: "/tmp/codius-test/daemon.log" });
  ...
  test("relay TLS flag is passed to the supervised daemon", async () => {
    ...
    expect(launch?.options?.env?.CODIUS_RELAY_USE_TLS).toBe("true");
  });
  ...
  test("web UI flag is passed to the supervised daemon", async () => {
    ...
    expect(launch?.options?.env?.CODIUS_WEB_UI_ENABLED).toBe("true");
  });
  ...
  test("no-web UI flag is passed to the supervised daemon", async () => {
    ...
    expect(launch?.options?.env?.CODIUS_WEB_UI_ENABLED).toBe("false");
  });
  ...
  test("local daemon state keeps public relay TLS separate from daemon relay TLS", async () => {
    const home = await createCodiusHome({
      ...
      daemon: {
        relay: {
          ...
          publicEndpoint: "codius.example.com",
          useTls: false,
          publicUseTls: true,
        },
      },
    ...
    expect(state.relayEndpoint).toBe("codius.example.com");
