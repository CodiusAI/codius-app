...
const RELAY_DOCS_URL = "https://codius.ai/docs/security";
...
export async function resolveLocalPairingOffer(options: {
  ...
  const state = resolveLocalDaemonState({ home: options.codiusHome });
  ...
  const config = loadConfig(options.codiusHome);
...
async function resolveDaemonPairingOffer(
  ...
  try {
    ...
    if (serverInfo?.serverId.trim() !== expectedServerId) {
      throw new Error(
        "The reachable daemon belongs to a different Codius home. Check --home or the daemon listen configuration.",
      );
    }
    if (serverInfo?.features?.daemonStatusRpc !== true) {
      throw new Error("Update the Codius daemon before pairing from this command.");
    }
    ...
    if (!offer.relayEnabled && enableRelay) {
      if (serverInfo.features.relayConfig !== true) {
        throw new Error("Update the Codius daemon before enabling relay from this command.");
      }
...
export async function confirmRelayPairing(): Promise<boolean> {
  log.message("Your connection is end-to-end encrypted. Codius cannot read your code or messages.");
...
export async function runPairCommand(
  ...
  if (options.home) process.env.CODIUS_HOME = options.home;
  ...
  if (!pairing.relayEnabled && canPrompt) {
    ...
    pairing = await dependencies.resolveOffer({ codiusHome, enableRelay: true });
    ...
    if (options.json) {
      output.writeStderr(
        `${JSON.stringify({
          ...
          action: "Run codius daemon pair --relay --json to enable it explicitly.",
        })}\n`,
      );
    } else {
      output.writeStderr(`${chalk.red("Relay pairing is disabled for this daemon.")}\n`);
      output.writeStderr(`${chalk.yellow("Run codius daemon pair --relay to enable it.")}\n`);
    }
