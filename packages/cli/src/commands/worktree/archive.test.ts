...
function createFakeDaemonClient(
  overrides: Partial<
    Pick<DaemonClient, "getCodiusWorktreeList" | "archiveCodiusWorktree" | "close">
  > = {},
): DaemonClient {
  return {
    getCodiusWorktreeList: async () => ({
      worktrees: [],
      error: null,
      requestId: "req-list",
    }),
    archiveCodiusWorktree: async () => ({
      success: true,
      removedAgents: [],
      error: null,
...
describe("runArchiveCommand", () => {
  it("sends scope worktree when archiving by worktree path", async () => {
    const worktreePath = "/tmp/codius-home/worktrees/repo/feature";
    const archiveCalls: Array<{
      input: Parameters<DaemonClient["archiveCodiusWorktree"]>[0];
    }> = [];
    const fakeClient = createFakeDaemonClient({
      ...
      archiveCodiusWorktree: async (input) => {
  ...
  it("archives by matching branch name when no directory name matches", async () => {
    const worktreePath = "/tmp/codius-home/worktrees/repo/feature-branch";
    const archiveCalls: Array<{
      input: Parameters<DaemonClient["archiveCodiusWorktree"]>[0];
    }> = [];
    const fakeClient = createFakeDaemonClient({
