...
export async function runArchiveCommandWithDeps(
  ...
  try {
    ...
    const listResponse = await client.getCodiusWorktreeList({});
    ...
    // the directory and then removes the directory (Codius-owned gated).
    const response = await client.archiveCodiusWorktree({
