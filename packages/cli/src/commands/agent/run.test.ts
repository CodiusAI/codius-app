...
describe("managed agent caller context", () => {
  it("propagates a trimmed CODIUS_AGENT_ID", () => {
    expect(resolveRunCallerAgentId({ CODIUS_AGENT_ID: "  parent-agent  " })).toBe("parent-agent");
  });

  it("omits blank caller ids", () => {
    expect(resolveRunCallerAgentId({ CODIUS_AGENT_ID: "   " })).toBeUndefined();
  });
});
...
describe("runRunCommand option validation", () => {
  const originalWorkspaceId = process.env.CODIUS_WORKSPACE_ID;

  beforeEach(() => {
    delete process.env.CODIUS_WORKSPACE_ID;
  });
  ...
  afterEach(() => {
    if (originalWorkspaceId === undefined) {
      ...
      process.env.CODIUS_WORKSPACE_ID = originalWorkspaceId;
    }
  });
