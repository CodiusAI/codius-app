 ...
 * - `codiusctl agent ls`    → active non-archived agents
 * - `codiusctl agent ls -g` → global non-archived agents
 * - `codiusctl agent ls -a` → active agents, including archived
 * - `codiusctl agent ls -ag` → global agents, including archived
 */
export async function runLsCommand(
  ...
  try {
    ...
    const error: CommandError = {
      ...
      details:
        "Start the daemon with: codiusctl daemon start\nFor a remote daemon, pass --host <host:port> or set CODIUS_HOST.",
    };
    throw error;
  }
