...
try {
  // Test 1: inspect --help shows options
  {
    ...
    const result = await $`npx codius inspect --help`.nothrow();
  ...
  // Test 2: inspect requires id argument
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius inspect`.nothrow();
  ...
  // Test 3: inspect handles daemon not running
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius inspect abc123`.nothrow();
  ...
  // Test 4: inspect --host flag is accepted
  {
    ...
    const result =
      await $`CODIUS_HOME=${codiusHome} npx codius inspect --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: -q (quiet) flag is accepted with inspect
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius -q inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: --json flag is accepted with inspect
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius inspect abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --format yaml flag is accepted with inspect
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius --format yaml inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: codius --help shows inspect command
  {
    console.log("Test 8: codius --help shows inspect command");
    ...
    console.log("codius --help shows inspect command\n");
  }
