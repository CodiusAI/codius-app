...
async function readPidLockState(codiusHome: string): Promise<PidLockState> {
...
async function readCapturedSupervisorLogs(codiusHome: string, recentLogs: string): Promise<string> {
  const durableLogs = await readFile(join(codiusHome, "daemon.log"), "utf8").catch(() => "");
  return `${recentLogs}\n${durableLogs}`;
}
...
const codiusHome = await mkdtemp(join(tmpdir(), "codius-stop-supervisor-"));
...
try {
  console.log("Test 1: start supervisor-entrypoint in dev mode with isolated CODIUS_HOME");
  ...
  await waitFor(
    async () => {
      const status = await readDaemonStatus(codiusHome);
      return (
  ...
  const statusBeforeStop = await readDaemonStatus(codiusHome);
  ...
  const pidLockBeforeStop = await readPidLockState(codiusHome);
  ...
  const pidAfterStop = await readPidLockState(codiusHome);
  ...
  const statusAfterStop = await readDaemonStatus(codiusHome);
  assert.strictEqual(
    statusAfterStop.localDaemon,
    "stopped",
    "daemon should remain stopped after stop command",
  );
  const capturedSupervisorLogs = await readCapturedSupervisorLogs(codiusHome, recentSupervisorLogs);
  assert(
