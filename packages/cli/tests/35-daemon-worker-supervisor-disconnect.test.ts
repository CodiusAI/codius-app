...
const codiusHome = await mkdtemp(join(tmpdir(), "codius-worker-supervisor-disconnect-"));
...
try {
  console.log("Test 1: start supervised daemon with isolated CODIUS_HOME");
  ...
  const statusBeforeKill = await readDaemonStatus(codiusHome);
