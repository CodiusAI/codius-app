...
const root = await mkdtemp(join(tmpdir(), "codius-set-password-"));
const codiusHome = join(root, ".codius");
...
try {
  {
      ...
      `${JSON.stringify(
        {
          ...
          app: { baseUrl: "https://app.codius.ai" },
        },
        null,
        2,
      )}\n`,
    ...
    const result = await setDaemonPasswordInConfig("shared-secret", { home: codiusHome });
