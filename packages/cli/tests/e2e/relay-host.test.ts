...
(shouldRunRelayE2e ? describe : describe.skip)("CLI --host offer URL via relay", () => {
  ...
  beforeAll(async () => {
    ...
    ctx = await createE2ETestContext({
      timeout: 60_000,
      env: {
        CODIUS_RELAY_ENABLED: "true",
        CODIUS_RELAY_ENDPOINT: relayEndpoint,
        CODIUS_RELAY_PUBLIC_ENDPOINT: relayEndpoint,
      },
    });

    const offer = await generateLocalPairingOffer({
      codiusHome: ctx.codiusHome,
      relayEnabled: true,
      relayEndpoint,
  ...
  it("runs `codius --host <offer-url> ls` over the relay and matches direct ls output", async () => {
    ...
    const relay = await ctx.codius(["ls", "--json", "--host", offerUrl], {
      timeout: 30_000,
      env: { CODIUS_HOST: offerUrl },
    });
