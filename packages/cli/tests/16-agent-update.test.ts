...
try {
  ...
  // Test 7: agent update accepts thinking as an update field
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius agent update abc123 --thinking high`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: thinking cannot be combined with metadata updates
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius agent update abc123 --name renamed --thinking high`.nothrow();
