...
try {
...

  {
    ...
    const posted = await ctx.codius(["chat", "post", "coord-room", "first message for @agent-1"], {
      env: { CODIUS_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
    });
    ...
    const read = await ctx.codius(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.codius(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    // always wakes it up.
    const waitPromise = ctx.codius(["chat", "wait", "coord-room", "--timeout", "30s"]);
    const waitSentinel = waitPromise.then(() => "settled" as const);
    let postedSecond = false;
    for (let attempt = 0; attempt < 60; attempt++) {
      const post = await ctx.codius(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.codius(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.codius(["chat", "delete", "coord-room"]);
  ...
  await ctx.stop();
  await rm(ctx.codiusHome, { recursive: true, force: true });
