...
/**
 * Test runner for Codius CLI E2E tests
 *
 ...
 * its own daemon (ephemeral port + tmp CODIUS_HOME), so parallelism is safe.
 */
...
const concurrencyEnv = process.env.CODIUS_CLI_TEST_CONCURRENCY;
...
const shardTotal = parsePositiveInt(process.env.CODIUS_CLI_TEST_SHARD_TOTAL, 1);
const shardIndexRaw = parsePositiveInt(process.env.CODIUS_CLI_TEST_SHARD, 1);
if (shardIndexRaw < 1 || shardIndexRaw > shardTotal) {
  throw new Error(
    `CODIUS_CLI_TEST_SHARD=${shardIndexRaw} out of range for SHARD_TOTAL=${shardTotal}`,
  );
}
...
console.log("🧪 Codius CLI E2E Test Runner\n");
console.log("=".repeat(50));
...
async function runSingleTest(testFile: string): Promise<TestOutcome> {
  ...
  const npmCache = await mkdtemp(join(tmpdir(), "codius-cli-test-npm-cache-"));
    ...
    return await new Promise<TestOutcome>((resolve) => {
      const proc = spawn("npx", ["tsx", testPath], {
        env: {
          ...
          CODIUS_LOCAL_SPEECH_AUTO_DOWNLOAD: testEnvDefaults.CODIUS_LOCAL_SPEECH_AUTO_DOWNLOAD,
          CODIUS_DICTATION_ENABLED: testEnvDefaults.CODIUS_DICTATION_ENABLED,
          CODIUS_VOICE_MODE_ENABLED: testEnvDefaults.CODIUS_VOICE_MODE_ENABLED,
        },
