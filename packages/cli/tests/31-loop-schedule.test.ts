...
async function waitForLoopInList(
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    const listed = await ctx.codius(["loop", "ls", "--json"]);
...
try {
  {
    ...
    const created = await ctx.codius(
      [
        "schedule",
        "create",
        "Review new PRs",
    ...
    const listed = await ctx.codius(["schedule", "ls", "--json"]);
    assert.strictEqual(listed.exitCode, 0, listed.stderr);
    const listedJson = JSON.parse(listed.stdout);
    assert(Array.isArray(listedJson), listed.stdout);
    assert(
    ...
    const inspected = await ctx.codius(["schedule", "inspect", createdJson.id, "--json"]);
...

  {
    ...
    const logs = await ctx.codius(["loop", "logs", runJson.id], { timeout: 15000 });
