 ...
 * Test Daemon Helper
 *
 * Provides utilities for launching real Codius daemons in E2E tests.
 * Each test gets an isolated daemon on an available local port with its own CODIUS_HOME.
 *
...
const TEST_OUTPUT_CAPTURE_LIMIT = Number.parseInt(
  process.env.CODIUS_TEST_OUTPUT_CAPTURE_BYTES ?? `${DEFAULT_OUTPUT_CAPTURE_LIMIT}`,
  10,
);
...
export async function createTempDirs(): Promise<{ codiusHome: string; workDir: string }> {
  const codiusHome = await mkdtemp(join(tmpdir(), "codius-e2e-home-"));
  const workDir = await mkdtemp(join(tmpdir(), "codius-e2e-work-"));

  // Create the agents directory that the daemon expects
  const agentsDir = join(codiusHome, "agents");
...
async function probeDaemonReady(port: number, env?: NodeJS.ProcessEnv): Promise<boolean> {
  try {
    const { exitCode } = await runCodiusCli(
      {
        port,
 ...
 * with isolated CODIUS_HOME and CODIUS_LISTEN environment variables.
 */
export async function startTestDaemon(options?: {
  ...
  const { codiusHome, workDir } =
    options?.codiusHome && options?.workDir
      ? { codiusHome: options.codiusHome, workDir: options.workDir }
  ...
  const daemonProcess = spawn(
    ...
    [TSX_ENTRY, cliSrcPath, "daemon", "start", "--foreground"],
    {
      env: {
        ...
        CODIUS_LISTEN: `${TEST_DAEMON_HOST}:${port}`,
 ...
 */
export async function runCodiusCli(
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn(process.execPath, [TSX_ENTRY, cliSrcPath, ...args], {
      env: {
        ...process.env,
        ...TEST_DAEMON_ENV_DEFAULTS,
        CODIUS_HOST: `${TEST_DAEMON_HOST}:${ctx.port}`,
        CODIUS_HOME: ctx.codiusHome,
        ...options?.env,
      },
      cwd,
      stdio: ["ignore", "pipe", "pipe"],
