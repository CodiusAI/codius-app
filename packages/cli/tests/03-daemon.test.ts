 ...
 *
 * Tests daemon commands with an isolated CODIUS_HOME.
 *
 * Tests:
...
function daemonCommand(args: string[]) {
  return runLocalCodius(["daemon", ...args], { CODIUS_HOME: codiusHome });
}
...
try {
  // Test 1: daemon --help shows subcommands
  {
    ...
    const result = await runLocalCodius(["daemon", "--help"]);
  ...
  // Test 5: daemon status --json outputs valid JSON
  {
    ...
    assert.strictEqual(status.home, codiusHome, "json status should reflect the isolated home");
    assert.strictEqual(
      status.hostname,
      null,
  ...
  // Test 8: status uses the running daemon as relay authority over local IPC
  {
    ...
    const listen =
      process.platform === "win32"
        ? `\\\\.\\pipe\\codius-status-${process.pid}-${Date.now()}`
        : join(codiusHome, "status.sock");
    ...
    const configPath = join(codiusHome, "config.json");
    ...
    const foreignHome = await mkdtemp(join(tmpdir(), "codius-test-foreign-home-"));
    try {
      await writeFile(
      ...
      const foreignPairing = await runLocalCodius(
        ["daemon", "pair", "--home", foreignHome, "--json"],
        { CODIUS_HOME: foreignHome },
      );
      ...
      assert(
        foreignPairing.stderr.includes("different Codius home"),
