...
try {
  // Test 1: stop --help shows options
  {
    ...
    const result = await $`npx codius stop --help`.nothrow();
  ...
  // Test 2: stop requires ID, --all, or --cwd
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius stop`.nothrow();
  ...
  // Test 3: stop handles daemon not running
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius stop abc123`.nothrow();
  ...
  // Test 4: stop --all flag is accepted
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius stop --all`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: stop --cwd flag is accepted
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius stop --cwd /tmp`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: stop with ID and --host flag is accepted
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius stop abc123 --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: codius --help shows stop command
  {
    console.log("Test 7: codius --help shows stop command");
    ...
    console.log("✓ codius --help shows stop command\n");
  }
  ...
  // Test 8: -q (quiet) flag is accepted with stop
  {
    ...
    const result =
      await $`CODIUS_HOST=localhost:${port} CODIUS_HOME=${codiusHome} npx codius -q stop abc123`.nothrow();
    const output = result.stdout + result.stderr;
