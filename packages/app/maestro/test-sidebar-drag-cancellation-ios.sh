...
OUT_DIR="/tmp/codius-sidebar-drag-cancellation-$(date +%s)"
CLIENT_EXPORTS="$REPO_ROOT/packages/client/dist/daemon-client.js"
RELAY_EXPORTS="$REPO_ROOT/node_modules/@codius.ai/relay/dist/e2ee.js"
FIXTURE_ROOT=""
...
export CODIUS_MAESTRO_DAEMON_HEALTH_URL="${CODIUS_MAESTRO_DAEMON_HEALTH_URL:-http://127.0.0.1:6767/api/health}"
...
if ! curl --fail --silent --show-error --max-time 3 "$CODIUS_MAESTRO_DAEMON_HEALTH_URL" >/dev/null; then
  echo "Codius daemon is unavailable at $CODIUS_MAESTRO_DAEMON_HEALTH_URL" >&2
  exit 1
fi

FIXTURE_ROOT="$(mktemp -d /tmp/codius-sidebar-drag-fixture-XXXXXX)"
export CODIUS_MAESTRO_DRAG_A_NAME="000-codius-drag-a-$(basename "$FIXTURE_ROOT")"
export CODIUS_MAESTRO_DRAG_B_NAME="001-codius-drag-b-$(basename "$FIXTURE_ROOT")"
export CODIUS_MAESTRO_DRAG_Z_NAME="zzz-codius-drag-z-$(basename "$FIXTURE_ROOT")"

mkdir -p "$OUT_DIR/flows"

if [ -z "${CODIUS_MAESTRO_IOS_UDID:-}" ]; then
  export CODIUS_MAESTRO_IOS_UDID
  CODIUS_MAESTRO_IOS_UDID="$({ xcrun simctl list devices booted -j || true; } | node -e '
    let input = "";
...
render_flow() {
  ...
  local target="$2"
  perl -0pe '
    s/^appId: ai\.codius$/appId: $ENV{CODIUS_MAESTRO_APP_ID}/m;
    s/\$\{CODIUS_MAESTRO_APP_ID\}/$ENV{CODIUS_MAESTRO_APP_ID}/g;
    s/\$\{CODIUS_MAESTRO_DIRECT_ENDPOINT\}/$ENV{CODIUS_MAESTRO_DIRECT_ENDPOINT}/g;
    s/\$\{CODIUS_MAESTRO_DRAG_A_NAME\}/$ENV{CODIUS_MAESTRO_DRAG_A_NAME}/g;
    s/\$\{CODIUS_MAESTRO_DRAG_B_NAME\}/$ENV{CODIUS_MAESTRO_DRAG_B_NAME}/g;
    s/\$\{CODIUS_MAESTRO_DRAG_Z_NAME\}/$ENV{CODIUS_MAESTRO_DRAG_Z_NAME}/g;
  ' "$source" > "$target"
}

for project_name in \
  "$CODIUS_MAESTRO_DRAG_A_NAME" \
  "$CODIUS_MAESTRO_DRAG_B_NAME" \
  "$CODIUS_MAESTRO_DRAG_Z_NAME"; do
  ...
  git -C "$project_path" config user.name "Codius Maestro"
  git -C "$project_path" config user.email "maestro@codius.local"
...
const client = new DaemonClient({
  url: process.env.CODIUS_MAESTRO_DAEMON_WS_URL,
...
const { DaemonClient } = await import(moduleUrl);
const projectNames = [
  process.env.CODIUS_MAESTRO_DRAG_A_NAME,
  process.env.CODIUS_MAESTRO_DRAG_B_NAME,
  process.env.CODIUS_MAESTRO_DRAG_Z_NAME,
];
const client = new DaemonClient({
  url: process.env.CODIUS_MAESTRO_DAEMON_WS_URL,
...
echo "Running sidebar drag cancellation regression on $CODIUS_MAESTRO_IOS_UDID"
echo "Artifacts: $OUT_DIR"
(cd "$OUT_DIR" && maestro test "$FLOW" --udid "$CODIUS_MAESTRO_IOS_UDID")
