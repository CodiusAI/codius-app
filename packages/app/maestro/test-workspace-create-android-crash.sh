...
OUT_DIR="/tmp/codius-workspace-create-android-$(date +%s)"
...
render_flow() {
  ...
  perl -0pe '
    ...
    s/\$\{CODIUS_MAESTRO_PROJECT_NAME\}/$ENV{CODIUS_MAESTRO_PROJECT_NAME}/g;
  ' "$source" > "$target"
}
...
if [ -z "${CODIUS_MAESTRO_PROJECT_PATH:-}" ]; then
  PROJECT_PARENT="$(mktemp -d /tmp/codius-maestro-project-XXXXXX)"
  PROJECT_BASENAME="aaa-workspace-create-android-$(basename "$PROJECT_PARENT")"
  export CODIUS_MAESTRO_PROJECT_PATH="$PROJECT_PARENT/$PROJECT_BASENAME"
  mkdir -p "$CODIUS_MAESTRO_PROJECT_PATH"
  git -C "$CODIUS_MAESTRO_PROJECT_PATH" init >/dev/null
  git -C "$CODIUS_MAESTRO_PROJECT_PATH" checkout -b main >/dev/null 2>&1 || true
  git -C "$CODIUS_MAESTRO_PROJECT_PATH" config user.name "Codius Maestro"
  git -C "$CODIUS_MAESTRO_PROJECT_PATH" config user.email "maestro@codius-ai.local"
  printf "# Workspace create Android repro\n" > "$CODIUS_MAESTRO_PROJECT_PATH/README.md"
  git -C "$CODIUS_MAESTRO_PROJECT_PATH" add README.md
  git -C "$CODIUS_MAESTRO_PROJECT_PATH" commit -m "Initial commit" >/dev/null
else
  PROJECT_PARENT=""
fi

export CODIUS_MAESTRO_PROJECT_NAME="${CODIUS_MAESTRO_PROJECT_NAME:-$(basename "$CODIUS_MAESTRO_PROJECT_PATH")}"

echo "=== Workspace Create Android Crash Harness ==="
echo "Output dir: $OUT_DIR"
echo "App id: $CODIUS_MAESTRO_APP_ID"
echo "Android direct endpoint: $CODIUS_MAESTRO_DIRECT_ENDPOINT"
echo "Daemon websocket: $CODIUS_MAESTRO_DAEMON_WS_URL"
echo "Project: $CODIUS_MAESTRO_PROJECT_PATH"
echo "Project name: $CODIUS_MAESTRO_PROJECT_NAME"
...
const projectPath = process.env.CODIUS_MAESTRO_PROJECT_PATH;
const daemonUrl = process.env.CODIUS_MAESTRO_DAEMON_WS_URL;
