...
describe("ReplicaCache", () => {
  ...
  it("persists only the focused agent view with a short timeline tail", async () => {
    ...
    const persisted = JSON.parse(storage.values.get("@codius:replica-cache") ?? "null") as {
      version: number;
  ...
  it("rejects version 1 cache data and overwrites it on flush", async () => {
    ...
    expect(JSON.parse(storage.values.get("@codius:replica-cache") ?? "null")).toEqual({
      version: 3,
      hosts: [],
    });
  });
