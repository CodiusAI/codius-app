...
describe("tool call detail-level projection", () => {
  ...
  it("distinguishes reads, searches, and other tools in overview", () => {
    const calls = [
      ...
      toolCall(
        "4",
        { type: "search", query: "codius", toolName: "web_search" },
        { status: "failed" },
      ),
  ...
  it("counts Codius calls separately from other tools", () => {
    const calls = [
      toolCall("1", { type: "unknown", input: null, output: null }, { name: "codius.list_agents" }),
      toolCall(
        "2",
        { type: "unknown", input: null, output: null },
        { name: "mcp__codius__list_worktrees" },
      ),
      toolCall("3", { type: "fetch", url: "https://codius.ai" }),
      toolCall("4", { type: "fetch", url: "https://github.com/codius-ai" }),
    ];

    const result = project({ level: "overview", head: calls });

    expect(result.groupsByHostId.get("1")).toMatchObject({
      summary: { otherToolCount: 2, codiusCallCount: 2 },
    });
  });

  it("classifies direct Brave search and Codius runtime tool names", () => {
    ...
    const calls = [
      ...
      toolCall("3", unknownDetail, { name: "codius_list_providers" }),
      toolCall("4", unknownDetail, { name: "codius_list_worktrees" }),
      toolCall("5", unknownDetail, { name: "codius_list_worktrees" }),
    ...
    expect(result.groupsByHostId.get("1")).toMatchObject({
      summary: { searchCount: 3, otherToolCount: 0, codiusCallCount: 3 },
    });
  });
