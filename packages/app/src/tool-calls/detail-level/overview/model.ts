...
function isCodiusCall(name: string, normalizedName: string): boolean {
  return isCodiusToolName(name) || normalizedName.startsWith(DIRECT_CODIUS_TOOL_PREFIX);
}
...
export function buildOverviewGroup(run: ToolCallRun): OverviewToolCallGroup {
  ...
  for (const call of run.calls) {
    ...
    if (isCodiusCall(descriptor.name, normalizedName)) {
      codiusCallCount += 1;
