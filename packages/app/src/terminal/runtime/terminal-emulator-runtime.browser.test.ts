...
function getBrowserTerminal(): BrowserTerminal {
  const terminal = window.__codiusTerminal as BrowserTerminal | undefined;
  if (!terminal) {
...
describe("terminal emulator runtime in a real browser", () => {
  it("passes configured scrollback to xterm", async () => {
    ...
    await waitFor({
      predicate: () => window.__codiusTerminal !== undefined,
    });

    expect(window.__codiusTerminal?.options.scrollback).toBe(42_000);
  });
  ...
  it("updates scrollback on the mounted xterm", async () => {
    ...
    const terminal = window.__codiusTerminal;

    mounted.runtime.setScrollback({ lines: 42_000 });

    expect(window.__codiusTerminal).toBe(terminal);
    expect(window.__codiusTerminal?.options.scrollback).toBe(42_000);
  });
