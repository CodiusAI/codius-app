...
import type { CheckoutPrMergeMethod, PullRequestMergeable } from "@codius-ai/protocol/messages";
...
function getPrimaryActionId(input: BuildGitActionsInput): GitActionId | null {
  ...
  // Only Codius-owned worktrees get Archive as a fallback primary action.
  // Regular Git checkouts should not show the destructive archive CTA by default.
  if (input.isCodiusOwnedWorktree) {
    return "archive-workspace";
  }
...
function hasPushableCommits(input: BuildGitActionsInput): boolean {
  ...
  // No-upstream Codius worktrees are first-pushable: the daemon push sets upstream with `git push -u`.
  // Do not fold this into aheadOfOrigin; null also covers deleted/pruned upstream branches.
  return input.isCodiusOwnedWorktree && input.aheadOfOrigin === null && input.aheadCount > 0;
}
