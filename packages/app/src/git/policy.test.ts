...
describe("git-actions-policy", () => {
  ...
  it("keeps push available for a no-upstream Codius worktree with local commits", () => {
    const actions = buildGitActions(
      createInput({
        hasRemote: true,
  ...
  it("offers archive workspace for Git checkouts and worktrees", () => {
    ...
    const worktree = buildGitActions(
      createInput({ hasUncommittedChanges: true, isCodiusOwnedWorktree: true }),
    );
  ...
  it("still promotes archive as primary for an idle Codius-owned worktree", () => {
    const actions = buildGitActions(createInput({ isCodiusOwnedWorktree: true }));
