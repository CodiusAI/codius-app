...
export default function TerminalEmulator({
  ...
  const flushPendingMessages = useCallback(() => {
    ...
    for (const message of pending) {
      ...
      webViewRef.current.injectJavaScript(
        `window.__CODIUS_TERMINAL_WEBVIEW_RECEIVE__ && window.__CODIUS_TERMINAL_WEBVIEW_RECEIVE__(${payload}); true;`,
      );
    }
  }, []);
  ...
  useImperativeHandle(
    ...
    (): TerminalEmulatorHandle => ({
      ...
      blur: () => {
        webViewRef.current?.injectJavaScript(
          "window.__CODIUS_TERMINAL_WEBVIEW_BLUR__ && window.__CODIUS_TERMINAL_WEBVIEW_BLUR__(); true;",
        );
        Keyboard.dismiss();
      },
    }),
