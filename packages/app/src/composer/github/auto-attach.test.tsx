...
const pr202: ForgeSearchItem = {
  ...
  url: "https://github.com/acme/codius/pull/202",
...
describe("useComposerGithubAutoAttach", () => {
  ...
  it("preserves pull request order across lookup batches", async () => {
    ...
    act(() => {
      result.current.setText(
        "Review https://github.com/acme/codius/pull/101 and https://github.com/acme/codius/pull/202",
      );
    });
    expect(result.current.isResolving).toBe(true);
    await flushDebounce();
  ...
  it("uses the latest pull request order when URLs are reordered during debounce", async () => {
    ...
    act(() => {
      result.current.setText(
        "Refs https://github.com/acme/codius/pull/202 and https://github.com/acme/codius/pull/101",
      );
    });
    await flushDebounce();
  ...
  it("does not report a buffered pull request after its URL is removed", async () => {
    ...
    act(() => {
      result.current.setText("Still https://github.com/acme/codius/pull/101");
    });
    await flushDebounce();
  ...
  it("stays resolving when an unrelated attachment is added during lookup", async () => {
    ...
    act(() => {
      ...
      result.current.setText("Review https://github.com/acme/codius/pull/101 please");
    });
