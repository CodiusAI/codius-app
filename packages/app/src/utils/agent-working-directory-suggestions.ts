...
const CODIUS_WORKTREE_PATH_PATTERN = /(^|\/)\.codius\/worktrees(\/|$)/;
...
export function collectAgentWorkingDirectorySuggestions(
  ...
  for (const source of sources) {
    ...
    if (isCodiusOwnedWorktreePath(cwd)) {
      continue;
    }
...
function isCodiusOwnedWorktreePath(cwd: string): boolean {
  return CODIUS_WORKTREE_PATH_PATTERN.test(cwd.replace(/\\/g, "/"));
}
