...
const runningService: WorkspaceScriptPayload = {
  scriptName: "web",
  type: "service",
  hostname: "web--feature--codius.localhost",
  port: 3000,
  localProxyUrl: "http://web--feature--codius.localhost:6767",
  publicProxyUrl: null,
  proxyUrl: "http://web--feature--codius.localhost:6767",
  lifecycle: "running",
  health: "healthy",
  exitCode: null,
  terminalId: null,
...
describe("resolveWorkspaceScriptLink", () => {
  it("defaults to the memorable Codius URL locally and keeps direct as a fallback", () => {
    expect(
      resolveLink({ type: "directTcp", endpoint: "localhost:6767", display: "localhost:6767" }),
    ).toEqual({
      primary: {
        kind: "codius",
        label: "web--feature--codius.localhost:6767",
        url: "http://web--feature--codius.localhost:6767",
      },
      targets: [
        {
          kind: "codius",
  ...
  it("defaults to an explicitly configured reverse proxy", () => {
    const publicUrl = "https://web--feature--codius.services.example.com";
    expect(
      resolveLink(
        { type: "directSocket", endpoint: "/tmp/codius.sock", display: "socket" },
        { ...runningService, publicProxyUrl: publicUrl, proxyUrl: publicUrl },
      ),
    ).toEqual({
      primary: {
        kind: "public",
        label: "web--feature--codius.services.example.com",
        url: publicUrl,
      },
      targets: [
        {
  ...
  it("offers the reverse proxy and direct route over a direct network connection", () => {
    ...
    expect(
      ...
      { kind: "public", label: "web--feature--codius.services.example.com", url: publicUrl },
      {
        kind: "codius",
        label: "web--feature--codius.localhost:6767",
        url: "http://web--feature--codius.localhost:6767",
  ...
  it("classifies proxyUrl from older daemons", () => {
    ...
    expect(
      resolveLink(
        { type: "relay", endpoint: "relay.codius.ai:443", display: "relay" },
