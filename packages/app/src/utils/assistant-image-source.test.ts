...
describe("resolveAssistantImageSource", () => {
  ...
  it("falls back to filesystem root for absolute paths outside the workspace", () => {
    expect(
      resolveAssistantImageSource({
        source: "/tmp/codius-codex-screenshot.png",
        workspaceRoot: "/Users/test/project",
      }),
    ).toEqual({
      kind: "file_rpc",
      cwd: "/",
      path: "/tmp/codius-codex-screenshot.png",
    });
  });
  ...
  it("uses the same home-root target as file previews for tilde paths", () => {
    expect(
      resolveAssistantImageSource({
        source: "~/.codius/screenshots/output.png",
        workspaceRoot: "/Users/test/project",
      }),
    ).toEqual({
      kind: "file_rpc",
      cwd: "~",
      path: "~/.codius/screenshots/output.png",
    });
  });
  ...
  it("normalizes file URIs into file RPC requests", () => {
    expect(
      resolveAssistantImageSource({
        source: "file:///tmp/codius-codex-screenshot.png",
        workspaceRoot: "/Users/test/project",
      }),
    ).toEqual({
      kind: "file_rpc",
  ...
  it("normalizes markdown-encoded Windows paths into file RPC requests", () => {
    expect(
      resolveAssistantImageSource({
        source: "C:%5CUsers%5Chanse%5CAppData%5CLocal%5CTemp%5Ccodius-attachments%5Cimage.png",
        workspaceRoot: "C:/Users/hanse/eatingkat",
      }),
    ).toEqual({
      kind: "file_rpc",
      cwd: "C:/",
      path: "C:/Users/hanse/AppData/Local/Temp/codius-attachments/image.png",
    });
  });
