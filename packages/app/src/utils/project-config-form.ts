...
export interface ProjectConfigDraft {
  ...
  metadataGenerationBase: CodiusMetadataGeneration | undefined;
}
...
export function configToDraft(config: CodiusConfigRaw | null | undefined): ProjectConfigDraft {
...
interface ApplyDraftInput {
  ...
  base: CodiusConfigRaw | null | undefined;
}

export function applyDraftToConfig(input: ApplyDraftInput): CodiusConfigRaw {
  ...
  const nextScripts: Record<string, CodiusScriptEntryRaw> = {};
  for (const row of input.draft.scripts) {
    ...
    nextScripts[trimmedName] = nextEntry as CodiusScriptEntryRaw;
  }
  ...
  return result as CodiusConfigRaw;
}
