...
import type { DaemonClientConfig } from "@codius.ai/client/internal/daemon-client";
...
class FakeDaemonProbe {
  ...
  readonly deps: DaemonConnectionDependencies<FakeDaemonClient> = {
    ...
    buildLocalTransportUrl: ({ transportType, transportPath }) =>
      `codius+local://${transportType}?path=${encodeURIComponent(transportPath)}`,
    createClient: (config) => {
...
describe("test-daemon-connection connectToDaemon", () => {
  ...
  it("encodes the local socket target into the client config", async () => {
    ...
    const result = await connectToDaemon(
      {
        id: "socket:/tmp/codius.sock",
        type: "directSocket",
        path: "/tmp/codius.sock",
      },
      undefined,
      probe.deps,
    );
    ...
    expect(probe.createdConfigs()[0]?.url).toBe("codius+local://socket?path=%2Ftmp%2Fcodius.sock");
  });
  ...
  it("uses relay TLS from the stored connection", async () => {
    ...
    expect(probe.createdConfigs()[1]?.url).toMatch(/^ws:\/\/relay\.codius\.ai:443\/ws\?/);
  });
