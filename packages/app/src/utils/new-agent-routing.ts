...
function inferMainRepoRootFromCodiusWorktreePath(cwd: string): string | null {
  const normalizedPath = cwd.replace(/\\/g, "/");
  const marker = "/.codius/worktrees";
...
export function resolveNewAgentWorkingDir(
  ...
  const explicitMainRepoRoot = checkout?.isCodiusOwnedWorktree
  ...
  return inferMainRepoRootFromCodiusWorktreePath(cwd) ?? cwd;
}
