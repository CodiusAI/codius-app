...
const SHARED_PROJECT_NAME = "Codius";
const SHARED_REMOTE_URL = "https://github.com/getcodius/codius.git";
...
const test = base.extend<{ crossHostProject: CrossHostProjectScenario }>({
  crossHostProject: async ({ page: _page }, provide) => {
    ...
    const primarySharedRepo = await createTempGitRepo("new-workspace-codius-primary-", {
      originUrl: SHARED_REMOTE_URL,
    });
    const primaryDuplicateRepo = await createTempGitRepo("new-workspace-codius-duplicate-", {
      originUrl: SHARED_REMOTE_URL,
    });
    const secondaryFallbackRepo = await createTempGitRepo("new-workspace-fallback-");
    const secondarySharedRepo = await createTempGitRepo("new-workspace-codius-secondary-", {
      originUrl: SHARED_REMOTE_URL,
    });
    ...
    try {
      ...
      const primarySharedProject = await createProject(primaryClient, {
        ...
        workspaceName: "Primary Codius workspace",
      });
      ...
      const primaryDuplicateProject = await createProject(primaryClient, {
        ...
        projectName: "Codius duplicate",
        workspaceName: "Duplicate Codius workspace",
      });
      ...
      const secondarySharedProject = await createProject(secondaryClient, {
        ...
        projectName: SHARED_PROJECT_NAME,
        workspaceName: "Secondary Codius workspace",
      });
      ...
      if (
        ...
        throw new Error("Expected the Codius clones to have the same equivalence key");
      }
