    ...
    (window as unknown as { codiusDesktop: unknown }).codiusDesktop = {
      ...
      opener: {
        openUrl: async (url: string) => {
          localStorage.setItem("@codius:e2e-opened-url", url);
        },
      },
    };
  }, daemon.serverId);
...
export async function expectRelayConsent(page: Page): Promise<void> {
  ...
  await expect(modal.getByRole("link", { name: "Read how Codius relay works" })).toBeVisible();
...
export async function reloadAndOpenPairDevice(page: Page): Promise<void> {
  await page.evaluate(() => {
    const nonce = localStorage.getItem("@codius:e2e-seed-nonce");
    if (!nonce) throw new Error("Expected e2e seed nonce");
    localStorage.setItem("@codius:e2e-disable-default-seed-once", nonce);
  });
  await page.reload();
...
export async function expectRelayUpdateRequired(page: Page): Promise<void> {
  const modal = page.getByTestId("host-page-pair-device-card");
  await expect(
    modal.getByText("Update the host to enable relay from Codius Desktop."),
  ).toBeVisible();
...
export async function openRelaySecurityDocs(page: Page): Promise<void> {
  await page.getByRole("link", { name: "Read how Codius relay works" }).click();
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("@codius:e2e-opened-url")))
    .toBe("https://codius.ai/docs/security");
}
