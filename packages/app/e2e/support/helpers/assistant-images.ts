    ...
    ({ accessibleName, errorText, tabTestId }) => {
      ...
      (
        window as unknown as {
          __codiusAssistantImageObservation?: typeof record;
        }
      ).__codiusAssistantImageObservation = record;
    },
    {
      accessibleName: alt,
...
async function expectNoVisibleImageInstability(page: Page): Promise<void> {
  const observation = await page.evaluate(() => {
    const owner = window as unknown as {
      __codiusAssistantImageObservation?: {
        errorSeen: boolean;
        missingSeen: boolean;
        inspect(): void;
    ...
    const record = owner.__codiusAssistantImageObservation;
    if (!record) throw new Error("Assistant image stability observation was not started");
    record.inspect();
    record.observer?.disconnect();
    delete owner.__codiusAssistantImageObservation;
