...
interface ProjectClient {
  ...
  close(): Promise<void>;
  createCodiusWorktree?: (...args: unknown[]) => Promise<OwnedResourceResult>;
...
export function withProjectOwnership<Client extends object>(client: Client): Client {
  ...
  return new Proxy(client, {
    get(_target, property) {
      ...
      if (property === "createCodiusWorktree" && target.createCodiusWorktree) {
        const createCodiusWorktree = target.createCodiusWorktree.bind(target);
        return async (...args: unknown[]) => trackProject(await createCodiusWorktree(...args));
      }
