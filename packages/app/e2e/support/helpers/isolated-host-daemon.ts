...
export async function startIsolatedHostDaemon(
  ...
  const codiusHome =
    options.codiusHome ?? (await mkdtemp(path.join(tmpdir(), "codius-e2e-secondary-host-")));
  let publishedPackageRoot: string | null = null;
  if (options.publishedVersion) {
    publishedPackageRoot = await mkdtemp(path.join(tmpdir(), "codius-e2e-published-server-"));
    await writeFile(
    ...
    try {
      execFileSync(
        ...
        [
          ...
          "--no-fund",
          "--no-package-lock",
          `@codius.ai/server@${options.publishedVersion}`,
        ],
  ...
  const serverDir = publishedPackageRoot
    ? path.join(publishedPackageRoot, "node_modules", "@codius.ai", "server")
  ...
  const spawnDaemon = async (): Promise<ChildProcess> => {
    const spawnOptions: SpawnOptions = {
      ...
      env: withDisabledE2ESpeechEnv({
        ...
        CODIUS_RELAY_ENABLED: options.mutableRelay ? undefined : "0",
