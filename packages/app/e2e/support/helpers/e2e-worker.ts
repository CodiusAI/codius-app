...
import { forkCodiusHomeMetadata, resolveCodiusHomePath } from "./codius-home-fork";
...
function resolveOptionalHome(value: string | undefined): string | null {
  ...
  return resolveCodiusHomePath(trimmed === "current" ? "~/.codius" : trimmed);
}
...
async function applyMetadataFork(targetHome: string, providerIds: string[]): Promise<void> {
  const sourceHome = resolveOptionalHome(process.env.E2E_FORK_CODIUS_HOME_FROM);
  if (!sourceHome) return;
  const result = await forkCodiusHomeMetadata({ sourceHome, targetHome });
  process.env.E2E_FORK_SOURCE_CODIUS_HOME = result.sourceHome;
  process.env.E2E_FORK_TARGET_CODIUS_HOME = result.targetHome;
...
export async function startE2EWorker(
  ...
  const requestedRoot = resolveOptionalHome(process.env.E2E_CODIUS_HOME);
  const codiusHome = requestedRoot
    ? path.join(requestedRoot, `worker-${workerIndex}`)
    : await mkdtemp(path.join(tmpdir(), `codius-e2e-worker-${workerIndex}-`));
  const preserveHome = Boolean(requestedRoot) || process.env.E2E_KEEP_CODIUS_HOME === "1";
  ...
  try {
    await applyMetadataFork(codiusHome, options.forkProviders ?? []);
    const daemon = await startIsolatedHostDaemon(serverId, {
      ...
      environment: {
        ...
        CODIUS_E2E_EDITOR_RECORD_PATH: editorRecordPath,
      },
    });

    process.env.E2E_DAEMON_PORT = String(daemon.port);
    process.env.E2E_SERVER_ID = daemon.serverId;
    process.env.E2E_CODIUS_HOME = daemon.codiusHome;
    ...
    console.log(
      `[e2e] Worker ${workerIndex} daemon started on port ${daemon.port}, home: ${daemon.codiusHome}`,
    );
    return {
      close: async () => {
        await daemon.close();
    ...
    await rm(fakeEditorBin, { recursive: true, force: true });
    if (!preserveHome) await rm(codiusHome, { recursive: true, force: true });
    throw error;
  }
}
