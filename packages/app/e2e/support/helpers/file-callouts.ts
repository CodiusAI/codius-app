...
export async function recordFileCallouts(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    Object.assign(globalThis, { __CODIUS_RECORDED_FILE_CALLOUTS__: callouts });
  });
}
...
function recordedFileCallouts(page: Page): Promise<string[]> {
  return page.evaluate(
    () =>
      (globalThis as typeof globalThis & { __CODIUS_RECORDED_FILE_CALLOUTS__?: string[] })
        .__CODIUS_RECORDED_FILE_CALLOUTS__ ?? [],
  );
}
