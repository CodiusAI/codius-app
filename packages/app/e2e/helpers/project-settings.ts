...
export async function corruptCodiusConfig(repoPath: string): Promise<void> {
  await writeFile(path.join(repoPath, "codius.json"), "{not valid json}");
}

export async function bumpCodiusConfigOnDisk(repoPath: string): Promise<void> {
  const configPath = path.join(repoPath, "codius.json");
...
export async function restoreCodiusConfig(
  repoPath: string,
  config: Record<string, unknown>,
): Promise<void> {
  await writeFile(path.join(repoPath, "codius.json"), JSON.stringify(config, null, 2) + "\n");
}
...
export async function blockCodiusConfigWrites(repoPath: string): Promise<void> {
  await chmod(repoPath, 0o555);
}

export async function unblockCodiusConfigWrites(repoPath: string): Promise<void> {
  await chmod(repoPath, 0o755);
}

// --- WebSocket helpers ---

// Proxies all daemon WS traffic transparently, but rejects codius.json reads
