...
export async function installTerminalRenderProbe(page: Page): Promise<void> {
  await page.addInitScript(() => {
    interface ProbeTerm {
      ...
      __codiusRenderProbeWriteWrapped?: boolean;
    }
    ...
    const win = window as unknown as Record<string, unknown> & {
      ...
      __codiusTerminal?: ProbeTerm;
    };
    const existingDescriptor = Object.getOwnPropertyDescriptor(win, "__codiusTerminal");
    const getExisting = () =>
    ...
    Object.defineProperty(win, "__codiusTerminal", {
      configurable: true,
      get() {
        return probe.term;
      },
      set(next: ProbeTerm | undefined) {
        ...
        if (next?.write && !next.__codiusRenderProbeWriteWrapped) {
          ...
          next.__codiusRenderProbeWriteWrapped = true;
        }
      },
    });
  });
...
export async function installTerminalKeystrokeStressProbe(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    const existingDescriptor = Object.getOwnPropertyDescriptor(window, "__codiusTerminal");
    const getExisting = () =>
      existingDescriptor?.get ? existingDescriptor.get.call(window) : existingDescriptor?.value;

    let terminal = getExisting();
    Object.defineProperty(window, "__codiusTerminal", {
      configurable: true,
      get() {
        return terminal;
      },
      set(next: {
        write?: (data: string | Uint8Array, callback?: () => void) => void;
        __codiusKeystrokeProbeWriteWrapped?: boolean;
      }) {
        terminal = next;
        if (next?.write && !next.__codiusKeystrokeProbeWriteWrapped) {
          const originalWrite = next.write.bind(next);
          next.write = (data: string | Uint8Array, callback?: () => void) => {
            const text = typeof data === "string" ? data : new TextDecoder().decode(data);
          ...
          next.__codiusKeystrokeProbeWriteWrapped = true;
        }
      },
    });
  });
