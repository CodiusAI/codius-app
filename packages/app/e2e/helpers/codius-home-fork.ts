...
export interface CodiusHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveCodiusHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkCodiusHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<CodiusHomeMetadataForkResult> {
  const sourceHome = resolveCodiusHomePath(input.sourceHome);
  const targetHome = resolveCodiusHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Codius metadata onto the same CODIUS_HOME.");
  }
