...
const DISABLE_DEFAULT_SEED_ONCE_KEY = "@codius:e2e-disable-default-seed-once";
const SEED_NONCE_KEY = "@codius:e2e-seed-nonce";
const REGISTRY_KEY = "@codius:daemon-registry";
...
export async function seedSavedSettingsHosts(
  ...
  await page.evaluate(
    ({ keys, storedRegistry, storedPreferences }) => {
      ...
      localStorage.setItem("@codius:create-agent-preferences", JSON.stringify(storedPreferences));
...
export async function expectHostInjectMcpCard(page: Page): Promise<void> {
  ...
  await expect(card).toBeVisible();
  await expect(card.getByRole("switch", { name: "Inject Codius tools" })).toBeVisible();
}
