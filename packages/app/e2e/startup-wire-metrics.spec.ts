...
class WireMonitor {
  ...
  summarize(clickedWorkspaces: WireSummary["clickedWorkspaces"]): WireSummary {
    return {
      ...
      fork: {
        sourceHome: process.env.E2E_FORK_SOURCE_CODIUS_HOME ?? null,
        targetHome: process.env.E2E_FORK_TARGET_CODIUS_HOME ?? null,
...
test.describe("ad hoc startup wire metrics", () => {
  ...
  test("measures startup hydration and workspace navigation websocket traffic", async ({
    ...
    console.log("CODIUS_STARTUP_WIRE_METRICS_BEGIN");
    console.log(JSON.stringify(summary, null, 2));
    console.log("CODIUS_STARTUP_WIRE_METRICS_END");
