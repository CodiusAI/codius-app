...
async function readShimmerEvidence(locator: Locator): Promise<ShimmerEvidence> {
  return locator.evaluate((root) => {
    const shimmer = Array.from(root.querySelectorAll<HTMLElement>("*")).find((element) =>
      getComputedStyle(element).animationName.includes("codius-toolcall-shimmer"),
    );
    if (!shimmer) {
    ...
    return {
      ...
      endPx: Number.parseFloat(style.getPropertyValue("--codius-shimmer-end")),
      label: shimmer.textContent ?? "",
      renderedWidth: shimmer.getBoundingClientRect().width,
      startPx: Number.parseFloat(style.getPropertyValue("--codius-shimmer-start")),
    };
  });
}
...
test("measures an overview heading that becomes loading after its idle mount", async ({
  ...
  try {
    ...
    await expect(group.locator('[style*="codius-toolcall-shimmer"]')).toHaveCount(0);

    await gate.waitForSecondRunning();
    gate.releaseSecondRunning();
    await expect(group.locator('[style*="codius-toolcall-shimmer"]')).not.toHaveCount(0);
