...
interface CodiusOwnedWorktree {
  projectName: string;
  workspaceId: string;
  worktreeSlug: string;
}

async function withCodiusOwnedWorktree(
  run: (workspace: CodiusOwnedWorktree) => Promise<void>,
): Promise<void> {
  ...
  try {
    ...
    if (!created.workspace) {
      throw new Error(created.error ?? "Failed to create Codius-owned worktree");
    }
...
test.describe("Sidebar workspace list", () => {
  ...
  test("Codius-owned worktree hover card shows the worktree directory name", async ({ page }) => {
    await withCodiusOwnedWorktree(async ({ projectName, workspaceId, worktreeSlug }) => {
      await gotoAppShell(page);
      await waitForSidebarProject(page, projectName);
