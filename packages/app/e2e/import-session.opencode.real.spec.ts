...
const CODIUS_REPO_PATH = path.resolve(__dirname, "../../..");
...
test("imports a real OpenCode session from the workspace import sheet", async ({ page }) => {
  const scenario = await seedCodiusWorkspaceWithOpenCodeSession();
...
async function seedCodiusWorkspaceWithOpenCodeSession(): Promise<OpenCodeImportScenario> {
  const response = `CODIUS_OPENCODE_IMPORT_E2E_OK_${randomUUID().slice(0, 8)}`;
  const prompt = `Do not use tools. Reply with exactly: ${response}`;
  const promptPreview = JSON.stringify(prompt);
  await launchOpenCodeSessionInWorkspace(CODIUS_REPO_PATH, prompt);
  const client = await connectSeedClient();
  try {
    const createdWorkspace = await client.createWorkspace({
      source: { kind: "directory", path: CODIUS_REPO_PATH },
    });
    if (!createdWorkspace.workspace) {
      throw new Error(createdWorkspace.error ?? `Failed to create workspace ${CODIUS_REPO_PATH}`);
    }
...
function formatOpenCodeLaunchError(result: OpenCodeSeedResult, prompt: string): string {
  return [
    ...
    `command: ${["opencode", ...openCodeSeedArgs(CODIUS_REPO_PATH, prompt)].join(" ")}`,
