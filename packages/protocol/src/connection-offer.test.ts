...
describe("connection offer", () => {
  ...
  it("parses connection offers from QR-style URLs", () => {
    ...
    expect(parseConnectionOfferFromUrl(`https://app.codius.ai/#offer=${encoded}`)).toEqual(offer);
  });
  ...
  it("round-trips relay TLS in offers without rejecting extra relay fields", () => {
    ...
    expect(parseConnectionOfferFromUrl(`https://app.codius.ai/#offer=${encoded}`)).toEqual({
      v: 2,
      serverId: "server-123",
      daemonPublicKeyB64: "pubkey",
  ...
  it("returns null when the URL has no offer fragment", () => {
    expect(parseConnectionOfferFromUrl("https://app.codius.ai/pair")).toBeNull();
  });
});
