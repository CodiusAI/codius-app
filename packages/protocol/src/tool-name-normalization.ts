...
export function isCodiusToolName(name: string): boolean {
  const normalized = normalizeToolName(name);
  ...
  if (normalized.includes("__")) {
    ...
    return (
      segments.length >= 3 &&
      segments[0] === "mcp" &&
      (segments[1] === "codius" || segments[1].startsWith("codius_"))
    );
  }
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0];
    return firstSegment === "codius" || firstSegment.startsWith("codius_");
  }
  return false;
}

export function getCodiusToolLeafName(name: string): string | null {
  const normalized = normalizeToolName(name);
  if (normalized.includes("__")) {
    const segments = normalized.split("__").filter((s) => s.length > 0);
    if (
  ...
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0];
    if (firstSegment === "codius" || firstSegment.startsWith("codius_")) {
