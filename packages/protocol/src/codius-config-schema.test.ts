...
describe("codius config schema", () => {
  it("parses an empty config without metadata generation", () => {
    const parsed = CodiusConfigSchema.parse({});
  ...
  it("parses old-style worktree and scripts config unchanged", () => {
    ...
    expect(CodiusConfigSchema.parse(config)).toEqual({
      worktree: {
        setup: ["npm install"],
        teardown: ["npm run clean"],
      },
  ...
  it("rejects invalid service port ranges", () => {
    expect(() =>
      CodiusConfigRawSchema.parse({ worktree: { servicePorts: { range: "4000-3000" } } }),
