...
export const Route = createFileRoute("/changelog")({
  head: () =>
    pageMeta(
      "Changelog - Codius",
      "Product updates, bug fixes, and improvements shipped in each Codius release. Track new agent providers, mobile features, and daemon changes over time.",
      "/changelog",
    ),
  component: Changelog,
});
...
function HeadingAnchor({ version }: { version: string }) {
  ...
  return (
    <a
      ...
      aria-label={`Link to Codius ${version}`}
    >
    ...
    <article className="changelog-release">
      <div id={anchor} className="changelog-release-heading">
        ...
        <h2 className="changelog-release-title">
          Codius <span>{group.version}</span>
        </h2>
      </div>
