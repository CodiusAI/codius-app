...
const SITE_URL = "https://codius.ai";

const PRODUCT_PREAMBLE = `# Codius
...
Codius is an open source application that lets you run AI coding agents on your own machine and drive them from your phone, desktop, browser, or terminal. Your code stays local — Codius connects directly to your real development environment instead of running agents in someone else's cloud.

A self-hosted daemon manages agent lifecycle, exposes a WebSocket API, and ships with an MCP server so other agents can talk to it. Native apps for iOS, Android, macOS, Windows, Linux, and the web let you launch sessions, watch them work, review diffs, and ship from anywhere. A Docker-style CLI ("codius run", "codius ls", "codius logs", "codius wait") gives you scripting access. An end-to-end encrypted relay lets the mobile app reach your daemon over the public internet without exposing it.
...
- [Changelog](${SITE_URL}/changelog): Release notes for the Codius daemon, CLI, desktop, and mobile apps.
- [Download](${SITE_URL}/download): Install Codius on Mac, Windows, Linux, iOS, Android, or run the web app.
