...
buildNpmPackage {
  ...
  installPhase = ''
    ...
    ${lib.optionalString stdenv.hostPlatform.isLinux ''
      ...
      CODIUS_TRACE_DESKTOP=1 node scripts/trace-daemon.mjs > desktop-files.txt

      while IFS= read -r path; do
        [ -z "$path" ] && continue
        mkdir -p "$out/share/codius-app/$(dirname "$path")"
        cp -a "$path" "$out/share/codius-app/$path"
      done < desktop-files.txt

      # Keep the same unpackaged monorepo layout expected by main.js.
      cp package.json $out/share/codius-app/
      mkdir -p $out/share/codius-app/packages/app
      cp -a packages/app/dist $out/share/codius-app/packages/app/
      ...
      for runtime_path in \
        ...
        if [ ! -e "$out/share/codius-app/$runtime_path" ]; then
          echo "desktop runtime trace omitted $runtime_path" >&2
          exit 1
        fi
      done

      if [ -e $out/share/codius-app/node_modules/electron ]; then
      ...
      install -Dm644 packages/desktop/assets/codius-icon.png \
      ...
      # app root it launches. Point it at a one-file app named "codius-app"
      # so shells can match the window to the desktop entry and hicolor icon.
      mkdir -p $out/share/codius-app/electron-app
      printf '%s\n' "{ \"name\": \"codius-app\", \"version\": \"$version\", \"main\": \"index.js\" }" \
        > $out/share/codius-app/electron-app/package.json
      printf '%s\n' 'require("../packages/desktop/dist/main.js");' \
        > $out/share/codius-app/electron-app/index.js

      # Chromium's setuid sandbox cannot live in the immutable Nix store.
      makeWrapper ${electron}/bin/electron $out/bin/codius-app \
        --add-flags "$out/share/codius-app/electron-app" \
        --add-flags "--no-sandbox" \
        --add-flags "--class=codius-app" \
        --set EXPO_DEV_URL "codius://app/" \
        --set CHROME_DESKTOP "codius-app.desktop"

      copyDesktopItems
    ''}

    ${lib.optionalString stdenv.hostPlatform.isDarwin ''
      app="$(find packages/desktop/release -maxdepth 3 -type d -name Codius.app -print -quit)"
      if [ -z "$app" ]; then
        echo "electron-builder did not produce Codius.app" >&2
        exit 1
      fi
      mkdir -p "$out/Applications"
      cp -R "$app" "$out/Applications/Codius.app"
      ln -s ../Applications/Codius.app/Contents/MacOS/Codius "$out/bin/codius-app"
    ''}
  ...
  desktopItems = lib.optionals stdenv.hostPlatform.isLinux [
    ...
    # package.json `name` ("codius-app"), 38 uses the runtime app name that
    # main.ts sets ("Codius"). Ship a NoDisplay entry for the second spelling so
