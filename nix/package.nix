...
buildNpmPackage rec {
  ...
  installPhase = ''
    ...
    mkdir -p $out/lib/codius
    node scripts/trace-daemon.mjs > daemon-files.txt

    while IFS= read -r path; do
      [ -z "$path" ] && continue
      mkdir -p "$out/lib/codius/$(dirname "$path")"
      cp -a "$path" "$out/lib/codius/$path"
    done < daemon-files.txt

    # Root package.json lets node resolve the workspace layout when the
    # CLI/server bin starts from $out.
    cp package.json $out/lib/codius/

    # Web UI Assets
    cp -r packages/server/dist/server/web-ui $out/lib/codius/packages/server/dist/server/

    # Create wrapper for the server entry point (for systemd / direct use)
    mkdir -p $out/bin
    makeWrapper ${nodejs}/bin/node $out/bin/codius-server \
