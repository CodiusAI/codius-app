...
let
  cfg = config.services.codius;
in
{
  imports = [
    (lib.mkRenamedOptionModule [ "services" "codius" "allowedHosts" ] [ "services" "codius" "hostnames" ])
  ];

  options.services.codius = {
    enable = lib.mkEnableOption "Codius, a self-hosted daemon for AI coding agents";

    package = lib.mkPackageOption pkgs "codius" { };

    user = lib.mkOption {
      type = lib.types.str;
      default = "codius";
      description = "User account under which Codius runs.";
    };

    group = lib.mkOption {
      type = lib.types.str;
      default = "codius";
      description = "Group under which Codius runs.";
    };
    ...
    dataDir = lib.mkOption {
      ...
      default =
        if cfg.user == "codius"
        then "/var/lib/codius"
        else "/home/${cfg.user}/.codius";
      defaultText = lib.literalExpression ''
        if cfg.user == "codius"
        then "/var/lib/codius"
        else "/home/''${cfg.user}/.codius"
      '';
      description = "Directory for Codius state (CODIUS_HOME). Stores agent data, config, and logs.";
    };

    port = lib.mkOption {
      type = lib.types.port;
      default = 6767;
      description = "Port for the Codius daemon to listen on.";
    };

    listenAddress = lib.mkOption {
      type = lib.types.str;
      default = "127.0.0.1";
      description = "Address for the Codius daemon to bind to.";
    };

    openFirewall = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Whether to open the firewall for the Codius daemon port.";
    };
    ...
    hostnames = lib.mkOption {
      ...
      description = ''
        Hostnames the Codius daemon accepts in the Host header (DNS rebinding protection).
    ...
    relay = {
      ...
      mode = lib.mkOption {
        ...
        description = ''
          ...
          - `"hosted"` (default): use the upstream `app.codius.ai` relay.
            Preserves the current behavior; no extra options needed.
          - `"remote"`: connect to a self-hosted relay at
            `relay.host:relay.port`. Sets `CODIUS_RELAY_ENDPOINT` and
            `CODIUS_RELAY_USE_TLS` for the daemon.
    ...
    inheritUserEnvironment = lib.mkOption {
      type = lib.types.bool;
      default = cfg.user != "codius";
      defaultText = lib.literalExpression ''cfg.user != "codius"'';
      description = ''
        Whether to include the user's profile PATH in the service environment.

        When Codius runs as a real user (not the default system user), AI agents
    ...
    environment = lib.mkOption {
      ...
      example = lib.literalExpression ''
        {
          CODIUS_RELAY_ENDPOINT = "relay.codius.ai:443";
        }
      '';
      description = "Extra environment variables for the Codius daemon.";
    };
      ...
      example = lib.literalExpression ''
        {
          ...
          log.file = { level = "info"; path = "/var/lib/codius/daemon.log"; };
        }
      '';
      description = ''
        Declarative content for `$CODIUS_HOME/config.json`. Rendered to JSON
  ...
  config = lib.mkIf cfg.enable (
    let
      settingsFile = (pkgs.formats.json { }).generate "codius-config.json" cfg.settings;
    in
    {
    assertions = [
      {
        ...
        message = ''
          services.codius.relay.host must be set when relay.mode = "remote".
        '';
      }
    ];

    users.users.${cfg.user} = lib.mkIf (cfg.user == "codius") {
      isSystemUser = true;
      group = cfg.group;
      home = cfg.dataDir;
    };

    users.groups.${cfg.group} = lib.mkIf (cfg.group == "codius") { };

    systemd.tmpfiles.rules = [
      "d ${cfg.dataDir} 0700 ${cfg.user} ${cfg.group} - -"
    ];

    systemd.services.codius = {
      ...
      environment = {
        ...
        CODIUS_HOSTNAMES = "true";
      } // lib.optionalAttrs (lib.isList cfg.hostnames && cfg.hostnames != [ ]) {
        CODIUS_HOSTNAMES = lib.concatStringsSep "," cfg.hostnames;
      } // lib.optionalAttrs (cfg.relay.enable && cfg.relay.mode == "remote") {
        CODIUS_RELAY_ENDPOINT = "${cfg.relay.host}:${toString cfg.relay.port}";
        CODIUS_RELAY_USE_TLS = if cfg.relay.useTls then "true" else "false";
      } // lib.optionalAttrs (cfg.relay.enable && cfg.relay.mode == "remote" && cfg.relay.publicUseTls != null) {
        CODIUS_RELAY_PUBLIC_USE_TLS = if cfg.relay.publicUseTls then "true" else "false";
      } // cfg.environment;
