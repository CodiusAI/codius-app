...
if (process.env.CODIUS_SKIP_IOS_NATIVE_CACHE === "1") {
  process.exit(0);
}
...
function simulatorSlug() {
  const worktreeName = process.env.CODIUS_BRANCH_NAME || basename(targetRoot);
  const worktreeHash = createHash("sha1").update(targetRoot).digest("hex").slice(0, 8);
  const simulatorName = `Codius ${worktreeName} ${worktreeHash}`;
