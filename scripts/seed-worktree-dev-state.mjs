...
// one interpreter every Codius checkout already depends on.

const sourceRoot = process.env.CODIUS_SOURCE_CHECKOUT_PATH;
const targetRoot = process.env.CODIUS_WORKTREE_PATH || process.cwd();
...
function seedCodiusHome() {
  const source = process.env.CODIUS_DEV_SEED_HOME || join(sourceRoot, ".dev/codius-home");
  const target = join(targetRoot, ".dev/codius-home");
  ...
  if (process.env.CODIUS_DEV_RESET_HOME === "1") {
