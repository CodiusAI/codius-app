...
const appProductName = "CodiusDebug";
const appScheme = "codius";
const preferredSimulatorType = process.env.CODIUS_IOS_DEVICE_TYPE || "iPhone 16 Pro";
const codiusPort = requiredEnv("CODIUS_PORT");
const worktreePath = process.env.CODIUS_WORKTREE_PATH || rootDir;
const worktreeName = process.env.CODIUS_BRANCH_NAME || basename(worktreePath);
const worktreeHash = createHash("sha1").update(worktreePath).digest("hex").slice(0, 8);
const simulatorName =
  process.env.CODIUS_IOS_SIMULATOR_NAME || `Codius ${worktreeName} ${worktreeHash}`;
const daemonEndpoint =
  process.env.CODIUS_DEV_DAEMON_ENDPOINT ||
  `localhost:${process.env.CODIUS_SERVICE_DAEMON_PORT || "6768"}`;
...
async function main() {
  ...
  metro = startMetro();
  await waitForUrl(`http://127.0.0.1:${codiusPort}/.sim`);
  console.log(`iOS preview: ${process.env.CODIUS_URL || `http://127.0.0.1:${codiusPort}`}/.sim`);
...
function launchApp() {
  const metroUrl = encodeURIComponent(`http://127.0.0.1:${codiusPort}`);
  run(
    "xcrun",
...
function startMetro() {
  const child = spawn("npx", ["expo", "start", "--port", codiusPort, "--localhost"], {
    cwd: appDir,
    env: {
      ...env,
      CODIUS_SERVE_SIM_PREVIEW: "1",
      CODIUS_SERVE_SIM_DEVICE_UDID: simulatorUdid,
      BROWSER: "none",
    },
    stdio: "inherit",
  });
...
function requiredEnv(name) {
  ...
  if (!value) throw new Error(`${name} is required; run this as a Codius service.`);
  return value;
}
