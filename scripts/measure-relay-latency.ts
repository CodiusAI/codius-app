...
async function main() {
  ...
  await measurePings("Relay (relay.codius.ai:443)", relayClient, PING_COUNT, WARMUP_COUNT);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const ws = new WebSocket(
      `wss://relay.codius.ai/ws?serverId=latency_probe_${Date.now()}&role=client&clientId=probe_${i}`,
    );
    await new Promise<void>((resolve, reject) => {
      ws.on("open", () => {
