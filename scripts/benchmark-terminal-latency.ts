 ...
 *
 * Boots its OWN isolated daemon subprocess (fresh mkdtemp CODIUS_HOME, random
 ...
 *
 * Output: pretty table to stdout + JSON to /tmp/codius-terminal-bench/<ts>.json
 *
...
async function bootDaemon(): Promise<BootedDaemon> {
  ...
  const codiusHome = await mkdtemp(path.join(os.tmpdir(), "codius-bench-home-"));
  ...
  const child = spawn(tsxBin, ["scripts/supervisor-entrypoint.ts", "--dev"], {
    ...
    env: {
      ...
      CODIUS_SERVER_ID: "srv_terminal_bench",
  ...
  return { child, port, codiusHome, pid: child.pid ?? -1 };
}
...
async function main(): Promise<void> {
  ...
  console.log("Booting isolated daemon (random port, fresh CODIUS_HOME)...");
  ...
  try {
    ...
    console.log(`Daemon ready: pid=${daemon.pid} port=${daemon.port} home=${daemon.codiusHome}`);

    const ctor = await loadDaemonClientCtor();
    client = await connectClient(ctor, daemon.port);

    workspaceDir = await mkdtemp(path.join(os.tmpdir(), "codius-bench-ws-"));
    ...
    const outDir = "/tmp/codius-terminal-bench";
    ...
    if (daemon) {
      ...
      await rm(daemon.codiusHome, { recursive: true, force: true }).catch(() => undefined);
    }
    if (workspaceDir) {
